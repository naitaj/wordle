...
export class SedecordleAdapter implements GameAdapter {
  ...
  readBoard(boardIndex: number = 0): BoardState {
    // Sedecordle features 16 simultaneous boards with 21 guesses
    const boardContainers = Array.from(document.querySelectorAll('[id*="board"], [class*="board"], .grid'));
