//#region extension/src/solver/wordleRules.ts
function evaluateGuess(guess, target) {
	const g = guess.toUpperCase().split("");
	const t = target.toUpperCase().split("");
	const result = new Array(5).fill("absent");
	const targetPool = [...t];
	for (let i = 0; i < 5; i++) if (g[i] === t[i]) {
		result[i] = "correct";
		targetPool[i] = "";
	}
	for (let i = 0; i < 5; i++) {
		if (result[i] === "correct") continue;
		const poolIdx = targetPool.indexOf(g[i]);
		if (poolIdx !== -1) {
			result[i] = "present";
			targetPool[poolIdx] = "";
		}
	}
	return result;
}
function filterWordList(words, guess, result) {
	const letterMap = /* @__PURE__ */ new Map();
	for (let i = 0; i < 5; i++) {
		const letter = guess[i].toUpperCase();
		const res = result[i];
		const current = letterMap.get(letter) ?? {
			confirmed: 0,
			hasGrey: false
		};
		if (res === "correct" || res === "present") current.confirmed += 1;
		else current.hasGrey = true;
		letterMap.set(letter, current);
	}
	return words.filter((candidateStr) => {
		const candidate = candidateStr.toUpperCase();
		for (let i = 0; i < 5; i++) {
			if (result[i] === "correct" && candidate[i] !== guess[i].toUpperCase()) return false;
			if (result[i] === "present" && candidate[i] === guess[i].toUpperCase()) return false;
			if (result[i] === "absent" && candidate[i] === guess[i].toUpperCase()) return false;
		}
		for (const [letter, { confirmed, hasGrey }] of letterMap.entries()) {
			let countInCandidate = 0;
			for (const char of candidate) if (char === letter) countInCandidate++;
			if (confirmed === 0) {
				if (countInCandidate > 0) return false;
			} else if (hasGrey) {
				if (countInCandidate !== confirmed) return false;
			} else if (countInCandidate < confirmed) return false;
		}
		return true;
	});
}
//#endregion
//#region extension/src/solver/entropySolver.ts
function encodeResult(result) {
	return result.map((r) => r === "correct" ? "G" : r === "present" ? "Y" : "X").join("");
}
var patternCache = /* @__PURE__ */ new Map();
function getPattern(word, target) {
	const key = `${word}:${target}`;
	let pattern = patternCache.get(key);
	if (!pattern) {
		pattern = encodeResult(evaluateGuess(word, target));
		patternCache.set(key, pattern);
	}
	return pattern;
}
function computeEntropy(word, remainingWords) {
	const buckets = /* @__PURE__ */ new Map();
	for (const answer of remainingWords) {
		const key = getPattern(word, answer);
		buckets.set(key, (buckets.get(key) ?? 0) + 1);
	}
	let entropy = 0;
	for (const count of buckets.values()) {
		const p = count / remainingWords.length;
		entropy -= p * Math.log2(p);
	}
	return entropy;
}
/**
* Synchronous entropy ranking with deterministic tie-breaking.
* Used in the Chrome Extension service worker (no Web Workers available).
*/
function rankGuesses(candidates, fullDictionary) {
	let poolToScore = candidates;
	if (fullDictionary && candidates.length > 1) poolToScore = fullDictionary;
	else poolToScore = candidates.slice(0, 1500);
	const candidateSet = new Set(candidates.map((w) => w.toUpperCase()));
	const scored = [];
	for (let idx = 0; idx < poolToScore.length; idx++) {
		const word = poolToScore[idx].toUpperCase();
		const entropy = computeEntropy(word, candidates);
		scored.push({
			word,
			entropy,
			remainingCount: candidates.length,
			isCandidate: candidateSet.has(word),
			uniqueLetters: new Set(word.split("")).size,
			index: idx
		});
	}
	scored.sort((a, b) => {
		if (Math.abs(b.entropy - a.entropy) > 1e-6) return b.entropy - a.entropy;
		if (a.isCandidate !== b.isCandidate) return a.isCandidate ? -1 : 1;
		if (b.uniqueLetters !== a.uniqueLetters) return b.uniqueLetters - a.uniqueLetters;
		if (a.index !== b.index) return a.index - b.index;
		return a.word.localeCompare(b.word);
	});
	return scored.map((s) => ({
		word: s.word,
		entropy: s.entropy,
		remainingCount: s.remainingCount
	}));
}
/**
* Chunked async wrapper — yields to the event loop every CHUNK_SIZE words
* to keep the service worker responsive.
*/
async function rankGuessesAsync(candidates, fullDictionary) {
	if (candidates.length <= 200 && !fullDictionary) return rankGuesses(candidates, fullDictionary);
	await new Promise((resolve) => setTimeout(resolve, 0));
	return rankGuesses(candidates, fullDictionary);
}
/**
* Clear the pattern cache (useful between games).
*/
function clearPatternCache() {
	patternCache.clear();
}
//#endregion
//#region extension/src/solver/llmFallback.ts
function buildPrompt(guessHistory, targetHint, validWords) {
	const guessLines = guessHistory.map((g, i) => {
		const feedback = g.result.map((r, j) => {
			const letter = g.word[j];
			if (r === "correct") return `${letter}=GREEN`;
			if (r === "present") return `${letter}=YELLOW`;
			return `${letter}=GRAY`;
		}).join(", ");
		return `Guess ${i + 1}: ${g.word} → [${feedback}]`;
	}).join("\n");
	const knownPositions = [
		"",
		"",
		"",
		"",
		""
	];
	const presentLetters = /* @__PURE__ */ new Set();
	const absentLetters = /* @__PURE__ */ new Set();
	const badPositions = {};
	for (const g of guessHistory) for (let i = 0; i < 5; i++) {
		const char = g.word[i];
		const res = g.result[i];
		if (res === "correct") knownPositions[i] = char;
		else if (res === "present") {
			presentLetters.add(char);
			if (!badPositions[char]) badPositions[char] = /* @__PURE__ */ new Set();
			badPositions[char].add(i);
		} else if (res === "absent") absentLetters.add(char);
	}
	const trueAbsent = Array.from(absentLetters).filter((c) => !presentLetters.has(c) && !knownPositions.includes(c));
	const explicitConstraints = [
		...knownPositions.map((c, i) => c ? `- Position ${i + 1} MUST be ${c}` : "").filter(Boolean),
		...Array.from(presentLetters).map((c) => {
			return `- MUST contain ${c}, but NOT in position(s) ${Array.from(badPositions[c] || []).map((n) => n + 1).join(", ")}`;
		}),
		`- CANNOT contain: ${trueAbsent.length > 0 ? trueAbsent.join(", ") : "None"}`
	].join("\n");
	return `You are an expert Wordle player. I am playing Wordle and my entropy-based solver has run out of candidate words. I need you to suggest the best next 5-letter word guess.

Here is everything I know:

TARGET HINT: ${targetHint}

VALID WORDS REMAINING:
The ONLY dictionary words that perfectly match all Green/Yellow/Gray clues are:
[ ${validWords.length > 0 ? validWords.join(", ") : "NONE - guess any word to gather clues"} ]
You MUST pick your guess from the valid words list above if it is not empty.

GUESS HISTORY:
${guessLines || "(no guesses yet)"}

EXPLICIT LOGICAL CONSTRAINTS (You MUST follow these exactly):
${explicitConstraints}

RULES REMINDER:
- GREEN means the letter is in the correct position.
- YELLOW means the letter is in the word but in the wrong position.
- GRAY means the letter is not in the word (or all instances are accounted for).
- DO NOT guess any word that is already in the GUESS HISTORY.
- You MUST guess a real, valid dictionary word. Do not invent fake words.
- Do not guess insect, plant, or similar names. Use only Wordle-appropriate words.

FORBIDDEN WORDS:
${guessHistory.length > 0 ? guessHistory.map((g) => g.word).join(", ") : "None"}

IMPORTANT: Respond with ONLY the 5-letter word in uppercase, nothing else.`;
}
/**
* Get the Groq API key from chrome.storage.
*/
async function getApiKey() {
	return new Promise((resolve) => {
		chrome.storage.local.get(["groqApiKey"], (result) => {
			resolve(result.groqApiKey || null);
		});
	});
}
/**
* Call the Groq API to get a word suggestion.
* API key is retrieved from chrome.storage.local.
*/
async function askGrokForGuess(guessHistory, targetHint, validWords) {
	const apiKey = await getApiKey();
	if (!apiKey) return { error: "Groq API key not configured. Set it in the extension settings." };
	const messages = [{
		role: "system",
		content: "You are a Wordle-solving assistant. You respond with exactly one 5-letter uppercase dictionary word. No explanations, no formatting, just the word."
	}, {
		role: "user",
		content: buildPrompt(guessHistory, targetHint, validWords)
	}];
	try {
		const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: `Bearer ${apiKey}`
			},
			body: JSON.stringify({
				model: "llama-3.3-70b-versatile",
				messages,
				temperature: .2,
				max_tokens: 50
			})
		});
		if (!response.ok) {
			const errorText = await response.text();
			return { error: `Groq API error (${response.status}): ${errorText}` };
		}
		const data = await response.json();
		if (!data.choices || data.choices.length === 0) return { error: "Groq returned no choices." };
		const raw = data.choices[0].message.content.trim().toUpperCase();
		const match = raw.match(/[A-Z]{5}/);
		if (!match) return { error: `Groq returned invalid response: "${raw}"` };
		return {
			word: match[0],
			source: "grok"
		};
	} catch (err) {
		return { error: `Groq API call failed: ${err instanceof Error ? err.message : String(err)}` };
	}
}
//#endregion
//#region extension/src/data/wordList.ts
var answerList = [
	"ABACK",
	"ABASE",
	"ABATE",
	"ABBEY",
	"ABBOT",
	"ABHOR",
	"ABIDE",
	"ABLED",
	"ABODE",
	"ABORT",
	"ABOUT",
	"ABOVE",
	"ABUSE",
	"ABYSS",
	"ACORN",
	"ACRID",
	"ACTOR",
	"ACUTE",
	"ADAGE",
	"ADAPT",
	"ADEPT",
	"ADMIN",
	"ADMIT",
	"ADOBE",
	"ADOPT",
	"ADORE",
	"ADORN",
	"ADULT",
	"AFFIX",
	"AFIRE",
	"AFOOT",
	"AFOUL",
	"AFTER",
	"AGAIN",
	"AGAPE",
	"AGATE",
	"AGENT",
	"AGILE",
	"AGING",
	"AGLOW",
	"AGONY",
	"AGREE",
	"AHEAD",
	"AIDER",
	"AISLE",
	"ALARM",
	"ALBUM",
	"ALERT",
	"ALGAE",
	"ALIBI",
	"ALIEN",
	"ALIGN",
	"ALIKE",
	"ALIVE",
	"ALLAY",
	"ALLEY",
	"ALLOT",
	"ALLOW",
	"ALLOY",
	"ALOFT",
	"ALONE",
	"ALONG",
	"ALOOF",
	"ALOUD",
	"ALPHA",
	"ALTAR",
	"ALTER",
	"AMASS",
	"AMAZE",
	"AMBER",
	"AMBLE",
	"AMEND",
	"AMISS",
	"AMITY",
	"AMONG",
	"AMPLE",
	"AMPLY",
	"AMUSE",
	"ANGEL",
	"ANGER",
	"ANGLE",
	"ANGRY",
	"ANGST",
	"ANIME",
	"ANKLE",
	"ANNEX",
	"ANNOY",
	"ANNUL",
	"ANODE",
	"ANTIC",
	"ANVIL",
	"AORTA",
	"APART",
	"APHID",
	"APING",
	"APNEA",
	"APPLE",
	"APPLY",
	"APRON",
	"APTLY",
	"ARBOR",
	"ARDOR",
	"ARENA",
	"ARGUE",
	"ARISE",
	"ARMOR",
	"AROMA",
	"AROSE",
	"ARRAY",
	"ARROW",
	"ARSON",
	"ARTSY",
	"ASCOT",
	"ASHEN",
	"ASIDE",
	"ASKEW",
	"ASSAY",
	"ASSET",
	"ATOLL",
	"ATONE",
	"ATTIC",
	"AUDIO",
	"AUDIT",
	"AUGUR",
	"AUNTY",
	"AVAIL",
	"AVERT",
	"AVIAN",
	"AVOID",
	"AWAIT",
	"AWAKE",
	"AWARD",
	"AWARE",
	"AWASH",
	"AWFUL",
	"AWOKE",
	"AXIAL",
	"AXIOM",
	"AXION",
	"AZURE",
	"BACON",
	"BADGE",
	"BADLY",
	"BAGEL",
	"BAGGY",
	"BAKER",
	"BALER",
	"BALMY",
	"BANAL",
	"BANJO",
	"BARGE",
	"BARON",
	"BASAL",
	"BASIC",
	"BASIL",
	"BASIN",
	"BASIS",
	"BASTE",
	"BATCH",
	"BATHE",
	"BATON",
	"BATTY",
	"BAWDY",
	"BAYOU",
	"BEACH",
	"BEADY",
	"BEARD",
	"BEAST",
	"BEECH",
	"BEEFY",
	"BEFIT",
	"BEGAN",
	"BEGAT",
	"BEGET",
	"BEGIN",
	"BEGUN",
	"BEING",
	"BELCH",
	"BELIE",
	"BELLE",
	"BELLY",
	"BELOW",
	"BENCH",
	"BERET",
	"BERRY",
	"BERTH",
	"BESET",
	"BETEL",
	"BEVEL",
	"BEZEL",
	"BIBLE",
	"BICEP",
	"BIDDY",
	"BIGOT",
	"BILGE",
	"BILLY",
	"BINGE",
	"BINGO",
	"BIOME",
	"BIRCH",
	"BIRTH",
	"BISON",
	"BITTY",
	"BLACK",
	"BLADE",
	"BLAME",
	"BLAND",
	"BLANK",
	"BLARE",
	"BLAST",
	"BLAZE",
	"BLEAK",
	"BLEAT",
	"BLEED",
	"BLEEP",
	"BLEND",
	"BLESS",
	"BLIMP",
	"BLIND",
	"BLINK",
	"BLISS",
	"BLITZ",
	"BLOAT",
	"BLOCK",
	"BLOKE",
	"BLOND",
	"BLOOD",
	"BLOOM",
	"BLOWN",
	"BLUER",
	"BLUFF",
	"BLUNT",
	"BLURB",
	"BLURT",
	"BLUSH",
	"BOARD",
	"BOAST",
	"BOBBY",
	"BONEY",
	"BONGO",
	"BONUS",
	"BOOBY",
	"BOOST",
	"BOOTH",
	"BOOTY",
	"BOOZE",
	"BOOZY",
	"BORAX",
	"BORNE",
	"BOSOM",
	"BOSSY",
	"BOTCH",
	"BOUGH",
	"BOULE",
	"BOUND",
	"BOWEL",
	"BOXER",
	"BRACE",
	"BRAID",
	"BRAIN",
	"BRAKE",
	"BRAND",
	"BRASH",
	"BRASS",
	"BRAVE",
	"BRAVO",
	"BRAWL",
	"BRAWN",
	"BREAD",
	"BREAK",
	"BREED",
	"BRIAR",
	"BRIBE",
	"BRICK",
	"BRIDE",
	"BRIEF",
	"BRINE",
	"BRING",
	"BRINK",
	"BRINY",
	"BRISK",
	"BROAD",
	"BROIL",
	"BROKE",
	"BROOD",
	"BROOK",
	"BROOM",
	"BROTH",
	"BROWN",
	"BRUNT",
	"BRUSH",
	"BRUTE",
	"BUDDY",
	"BUDGE",
	"BUGGY",
	"BUGLE",
	"BUILD",
	"BUILT",
	"BULGE",
	"BULKY",
	"BULLY",
	"BUNCH",
	"BUNNY",
	"BURLY",
	"BURNT",
	"BURST",
	"BUSED",
	"BUSHY",
	"BUTCH",
	"BUTTE",
	"BUXOM",
	"BUYER",
	"BYLAW",
	"CABAL",
	"CABBY",
	"CABIN",
	"CABLE",
	"CACAO",
	"CACHE",
	"CACTI",
	"CADDY",
	"CADET",
	"CAGEY",
	"CAIRN",
	"CAMEL",
	"CAMEO",
	"CANAL",
	"CANDY",
	"CANNY",
	"CANOE",
	"CANON",
	"CAPER",
	"CAPUT",
	"CARAT",
	"CARGO",
	"CAROL",
	"CARRY",
	"CARVE",
	"CASTE",
	"CATCH",
	"CATER",
	"CATTY",
	"CAULK",
	"CAUSE",
	"CAVIL",
	"CEASE",
	"CEDAR",
	"CELLO",
	"CHAFE",
	"CHAFF",
	"CHAIN",
	"CHAIR",
	"CHALK",
	"CHAMP",
	"CHANT",
	"CHAOS",
	"CHARD",
	"CHARM",
	"CHART",
	"CHASE",
	"CHASM",
	"CHEAP",
	"CHEAT",
	"CHECK",
	"CHEEK",
	"CHEER",
	"CHESS",
	"CHEST",
	"CHICK",
	"CHIDE",
	"CHIEF",
	"CHILD",
	"CHILI",
	"CHILL",
	"CHIME",
	"CHINA",
	"CHIRP",
	"CHOCK",
	"CHOIR",
	"CHOKE",
	"CHORD",
	"CHORE",
	"CHOSE",
	"CHUCK",
	"CHUMP",
	"CHUNK",
	"CHURN",
	"CHUTE",
	"CIDER",
	"CIGAR",
	"CINCH",
	"CIRCA",
	"CIVIC",
	"CIVIL",
	"CLACK",
	"CLAIM",
	"CLAMP",
	"CLANG",
	"CLANK",
	"CLASH",
	"CLASP",
	"CLASS",
	"CLEAN",
	"CLEAR",
	"CLEAT",
	"CLEFT",
	"CLERK",
	"CLICK",
	"CLIFF",
	"CLIMB",
	"CLING",
	"CLINK",
	"CLOAK",
	"CLOCK",
	"CLONE",
	"CLOSE",
	"CLOTH",
	"CLOUD",
	"CLOUT",
	"CLOVE",
	"CLOWN",
	"CLUCK",
	"CLUED",
	"CLUMP",
	"CLUNG",
	"COACH",
	"COAST",
	"COBRA",
	"COCOA",
	"COLON",
	"COLOR",
	"COMET",
	"COMFY",
	"COMIC",
	"COMMA",
	"CONCH",
	"CONDO",
	"CONIC",
	"COPSE",
	"CORAL",
	"CORER",
	"CORNY",
	"COUCH",
	"COUGH",
	"COULD",
	"COUNT",
	"COUPE",
	"COURT",
	"COVEN",
	"COVER",
	"COVET",
	"COVEY",
	"COWER",
	"COYLY",
	"CRACK",
	"CRAFT",
	"CRAMP",
	"CRANE",
	"CRANK",
	"CRASH",
	"CRASS",
	"CRATE",
	"CRAVE",
	"CRAWL",
	"CRAZE",
	"CRAZY",
	"CREAK",
	"CREAM",
	"CREDO",
	"CREED",
	"CREEK",
	"CREEP",
	"CREME",
	"CREPE",
	"CREPT",
	"CRESS",
	"CREST",
	"CRICK",
	"CRIED",
	"CRIER",
	"CRIME",
	"CRIMP",
	"CRISP",
	"CROAK",
	"CROCK",
	"CRONE",
	"CRONY",
	"CROOK",
	"CROSS",
	"CROUP",
	"CROWD",
	"CROWN",
	"CRUDE",
	"CRUEL",
	"CRUMB",
	"CRUMP",
	"CRUSH",
	"CRUST",
	"CRYPT",
	"CUBIC",
	"CUMIN",
	"CURIO",
	"CURLY",
	"CURRY",
	"CURSE",
	"CURVE",
	"CURVY",
	"CUTIE",
	"CYBER",
	"CYCLE",
	"CYNIC",
	"DADDY",
	"DAILY",
	"DAIRY",
	"DAISY",
	"DALLY",
	"DANCE",
	"DANDY",
	"DATUM",
	"DAUNT",
	"DEALT",
	"DEATH",
	"DEBAR",
	"DEBIT",
	"DEBUG",
	"DEBUT",
	"DECAL",
	"DECAY",
	"DECOR",
	"DECOY",
	"DECRY",
	"DEFER",
	"DEIGN",
	"DEITY",
	"DELAY",
	"DELTA",
	"DELVE",
	"DEMON",
	"DEMUR",
	"DENIM",
	"DENSE",
	"DEPOT",
	"DEPTH",
	"DERBY",
	"DETER",
	"DETOX",
	"DEUCE",
	"DEVIL",
	"DIARY",
	"DICEY",
	"DIGIT",
	"DILLY",
	"DIMLY",
	"DINER",
	"DINGO",
	"DINGY",
	"DIODE",
	"DIRGE",
	"DIRTY",
	"DISCO",
	"DITCH",
	"DITTO",
	"DITTY",
	"DIVER",
	"DIZZY",
	"DODGE",
	"DODGY",
	"DOGMA",
	"DOING",
	"DOLLY",
	"DONOR",
	"DONUT",
	"DOPEY",
	"DOUBT",
	"DOUGH",
	"DOWDY",
	"DOWEL",
	"DOWNY",
	"DOWRY",
	"DOZEN",
	"DRAFT",
	"DRAIN",
	"DRAKE",
	"DRAMA",
	"DRANK",
	"DRAPE",
	"DRAWL",
	"DRAWN",
	"DREAD",
	"DREAM",
	"DRESS",
	"DRIED",
	"DRIER",
	"DRIFT",
	"DRILL",
	"DRINK",
	"DRIVE",
	"DROIT",
	"DROLL",
	"DRONE",
	"DROOL",
	"DROOP",
	"DROSS",
	"DROVE",
	"DROWN",
	"DRUID",
	"DRUNK",
	"DRYER",
	"DRYLY",
	"DUCHY",
	"DULLY",
	"DUMMY",
	"DUMPY",
	"DUNCE",
	"DUSKY",
	"DUSTY",
	"DUTCH",
	"DUVET",
	"DWARF",
	"DWELL",
	"DWELT",
	"DYING",
	"EAGER",
	"EAGLE",
	"EARLY",
	"EARTH",
	"EASEL",
	"EATEN",
	"EATER",
	"EBONY",
	"ECLAT",
	"EDICT",
	"EDIFY",
	"EERIE",
	"EGRET",
	"EIGHT",
	"EJECT",
	"EKING",
	"ELATE",
	"ELBOW",
	"ELDER",
	"ELECT",
	"ELEGY",
	"ELFIN",
	"ELIDE",
	"ELITE",
	"ELOPE",
	"ELUDE",
	"EMAIL",
	"EMBED",
	"EMBER",
	"EMCEE",
	"EMPTY",
	"ENACT",
	"ENDOW",
	"ENEMA",
	"ENEMY",
	"ENJOY",
	"ENNUI",
	"ENSUE",
	"ENTER",
	"ENTRY",
	"ENVOY",
	"EPOCH",
	"EPOXY",
	"EQUAL",
	"EQUIP",
	"ERASE",
	"ERECT",
	"ERODE",
	"ERROR",
	"ERUPT",
	"ESSAY",
	"ESTER",
	"ETHER",
	"ETHIC",
	"ETHOS",
	"ETUDE",
	"EVADE",
	"EVENT",
	"EVERY",
	"EVICT",
	"EVOKE",
	"EXACT",
	"EXALT",
	"EXCEL",
	"EXERT",
	"EXILE",
	"EXIST",
	"EXPEL",
	"EXTOL",
	"EXTRA",
	"EXULT",
	"EYING",
	"FABLE",
	"FACET",
	"FAINT",
	"FAIRY",
	"FAITH",
	"FALSE",
	"FANCY",
	"FANNY",
	"FARCE",
	"FATAL",
	"FATTY",
	"FAULT",
	"FAUNA",
	"FAVOR",
	"FEAST",
	"FECAL",
	"FEIGN",
	"FELLA",
	"FELON",
	"FEMME",
	"FEMUR",
	"FENCE",
	"FERAL",
	"FERRY",
	"FETAL",
	"FETCH",
	"FETID",
	"FETUS",
	"FEVER",
	"FEWER",
	"FIBER",
	"FICUS",
	"FIELD",
	"FIEND",
	"FIERY",
	"FIFTH",
	"FIFTY",
	"FIGHT",
	"FILER",
	"FILET",
	"FILLY",
	"FILMY",
	"FILTH",
	"FINAL",
	"FINCH",
	"FINER",
	"FIRST",
	"FISHY",
	"FIXER",
	"FIZZY",
	"FJORD",
	"FLACK",
	"FLAIL",
	"FLAIR",
	"FLAKE",
	"FLAKY",
	"FLAME",
	"FLANK",
	"FLARE",
	"FLASH",
	"FLASK",
	"FLECK",
	"FLEET",
	"FLESH",
	"FLICK",
	"FLIER",
	"FLING",
	"FLINT",
	"FLIRT",
	"FLOAT",
	"FLOCK",
	"FLOOD",
	"FLOOR",
	"FLORA",
	"FLOSS",
	"FLOUR",
	"FLOUT",
	"FLOWN",
	"FLUFF",
	"FLUID",
	"FLUKE",
	"FLUME",
	"FLUNG",
	"FLUNK",
	"FLUSH",
	"FLUTE",
	"FLYER",
	"FOAMY",
	"FOCAL",
	"FOCUS",
	"FOGGY",
	"FOIST",
	"FOLIO",
	"FOLLY",
	"FORAY",
	"FORCE",
	"FORGE",
	"FORGO",
	"FORTE",
	"FORTH",
	"FORTY",
	"FORUM",
	"FOUND",
	"FOYER",
	"FRAIL",
	"FRAME",
	"FRANK",
	"FRAUD",
	"FREAK",
	"FREED",
	"FREER",
	"FRESH",
	"FRIAR",
	"FRIED",
	"FRILL",
	"FRISK",
	"FRITZ",
	"FROCK",
	"FROND",
	"FRONT",
	"FROST",
	"FROTH",
	"FROWN",
	"FROZE",
	"FRUIT",
	"FUDGE",
	"FUGUE",
	"FULLY",
	"FUNGI",
	"FUNKY",
	"FUNNY",
	"FUROR",
	"FURRY",
	"FUSSY",
	"FUZZY",
	"GAFFE",
	"GAILY",
	"GAMER",
	"GAMMA",
	"GAMUT",
	"GASSY",
	"GAUDY",
	"GAUGE",
	"GAUNT",
	"GAUZE",
	"GAVEL",
	"GAWKY",
	"GAYER",
	"GAYLY",
	"GAZER",
	"GECKO",
	"GEEKY",
	"GEESE",
	"GENIE",
	"GENRE",
	"GHOST",
	"GHOUL",
	"GIANT",
	"GIDDY",
	"GIPSY",
	"GIRLY",
	"GIRTH",
	"GIVEN",
	"GIVER",
	"GLADE",
	"GLAND",
	"GLARE",
	"GLASS",
	"GLAZE",
	"GLEAM",
	"GLEAN",
	"GLIDE",
	"GLINT",
	"GLOAT",
	"GLOBE",
	"GLOOM",
	"GLORY",
	"GLOSS",
	"GLOVE",
	"GLYPH",
	"GNASH",
	"GNOME",
	"GODLY",
	"GOING",
	"GOLEM",
	"GOLLY",
	"GONAD",
	"GONER",
	"GOODY",
	"GOOEY",
	"GOOFY",
	"GOOSE",
	"GORGE",
	"GOUGE",
	"GOURD",
	"GRACE",
	"GRADE",
	"GRAFT",
	"GRAIL",
	"GRAIN",
	"GRAND",
	"GRANT",
	"GRAPE",
	"GRAPH",
	"GRASP",
	"GRASS",
	"GRATE",
	"GRAVE",
	"GRAVY",
	"GRAZE",
	"GREAT",
	"GREED",
	"GREEN",
	"GREET",
	"GRIEF",
	"GRILL",
	"GRIME",
	"GRIMY",
	"GRIND",
	"GRIPE",
	"GROAN",
	"GROIN",
	"GROOM",
	"GROPE",
	"GROSS",
	"GROUP",
	"GROUT",
	"GROVE",
	"GROWL",
	"GROWN",
	"GRUEL",
	"GRUFF",
	"GRUNT",
	"GUARD",
	"GUAVA",
	"GUESS",
	"GUEST",
	"GUIDE",
	"GUILD",
	"GUILE",
	"GUILT",
	"GUISE",
	"GULCH",
	"GULLY",
	"GUMBO",
	"GUMMY",
	"GUPPY",
	"GUSTO",
	"GUSTY",
	"GYPSY",
	"HABIT",
	"HAIRY",
	"HALVE",
	"HANDY",
	"HAPPY",
	"HARDY",
	"HAREM",
	"HARPY",
	"HARRY",
	"HARSH",
	"HASTE",
	"HASTY",
	"HATCH",
	"HATER",
	"HAUNT",
	"HAUTE",
	"HAVEN",
	"HAVOC",
	"HAZEL",
	"HEADY",
	"HEARD",
	"HEART",
	"HEATH",
	"HEAVE",
	"HEAVY",
	"HEDGE",
	"HEFTY",
	"HEIST",
	"HELIX",
	"HELLO",
	"HENCE",
	"HERON",
	"HILLY",
	"HINGE",
	"HIPPO",
	"HIPPY",
	"HITCH",
	"HOARD",
	"HOBBY",
	"HOIST",
	"HOLLY",
	"HOMER",
	"HONEY",
	"HONOR",
	"HORDE",
	"HORNY",
	"HORSE",
	"HOTEL",
	"HOTLY",
	"HOUND",
	"HOUSE",
	"HOVEL",
	"HOVER",
	"HOWDY",
	"HUMAN",
	"HUMID",
	"HUMOR",
	"HUMPH",
	"HUMUS",
	"HUNCH",
	"HUNKY",
	"HURRY",
	"HUSKY",
	"HUSSY",
	"HUTCH",
	"HYDRO",
	"HYENA",
	"HYMEN",
	"HYPER",
	"ICILY",
	"ICING",
	"IDEAL",
	"IDIOM",
	"IDIOT",
	"IDLER",
	"IDYLL",
	"IGLOO",
	"ILIAC",
	"IMAGE",
	"IMBUE",
	"IMPEL",
	"IMPLY",
	"INANE",
	"INBOX",
	"INCUR",
	"INDEX",
	"INEPT",
	"INERT",
	"INFER",
	"INGOT",
	"INLAY",
	"INLET",
	"INNER",
	"INPUT",
	"INTER",
	"INTRO",
	"IONIC",
	"IRATE",
	"IRONY",
	"ISLET",
	"ISSUE",
	"ITCHY",
	"IVORY",
	"JAUNT",
	"JAZZY",
	"JELLY",
	"JERKY",
	"JETTY",
	"JEWEL",
	"JIFFY",
	"JOINT",
	"JOIST",
	"JOKER",
	"JOLLY",
	"JOUST",
	"JUDGE",
	"JUICE",
	"JUICY",
	"JUMBO",
	"JUMPY",
	"JUNTA",
	"JUNTO",
	"JUROR",
	"KAPPA",
	"KARMA",
	"KAYAK",
	"KEBAB",
	"KHAKI",
	"KINKY",
	"KIOSK",
	"KITTY",
	"KNACK",
	"KNAVE",
	"KNEAD",
	"KNEED",
	"KNEEL",
	"KNELT",
	"KNIFE",
	"KNOCK",
	"KNOLL",
	"KNOWN",
	"KOALA",
	"KRILL",
	"LABEL",
	"LABOR",
	"LADEN",
	"LADLE",
	"LAGER",
	"LANCE",
	"LANKY",
	"LAPEL",
	"LAPSE",
	"LARGE",
	"LARVA",
	"LASSO",
	"LATCH",
	"LATER",
	"LATHE",
	"LATTE",
	"LAUGH",
	"LAYER",
	"LEACH",
	"LEAFY",
	"LEAKY",
	"LEANT",
	"LEAPT",
	"LEARN",
	"LEASE",
	"LEASH",
	"LEAST",
	"LEAVE",
	"LEDGE",
	"LEECH",
	"LEERY",
	"LEFTY",
	"LEGAL",
	"LEGGY",
	"LEMON",
	"LEMUR",
	"LEPER",
	"LEVEL",
	"LEVER",
	"LIBEL",
	"LIEGE",
	"LIGHT",
	"LIKEN",
	"LILAC",
	"LIMBO",
	"LIMIT",
	"LINEN",
	"LINER",
	"LINGO",
	"LIPID",
	"LITHE",
	"LIVER",
	"LIVID",
	"LLAMA",
	"LOAMY",
	"LOATH",
	"LOBBY",
	"LOCAL",
	"LOCUS",
	"LODGE",
	"LOFTY",
	"LOGIC",
	"LOGIN",
	"LOOPY",
	"LOOSE",
	"LORRY",
	"LOSER",
	"LOUSE",
	"LOUSY",
	"LOVER",
	"LOWER",
	"LOWLY",
	"LOYAL",
	"LUCID",
	"LUCKY",
	"LUMEN",
	"LUMPY",
	"LUNAR",
	"LUNCH",
	"LUNGE",
	"LUPUS",
	"LURCH",
	"LURID",
	"LUSTY",
	"LYING",
	"LYMPH",
	"LYRIC",
	"MACAW",
	"MACHO",
	"MACRO",
	"MADAM",
	"MADLY",
	"MAFIA",
	"MAGIC",
	"MAGMA",
	"MAIZE",
	"MAJOR",
	"MAKER",
	"MAMBO",
	"MAMMA",
	"MAMMY",
	"MANGA",
	"MANGE",
	"MANGO",
	"MANGY",
	"MANIA",
	"MANIC",
	"MANLY",
	"MANOR",
	"MAPLE",
	"MARCH",
	"MARRY",
	"MARSH",
	"MASON",
	"MASSE",
	"MATCH",
	"MATEY",
	"MAUVE",
	"MAXIM",
	"MAYBE",
	"MAYOR",
	"MEALY",
	"MEANT",
	"MEATY",
	"MECCA",
	"MEDAL",
	"MEDIA",
	"MEDIC",
	"MELEE",
	"MELON",
	"MERCY",
	"MERGE",
	"MERIT",
	"MERRY",
	"METAL",
	"METER",
	"METRO",
	"MICRO",
	"MIDGE",
	"MIDST",
	"MIGHT",
	"MILKY",
	"MIMIC",
	"MINCE",
	"MINER",
	"MINIM",
	"MINOR",
	"MINTY",
	"MINUS",
	"MIRTH",
	"MISER",
	"MISSY",
	"MOCHA",
	"MODAL",
	"MODEL",
	"MODEM",
	"MOGUL",
	"MOIST",
	"MOLAR",
	"MOLDY",
	"MONEY",
	"MONTH",
	"MOODY",
	"MOOSE",
	"MORAL",
	"MORON",
	"MORPH",
	"MOSSY",
	"MOTEL",
	"MOTIF",
	"MOTOR",
	"MOTTO",
	"MOULT",
	"MOUND",
	"MOUNT",
	"MOURN",
	"MOUSE",
	"MOUTH",
	"MOVER",
	"MOVIE",
	"MOWER",
	"MUCKY",
	"MUCUS",
	"MUDDY",
	"MULCH",
	"MUMMY",
	"MUNCH",
	"MURAL",
	"MURKY",
	"MUSHY",
	"MUSIC",
	"MUSKY",
	"MUSTY",
	"MYRRH",
	"NADIR",
	"NAIVE",
	"NANNY",
	"NASAL",
	"NASTY",
	"NATAL",
	"NAVAL",
	"NAVEL",
	"NEEDY",
	"NEIGH",
	"NERDY",
	"NERVE",
	"NEVER",
	"NEWER",
	"NEWLY",
	"NICER",
	"NICHE",
	"NIECE",
	"NIGHT",
	"NINJA",
	"NINNY",
	"NINTH",
	"NOBLE",
	"NOBLY",
	"NOISE",
	"NOISY",
	"NOMAD",
	"NOOSE",
	"NORTH",
	"NOSEY",
	"NOTCH",
	"NOVEL",
	"NUDGE",
	"NURSE",
	"NUTTY",
	"NYLON",
	"NYMPH",
	"OAKEN",
	"OBESE",
	"OCCUR",
	"OCEAN",
	"OCTAL",
	"OCTET",
	"ODDER",
	"ODDLY",
	"OFFAL",
	"OFFER",
	"OFTEN",
	"OLDEN",
	"OLDER",
	"OLIVE",
	"OMBRE",
	"OMEGA",
	"ONION",
	"ONSET",
	"OPERA",
	"OPINE",
	"OPIUM",
	"OPTIC",
	"ORBIT",
	"ORDER",
	"ORGAN",
	"OTHER",
	"OTTER",
	"OUGHT",
	"OUNCE",
	"OUTDO",
	"OUTER",
	"OUTGO",
	"OVARY",
	"OVATE",
	"OVERT",
	"OVINE",
	"OVOID",
	"OWING",
	"OWNER",
	"OXIDE",
	"OZONE",
	"PADDY",
	"PAGAN",
	"PAINT",
	"PALER",
	"PALSY",
	"PANEL",
	"PANIC",
	"PANSY",
	"PAPAL",
	"PAPER",
	"PARER",
	"PARKA",
	"PARRY",
	"PARSE",
	"PARTY",
	"PASTA",
	"PASTE",
	"PASTY",
	"PATCH",
	"PATIO",
	"PATSY",
	"PATTY",
	"PAUSE",
	"PAYEE",
	"PAYER",
	"PEACE",
	"PEACH",
	"PEARL",
	"PECAN",
	"PEDAL",
	"PENAL",
	"PENCE",
	"PENNE",
	"PENNY",
	"PERCH",
	"PERIL",
	"PERKY",
	"PESKY",
	"PESTO",
	"PETAL",
	"PETTY",
	"PHASE",
	"PHONE",
	"PHONY",
	"PHOTO",
	"PIANO",
	"PICKY",
	"PIECE",
	"PIETY",
	"PIGGY",
	"PILOT",
	"PINCH",
	"PINEY",
	"PINKY",
	"PINTO",
	"PIPER",
	"PIQUE",
	"PITCH",
	"PITHY",
	"PIVOT",
	"PIXEL",
	"PIXIE",
	"PIZZA",
	"PLACE",
	"PLAID",
	"PLAIN",
	"PLAIT",
	"PLANE",
	"PLANK",
	"PLANT",
	"PLATE",
	"PLAZA",
	"PLEAD",
	"PLEAT",
	"PLIED",
	"PLIER",
	"PLUCK",
	"PLUMB",
	"PLUME",
	"PLUMP",
	"PLUNK",
	"PLUSH",
	"POESY",
	"POINT",
	"POISE",
	"POKER",
	"POLAR",
	"POLKA",
	"POLYP",
	"POOCH",
	"POPPY",
	"PORCH",
	"POSER",
	"POSIT",
	"POSSE",
	"POUCH",
	"POUND",
	"POUTY",
	"POWER",
	"PRANK",
	"PRAWN",
	"PREEN",
	"PRESS",
	"PRICE",
	"PRICK",
	"PRIDE",
	"PRIED",
	"PRIME",
	"PRIMO",
	"PRINT",
	"PRIOR",
	"PRISM",
	"PRIVY",
	"PRIZE",
	"PROBE",
	"PRONE",
	"PRONG",
	"PROOF",
	"PROSE",
	"PROUD",
	"PROVE",
	"PROWL",
	"PROXY",
	"PRUDE",
	"PRUNE",
	"PSALM",
	"PUBIC",
	"PUDGY",
	"PUFFY",
	"PULPY",
	"PULSE",
	"PUNCH",
	"PUPIL",
	"PUPPY",
	"PUREE",
	"PURER",
	"PURGE",
	"PURSE",
	"PUSHY",
	"PUTTY",
	"PYGMY",
	"QUACK",
	"QUAIL",
	"QUAKE",
	"QUALM",
	"QUARK",
	"QUART",
	"QUASH",
	"QUASI",
	"QUEEN",
	"QUEER",
	"QUELL",
	"QUERY",
	"QUEST",
	"QUEUE",
	"QUICK",
	"QUIET",
	"QUILL",
	"QUILT",
	"QUIRK",
	"QUITE",
	"QUOTA",
	"QUOTE",
	"QUOTH",
	"RABBI",
	"RABID",
	"RACER",
	"RADAR",
	"RADII",
	"RADIO",
	"RAINY",
	"RAISE",
	"RAJAH",
	"RALLY",
	"RALPH",
	"RAMEN",
	"RANCH",
	"RANDY",
	"RANGE",
	"RAPID",
	"RARER",
	"RASPY",
	"RATIO",
	"RATTY",
	"RAVEN",
	"RAYON",
	"RAZOR",
	"REACH",
	"REACT",
	"READY",
	"REALM",
	"REARM",
	"REBAR",
	"REBEL",
	"REBUS",
	"REBUT",
	"RECAP",
	"RECUR",
	"RECUT",
	"REEDY",
	"REFER",
	"REFIT",
	"REGAL",
	"REHAB",
	"REIGN",
	"RELAX",
	"RELAY",
	"RELIC",
	"REMIT",
	"RENAL",
	"RENEW",
	"REPAY",
	"REPEL",
	"REPLY",
	"RERUN",
	"RESET",
	"RESIN",
	"RETCH",
	"RETRO",
	"RETRY",
	"REUSE",
	"REVEL",
	"REVUE",
	"RHINO",
	"RHYME",
	"RIDER",
	"RIDGE",
	"RIFLE",
	"RIGHT",
	"RIGID",
	"RIGOR",
	"RINSE",
	"RIPEN",
	"RIPER",
	"RISEN",
	"RISER",
	"RISKY",
	"RIVAL",
	"RIVER",
	"RIVET",
	"ROACH",
	"ROAST",
	"ROBIN",
	"ROBOT",
	"ROCKY",
	"RODEO",
	"ROGER",
	"ROGUE",
	"ROOMY",
	"ROOST",
	"ROTOR",
	"ROUGE",
	"ROUGH",
	"ROUND",
	"ROUSE",
	"ROUTE",
	"ROVER",
	"ROWDY",
	"ROWER",
	"ROYAL",
	"RUDDY",
	"RUDER",
	"RUGBY",
	"RULER",
	"RUMBA",
	"RUMOR",
	"RUPEE",
	"RURAL",
	"RUSTY",
	"SADLY",
	"SAFER",
	"SAINT",
	"SALAD",
	"SALLY",
	"SALON",
	"SALSA",
	"SALTY",
	"SALVE",
	"SALVO",
	"SANDY",
	"SANER",
	"SAPPY",
	"SASSY",
	"SATIN",
	"SATYR",
	"SAUCE",
	"SAUCY",
	"SAUNA",
	"SAUTE",
	"SAVOR",
	"SAVOY",
	"SAVVY",
	"SCALD",
	"SCALE",
	"SCALP",
	"SCALY",
	"SCAMP",
	"SCANT",
	"SCARE",
	"SCARF",
	"SCARY",
	"SCENE",
	"SCENT",
	"SCION",
	"SCOFF",
	"SCOLD",
	"SCONE",
	"SCOOP",
	"SCOPE",
	"SCORE",
	"SCORN",
	"SCOUR",
	"SCOUT",
	"SCOWL",
	"SCRAM",
	"SCRAP",
	"SCREE",
	"SCREW",
	"SCRUB",
	"SCRUM",
	"SCUBA",
	"SEDAN",
	"SEEDY",
	"SEGUE",
	"SEIZE",
	"SEMEN",
	"SENSE",
	"SEPIA",
	"SERIF",
	"SERUM",
	"SERVE",
	"SETUP",
	"SEVEN",
	"SEVER",
	"SEWER",
	"SHACK",
	"SHADE",
	"SHADY",
	"SHAFT",
	"SHAKE",
	"SHAKY",
	"SHALE",
	"SHALL",
	"SHALT",
	"SHAME",
	"SHANK",
	"SHAPE",
	"SHARD",
	"SHARE",
	"SHARK",
	"SHARP",
	"SHAVE",
	"SHAWL",
	"SHEAR",
	"SHEEN",
	"SHEEP",
	"SHEER",
	"SHEET",
	"SHEIK",
	"SHELF",
	"SHELL",
	"SHIED",
	"SHIFT",
	"SHINE",
	"SHINY",
	"SHIRE",
	"SHIRK",
	"SHIRT",
	"SHOAL",
	"SHOCK",
	"SHONE",
	"SHOOK",
	"SHOOT",
	"SHORE",
	"SHORN",
	"SHORT",
	"SHOUT",
	"SHOVE",
	"SHOWN",
	"SHOWY",
	"SHREW",
	"SHRUB",
	"SHRUG",
	"SHUCK",
	"SHUNT",
	"SHUSH",
	"SHYLY",
	"SIEGE",
	"SIEVE",
	"SIGHT",
	"SIGMA",
	"SILKY",
	"SILLY",
	"SINCE",
	"SINEW",
	"SINGE",
	"SIREN",
	"SISSY",
	"SIXTH",
	"SIXTY",
	"SKATE",
	"SKIER",
	"SKIFF",
	"SKILL",
	"SKIMP",
	"SKIRT",
	"SKULK",
	"SKULL",
	"SKUNK",
	"SLACK",
	"SLAIN",
	"SLANG",
	"SLANT",
	"SLASH",
	"SLATE",
	"SLEEK",
	"SLEEP",
	"SLEET",
	"SLEPT",
	"SLICE",
	"SLICK",
	"SLIDE",
	"SLIME",
	"SLIMY",
	"SLING",
	"SLINK",
	"SLOOP",
	"SLOPE",
	"SLOSH",
	"SLOTH",
	"SLUMP",
	"SLUNG",
	"SLUNK",
	"SLURP",
	"SLUSH",
	"SLYLY",
	"SMACK",
	"SMALL",
	"SMART",
	"SMASH",
	"SMEAR",
	"SMELL",
	"SMELT",
	"SMILE",
	"SMIRK",
	"SMITE",
	"SMITH",
	"SMOCK",
	"SMOKE",
	"SMOKY",
	"SMOTE",
	"SNACK",
	"SNAIL",
	"SNAKE",
	"SNAKY",
	"SNARE",
	"SNARL",
	"SNEAK",
	"SNEER",
	"SNIDE",
	"SNIFF",
	"SNIPE",
	"SNOOP",
	"SNORE",
	"SNORT",
	"SNOUT",
	"SNOWY",
	"SNUCK",
	"SNUFF",
	"SOAPY",
	"SOBER",
	"SOGGY",
	"SOLAR",
	"SOLID",
	"SOLVE",
	"SONAR",
	"SONIC",
	"SOOTH",
	"SOOTY",
	"SORRY",
	"SOUND",
	"SOUTH",
	"SOWER",
	"SPACE",
	"SPADE",
	"SPANK",
	"SPARE",
	"SPARK",
	"SPASM",
	"SPAWN",
	"SPEAK",
	"SPEAR",
	"SPECK",
	"SPEED",
	"SPELL",
	"SPELT",
	"SPEND",
	"SPENT",
	"SPERM",
	"SPICE",
	"SPICY",
	"SPIED",
	"SPIEL",
	"SPIKE",
	"SPIKY",
	"SPILL",
	"SPILT",
	"SPINE",
	"SPINY",
	"SPIRE",
	"SPITE",
	"SPLAT",
	"SPLIT",
	"SPOIL",
	"SPOKE",
	"SPOOF",
	"SPOOK",
	"SPOOL",
	"SPOON",
	"SPORE",
	"SPORT",
	"SPOUT",
	"SPRAY",
	"SPREE",
	"SPRIG",
	"SPUNK",
	"SPURN",
	"SPURT",
	"SQUAD",
	"SQUAT",
	"SQUIB",
	"STACK",
	"STAFF",
	"STAGE",
	"STAID",
	"STAIN",
	"STAIR",
	"STAKE",
	"STALE",
	"STALK",
	"STALL",
	"STAMP",
	"STAND",
	"STANK",
	"STARE",
	"STARK",
	"START",
	"STASH",
	"STATE",
	"STAVE",
	"STEAD",
	"STEAK",
	"STEAL",
	"STEAM",
	"STEED",
	"STEEL",
	"STEEP",
	"STEER",
	"STEIN",
	"STERN",
	"STICK",
	"STIFF",
	"STILL",
	"STILT",
	"STING",
	"STINK",
	"STINT",
	"STOCK",
	"STOIC",
	"STOKE",
	"STOLE",
	"STOMP",
	"STONE",
	"STONY",
	"STOOD",
	"STOOL",
	"STOOP",
	"STORE",
	"STORK",
	"STORM",
	"STORY",
	"STOUT",
	"STOVE",
	"STRAP",
	"STRAW",
	"STRAY",
	"STRIP",
	"STRUT",
	"STUCK",
	"STUDY",
	"STUFF",
	"STUMP",
	"STUNG",
	"STUNK",
	"STUNT",
	"STYLE",
	"SUAVE",
	"SUGAR",
	"SUING",
	"SUITE",
	"SULKY",
	"SULLY",
	"SUMAC",
	"SUNNY",
	"SUPER",
	"SURER",
	"SURGE",
	"SURLY",
	"SUSHI",
	"SWAMI",
	"SWAMP",
	"SWARM",
	"SWASH",
	"SWATH",
	"SWEAR",
	"SWEAT",
	"SWEEP",
	"SWEET",
	"SWELL",
	"SWEPT",
	"SWIFT",
	"SWILL",
	"SWINE",
	"SWING",
	"SWIRL",
	"SWISH",
	"SWOON",
	"SWOOP",
	"SWORD",
	"SWORE",
	"SWORN",
	"SWUNG",
	"SYNOD",
	"SYRUP",
	"TABBY",
	"TABLE",
	"TABOO",
	"TACIT",
	"TACKY",
	"TAFFY",
	"TAINT",
	"TAKEN",
	"TAKER",
	"TALLY",
	"TALON",
	"TAMER",
	"TANGO",
	"TANGY",
	"TAPER",
	"TAPIR",
	"TARDY",
	"TAROT",
	"TASTE",
	"TASTY",
	"TATTY",
	"TAUNT",
	"TAWNY",
	"TEACH",
	"TEARY",
	"TEASE",
	"TEDDY",
	"TEETH",
	"TEMPO",
	"TENET",
	"TENOR",
	"TENSE",
	"TENTH",
	"TEPEE",
	"TEPID",
	"TERRA",
	"TERSE",
	"TESTY",
	"THANK",
	"THEFT",
	"THEIR",
	"THEME",
	"THERE",
	"THESE",
	"THETA",
	"THICK",
	"THIEF",
	"THIGH",
	"THING",
	"THINK",
	"THIRD",
	"THONG",
	"THORN",
	"THOSE",
	"THREE",
	"THREW",
	"THROB",
	"THROW",
	"THRUM",
	"THUMB",
	"THUMP",
	"THYME",
	"TIARA",
	"TIBIA",
	"TIDAL",
	"TIGER",
	"TIGHT",
	"TILDE",
	"TIMER",
	"TIMID",
	"TIPSY",
	"TITAN",
	"TITHE",
	"TITLE",
	"TOAST",
	"TODAY",
	"TODDY",
	"TOKEN",
	"TONAL",
	"TONGA",
	"TONIC",
	"TOOTH",
	"TOPAZ",
	"TOPIC",
	"TORCH",
	"TORSO",
	"TORUS",
	"TOTAL",
	"TOTEM",
	"TOUCH",
	"TOUGH",
	"TOWEL",
	"TOWER",
	"TOXIC",
	"TOXIN",
	"TRACE",
	"TRACK",
	"TRACT",
	"TRADE",
	"TRAIL",
	"TRAIN",
	"TRAIT",
	"TRAMP",
	"TRASH",
	"TRAWL",
	"TREAD",
	"TREAT",
	"TREND",
	"TRIAD",
	"TRIAL",
	"TRIBE",
	"TRICE",
	"TRICK",
	"TRIED",
	"TRIPE",
	"TRITE",
	"TROLL",
	"TROOP",
	"TROPE",
	"TROUT",
	"TROVE",
	"TRUCE",
	"TRUCK",
	"TRUER",
	"TRULY",
	"TRUMP",
	"TRUNK",
	"TRUSS",
	"TRUST",
	"TRUTH",
	"TRYST",
	"TUBAL",
	"TUBER",
	"TULIP",
	"TULLE",
	"TUMOR",
	"TUNIC",
	"TURBO",
	"TUTOR",
	"TWANG",
	"TWEAK",
	"TWEED",
	"TWEET",
	"TWICE",
	"TWINE",
	"TWIRL",
	"TWIST",
	"TWIXT",
	"TYING",
	"UDDER",
	"ULCER",
	"ULTRA",
	"UMBRA",
	"UNCLE",
	"UNCUT",
	"UNDER",
	"UNDID",
	"UNDUE",
	"UNFED",
	"UNFIT",
	"UNIFY",
	"UNION",
	"UNITE",
	"UNITY",
	"UNLIT",
	"UNMET",
	"UNSET",
	"UNTIE",
	"UNTIL",
	"UNWED",
	"UNZIP",
	"UPPER",
	"UPSET",
	"URBAN",
	"URINE",
	"USAGE",
	"USHER",
	"USING",
	"USUAL",
	"USURP",
	"UTILE",
	"UTTER",
	"VAGUE",
	"VALET",
	"VALID",
	"VALOR",
	"VALUE",
	"VALVE",
	"VAPID",
	"VAPOR",
	"VAULT",
	"VAUNT",
	"VEGAN",
	"VENOM",
	"VENUE",
	"VERGE",
	"VERSE",
	"VERSO",
	"VERVE",
	"VICAR",
	"VIDEO",
	"VIGIL",
	"VIGOR",
	"VILLA",
	"VINYL",
	"VIOLA",
	"VIPER",
	"VIRAL",
	"VIRUS",
	"VISIT",
	"VISOR",
	"VISTA",
	"VITAL",
	"VIVID",
	"VIXEN",
	"VOCAL",
	"VODKA",
	"VOGUE",
	"VOICE",
	"VOILA",
	"VOMIT",
	"VOTER",
	"VOUCH",
	"VOWEL",
	"VYING",
	"WACKY",
	"WAFER",
	"WAGER",
	"WAGON",
	"WAIST",
	"WAIVE",
	"WALTZ",
	"WARTY",
	"WASTE",
	"WATCH",
	"WATER",
	"WAVER",
	"WAXEN",
	"WEARY",
	"WEAVE",
	"WEDGE",
	"WEEDY",
	"WEIGH",
	"WEIRD",
	"WELCH",
	"WELSH",
	"WHACK",
	"WHALE",
	"WHARF",
	"WHEAT",
	"WHEEL",
	"WHELP",
	"WHERE",
	"WHICH",
	"WHIFF",
	"WHILE",
	"WHINE",
	"WHINY",
	"WHIRL",
	"WHISK",
	"WHITE",
	"WHOLE",
	"WHOOP",
	"WHOSE",
	"WIDEN",
	"WIDER",
	"WIDOW",
	"WIDTH",
	"WIELD",
	"WIGHT",
	"WILLY",
	"WIMPY",
	"WINCE",
	"WINCH",
	"WINDY",
	"WISER",
	"WISPY",
	"WITCH",
	"WITTY",
	"WOKEN",
	"WOMAN",
	"WOMEN",
	"WOODY",
	"WOOER",
	"WOOLY",
	"WOOZY",
	"WORDY",
	"WORLD",
	"WORRY",
	"WORSE",
	"WORST",
	"WORTH",
	"WOULD",
	"WOUND",
	"WOVEN",
	"WRACK",
	"WRATH",
	"WREAK",
	"WRECK",
	"WREST",
	"WRING",
	"WRIST",
	"WRITE",
	"WRONG",
	"WROTE",
	"WRUNG",
	"WRYLY",
	"YACHT",
	"YEARN",
	"YEAST",
	"YIELD",
	"YOUNG",
	"YOUTH",
	"ZEBRA",
	"ZESTY",
	"ZONAL",
	"SWIPE"
];
var wordList = [
	"AAPAS",
	"ABACA",
	"ABACI",
	"ABACK",
	"ABACS",
	"ABAFT",
	"ABAMP",
	"ABAND",
	"ABASE",
	"ABASH",
	"ABATE",
	"ABBES",
	"ABBEY",
	"ABBOT",
	"ABCEE",
	"ABEAR",
	"ABERS",
	"ABETS",
	"ABEYS",
	"ABHOR",
	"ABIDE",
	"ABIES",
	"ABIUS",
	"ABLED",
	"ABLER",
	"ABLES",
	"ABLET",
	"ABLOW",
	"ABNET",
	"ABODE",
	"ABOMA",
	"ABOON",
	"ABORN",
	"ABORT",
	"ABOUT",
	"ABOVE",
	"ABRAY",
	"ABRIN",
	"ABRIS",
	"ABSIT",
	"ABUNA",
	"ABUNE",
	"ABURA",
	"ABUSE",
	"ABUTS",
	"ABYSS",
	"ACARA",
	"ACARI",
	"ACCHA",
	"ACCOY",
	"ACENE",
	"ACERS",
	"ACHES",
	"ACHOO",
	"ACIES",
	"ACING",
	"ACKEE",
	"ACKER",
	"ACMES",
	"ACNES",
	"ACOCK",
	"ACORN",
	"ACRED",
	"ACRES",
	"ACRID",
	"ACRON",
	"ACROS",
	"ACTIN",
	"ACTOR",
	"ACUTE",
	"ACYLS",
	"ADAGE",
	"ADAPT",
	"ADATS",
	"ADAWN",
	"ADAWS",
	"ADAYS",
	"ADDED",
	"ADDIN",
	"ADDIO",
	"ADEAD",
	"ADEPT",
	"ADIEU",
	"ADIOS",
	"ADITS",
	"ADLIB",
	"ADMEN",
	"ADMIN",
	"ADMIT",
	"ADOBE",
	"ADOON",
	"ADOPT",
	"ADORE",
	"ADORN",
	"ADOWN",
	"ADOZE",
	"ADRAD",
	"ADRAW",
	"ADRET",
	"ADRIP",
	"ADUKI",
	"ADULT",
	"ADUNC",
	"ADUST",
	"ADVEW",
	"ADYTS",
	"ADZES",
	"AEDES",
	"AERIE",
	"AEROS",
	"AFARA",
	"AFARS",
	"AFEAR",
	"AFFIX",
	"AFION",
	"AFIRE",
	"AFLAJ",
	"AFOAM",
	"AFOOT",
	"AFORE",
	"AFOUL",
	"AFRIT",
	"AFROS",
	"AFTER",
	"AFTOS",
	"AGAIN",
	"AGAMA",
	"AGAMI",
	"AGAPE",
	"AGARS",
	"AGAST",
	"AGATE",
	"AGATY",
	"AGENT",
	"AGERS",
	"AGGAG",
	"AGGER",
	"AGGRO",
	"AGHAS",
	"AGILA",
	"AGILE",
	"AGING",
	"AGISM",
	"AGIST",
	"AGITA",
	"AGLOW",
	"AGOGE",
	"AGONS",
	"AGONY",
	"AGOOD",
	"AGORA",
	"AGREE",
	"AGRUM",
	"AGUED",
	"AGUTI",
	"AHEAD",
	"AHIND",
	"AHINT",
	"AHOLD",
	"AHOLE",
	"AHULL",
	"AIDAS",
	"AIDER",
	"AIDES",
	"AIGAS",
	"AIMAG",
	"AIMER",
	"AINGA",
	"AIRED",
	"AIRER",
	"AISLE",
	"AITUS",
	"AIYEE",
	"AIZLE",
	"AJUGA",
	"AJWAN",
	"AKARA",
	"AKENE",
	"AKING",
	"AKITA",
	"AKKAS",
	"AKOIA",
	"AKOYA",
	"AKSES",
	"ALALA",
	"ALAMO",
	"ALANE",
	"ALANG",
	"ALANS",
	"ALANT",
	"ALAPS",
	"ALARM",
	"ALATA",
	"ALAYS",
	"ALBAS",
	"ALBEE",
	"ALBID",
	"ALBUM",
	"ALCEA",
	"ALCID",
	"ALDEA",
	"ALDER",
	"ALECK",
	"ALEEM",
	"ALEFS",
	"ALERT",
	"ALGAE",
	"ALGAL",
	"ALGID",
	"ALGIN",
	"ALGOR",
	"ALGOS",
	"ALIBI",
	"ALICK",
	"ALIEN",
	"ALIGN",
	"ALIKE",
	"ALINE",
	"ALIOS",
	"ALIVE",
	"ALKIN",
	"ALKYD",
	"ALKYL",
	"ALLAY",
	"ALLEE",
	"ALLEN",
	"ALLEY",
	"ALLIN",
	"ALLOD",
	"ALLOT",
	"ALLOW",
	"ALLOY",
	"ALLUS",
	"ALLYL",
	"ALMAH",
	"ALMES",
	"ALMUD",
	"ALMUG",
	"ALOFT",
	"ALOHA",
	"ALONE",
	"ALONG",
	"ALOOF",
	"ALOOS",
	"ALOSE",
	"ALOUD",
	"ALOWE",
	"ALPHA",
	"ALTAR",
	"ALTER",
	"ALTHO",
	"ALULA",
	"ALURE",
	"AMAHS",
	"AMARI",
	"AMASS",
	"AMAZE",
	"AMBER",
	"AMBIT",
	"AMBLE",
	"AMBOS",
	"AMEND",
	"AMENE",
	"AMENT",
	"AMICE",
	"AMIDO",
	"AMIES",
	"AMIGO",
	"AMINE",
	"AMISS",
	"AMITY",
	"AMMAN",
	"AMMAS",
	"AMMOS",
	"AMNIC",
	"AMOKS",
	"AMOLE",
	"AMONG",
	"AMORE",
	"AMORT",
	"AMOUR",
	"AMOVE",
	"AMPLE",
	"AMPLY",
	"AMPUL",
	"AMRIT",
	"AMUSE",
	"AMYLS",
	"ANANA",
	"ANCLE",
	"ANDIC",
	"ANEAR",
	"ANELE",
	"ANENT",
	"ANGAS",
	"ANGEL",
	"ANGER",
	"ANGLE",
	"ANGRY",
	"ANGST",
	"ANILE",
	"ANILS",
	"ANIME",
	"ANISE",
	"ANKER",
	"ANKHS",
	"ANKLE",
	"ANKUS",
	"ANLAS",
	"ANNEX",
	"ANNOY",
	"ANNUL",
	"ANNUM",
	"ANOAS",
	"ANODE",
	"ANOLE",
	"ANOMY",
	"ANSAS",
	"ANTED",
	"ANTES",
	"ANTIC",
	"ANTRE",
	"ANTSY",
	"ANVIL",
	"ANYON",
	"AORTA",
	"APAGE",
	"APART",
	"APAYD",
	"APERS",
	"APERT",
	"APERY",
	"APGAR",
	"APHID",
	"APING",
	"APIOL",
	"APISH",
	"APNEA",
	"APODE",
	"APODS",
	"APOLS",
	"APORT",
	"APPAL",
	"APPAM",
	"APPAY",
	"APPEL",
	"APPLE",
	"APPLY",
	"APPRO",
	"APRON",
	"APSIS",
	"APSOS",
	"APTER",
	"APTLY",
	"AQUAS",
	"ARAKS",
	"ARARS",
	"ARBAS",
	"ARBOR",
	"ARCED",
	"ARCUS",
	"ARDEB",
	"ARDOR",
	"ARDRI",
	"AREAL",
	"AREAS",
	"ARECA",
	"AREFY",
	"ARENA",
	"ARETS",
	"ARETT",
	"ARGAL",
	"ARGIL",
	"ARGLE",
	"ARGOT",
	"ARGUE",
	"ARHAT",
	"ARIAS",
	"ARIEL",
	"ARIKI",
	"ARILS",
	"ARISE",
	"ARISH",
	"ARKED",
	"ARLES",
	"ARMED",
	"ARMER",
	"ARMIL",
	"ARMOR",
	"ARNIS",
	"ARNUT",
	"AROBA",
	"AROHA",
	"AROMA",
	"AROSE",
	"ARPEN",
	"ARRAY",
	"ARRIS",
	"ARROW",
	"ARSON",
	"ARTAL",
	"ARTER",
	"ARTLY",
	"ARTSY",
	"ARUMS",
	"ARVEE",
	"ARYLS",
	"ASADA",
	"ASANA",
	"ASCON",
	"ASCOT",
	"ASHEN",
	"ASHES",
	"ASHET",
	"ASIDE",
	"ASKAR",
	"ASKED",
	"ASKER",
	"ASKEW",
	"ASKOS",
	"ASPER",
	"ASPIC",
	"ASPIE",
	"ASPRO",
	"ASSAI",
	"ASSAM",
	"ASSAY",
	"ASSED",
	"ASSES",
	"ASSET",
	"ASSEZ",
	"ASSOT",
	"ASWAY",
	"ASWIM",
	"ASYLA",
	"ATAXY",
	"ATIMY",
	"ATMAN",
	"ATMOS",
	"ATOCS",
	"ATOKE",
	"ATOLL",
	"ATONE",
	"ATONY",
	"ATOPY",
	"ATRIP",
	"ATTAS",
	"ATTER",
	"ATTIC",
	"ATUAS",
	"AUDIO",
	"AUDIT",
	"AUGEN",
	"AUGES",
	"AUGUR",
	"AULAS",
	"AUMIL",
	"AUNTS",
	"AUNTY",
	"AURAE",
	"AURAR",
	"AURAS",
	"AUREI",
	"AURES",
	"AURUM",
	"AUTOS",
	"AVAIL",
	"AVALE",
	"AVAST",
	"AVELS",
	"AVENS",
	"AVERS",
	"AVERT",
	"AVGAS",
	"AVIAN",
	"AVION",
	"AVISE",
	"AVIZE",
	"AVOID",
	"AVYZE",
	"AWAIT",
	"AWAKE",
	"AWARD",
	"AWARE",
	"AWARI",
	"AWARN",
	"AWASH",
	"AWATO",
	"AWAVE",
	"AWAYS",
	"AWEEL",
	"AWETO",
	"AWFUL",
	"AWING",
	"AWKIN",
	"AWNED",
	"AWOKE",
	"AWOLS",
	"AWORK",
	"AXIAL",
	"AXILE",
	"AXILS",
	"AXING",
	"AXIOM",
	"AXION",
	"AXITE",
	"AXLES",
	"AXMEN",
	"AXONE",
	"AYAHS",
	"AYAYA",
	"AYGRE",
	"AYINS",
	"AYRIE",
	"AZIDE",
	"AZIDO",
	"AZINE",
	"AZLON",
	"AZOIC",
	"AZOLE",
	"AZOTH",
	"AZUKI",
	"AZURE",
	"AZURN",
	"AZURY",
	"AZYME",
	"AZYMS",
	"BAAED",
	"BAALS",
	"BABEL",
	"BABES",
	"BABKA",
	"BABUL",
	"BABUS",
	"BACHA",
	"BACKY",
	"BACON",
	"BADDY",
	"BADGE",
	"BADLY",
	"BAELS",
	"BAFTA",
	"BAFTS",
	"BAGEL",
	"BAGGY",
	"BAGIE",
	"BAGSY",
	"BAHUS",
	"BAILE",
	"BAILS",
	"BAIRN",
	"BAITH",
	"BAIZA",
	"BAIZE",
	"BAJRA",
	"BAJRI",
	"BAKED",
	"BAKER",
	"BALAS",
	"BALDS",
	"BALER",
	"BALKY",
	"BALLY",
	"BALMS",
	"BALMY",
	"BALOI",
	"BALON",
	"BALOO",
	"BALOT",
	"BALTI",
	"BALUN",
	"BALUS",
	"BALUT",
	"BAMBI",
	"BANAL",
	"BANCO",
	"BANDH",
	"BANDS",
	"BANED",
	"BANES",
	"BANGS",
	"BANJO",
	"BANTY",
	"BANYA",
	"BAOZI",
	"BAPUS",
	"BARBE",
	"BARBY",
	"BARCA",
	"BARDE",
	"BARDS",
	"BARDY",
	"BARED",
	"BARER",
	"BARES",
	"BARFS",
	"BARFY",
	"BARGE",
	"BARKY",
	"BARMS",
	"BARMY",
	"BARNS",
	"BARNY",
	"BARON",
	"BARRO",
	"BARYE",
	"BASAL",
	"BASAN",
	"BASED",
	"BASHA",
	"BASHO",
	"BASIC",
	"BASIJ",
	"BASIL",
	"BASIN",
	"BASIS",
	"BASKS",
	"BASON",
	"BASSE",
	"BASSI",
	"BASTA",
	"BASTE",
	"BASTS",
	"BATCH",
	"BATED",
	"BATES",
	"BATHE",
	"BATHS",
	"BATIK",
	"BATON",
	"BATTA",
	"BATTU",
	"BATTY",
	"BAUDS",
	"BAUKS",
	"BAWDY",
	"BAWLS",
	"BAWRS",
	"BAWTY",
	"BAYER",
	"BAYES",
	"BAYLE",
	"BAYOU",
	"BAYTS",
	"BAZAR",
	"BAZAS",
	"BBALL",
	"BEACH",
	"BEADS",
	"BEADY",
	"BEAKS",
	"BEAKY",
	"BEALS",
	"BEANS",
	"BEANY",
	"BEARD",
	"BEARE",
	"BEAST",
	"BEATS",
	"BEATY",
	"BEAUT",
	"BEBOP",
	"BECKS",
	"BEDAD",
	"BEDEW",
	"BEECH",
	"BEEDI",
	"BEEFY",
	"BEFIT",
	"BEGAD",
	"BEGAN",
	"BEGAT",
	"BEGET",
	"BEGIN",
	"BEGOB",
	"BEGOT",
	"BEGUM",
	"BEGUN",
	"BEIGE",
	"BEING",
	"BEISA",
	"BEKAH",
	"BELAH",
	"BELAR",
	"BELAY",
	"BELCH",
	"BELEE",
	"BELGA",
	"BELIE",
	"BELIT",
	"BELLE",
	"BELLO",
	"BELLS",
	"BELLY",
	"BELON",
	"BELOW",
	"BELTS",
	"BEMAD",
	"BEMIX",
	"BENCH",
	"BENDS",
	"BENDY",
	"BENES",
	"BENNI",
	"BENNY",
	"BENTY",
	"BEPAT",
	"BERES",
	"BERET",
	"BERGS",
	"BERKO",
	"BERMS",
	"BERRY",
	"BERTH",
	"BESEE",
	"BESES",
	"BESET",
	"BESOM",
	"BESOT",
	"BESTI",
	"BESTS",
	"BETAS",
	"BETEL",
	"BETHS",
	"BETID",
	"BETON",
	"BETTY",
	"BEVAN",
	"BEVEL",
	"BEVER",
	"BEVUE",
	"BEWET",
	"BEWIG",
	"BEZEL",
	"BEZES",
	"BEZIL",
	"BHAIS",
	"BHAJI",
	"BHANG",
	"BHATS",
	"BHAVA",
	"BHELS",
	"BHOOT",
	"BHUTS",
	"BIACH",
	"BIALY",
	"BIBBS",
	"BIBLE",
	"BICEP",
	"BIDDY",
	"BIDER",
	"BIDES",
	"BIDET",
	"BIDIS",
	"BIDON",
	"BIDRI",
	"BIFFO",
	"BIFFS",
	"BIFFY",
	"BIFID",
	"BIGAE",
	"BIGHT",
	"BIGOT",
	"BIHON",
	"BIJOU",
	"BIKER",
	"BIKES",
	"BIKKY",
	"BILAT",
	"BILBO",
	"BILBY",
	"BILED",
	"BILES",
	"BILGE",
	"BILLY",
	"BIMBO",
	"BINDI",
	"BINER",
	"BINGE",
	"BINGO",
	"BINGS",
	"BINGY",
	"BINKS",
	"BINKY",
	"BIOGS",
	"BIOME",
	"BIONS",
	"BIOTA",
	"BIPED",
	"BIPPY",
	"BIRCH",
	"BIRIS",
	"BIRKS",
	"BIRLS",
	"BIRTH",
	"BIRZZ",
	"BISES",
	"BISKS",
	"BISON",
	"BITCH",
	"BITEY",
	"BITOS",
	"BITTS",
	"BITTY",
	"BIVVY",
	"BIZES",
	"BIZZO",
	"BIZZY",
	"BLACK",
	"BLADE",
	"BLAER",
	"BLAES",
	"BLAHS",
	"BLAME",
	"BLAMS",
	"BLAND",
	"BLANK",
	"BLARE",
	"BLASH",
	"BLAST",
	"BLATE",
	"BLAUD",
	"BLAWN",
	"BLAWS",
	"BLAZE",
	"BLEAK",
	"BLEAT",
	"BLEED",
	"BLEEP",
	"BLEND",
	"BLESS",
	"BLEST",
	"BLETS",
	"BLEYS",
	"BLIMP",
	"BLIND",
	"BLINK",
	"BLINS",
	"BLIPS",
	"BLISS",
	"BLITE",
	"BLITZ",
	"BLIVE",
	"BLOAT",
	"BLOBS",
	"BLOCK",
	"BLOCS",
	"BLOGS",
	"BLOKE",
	"BLOND",
	"BLONX",
	"BLOOD",
	"BLOOK",
	"BLOOM",
	"BLOWN",
	"BLOWS",
	"BLUDY",
	"BLUER",
	"BLUET",
	"BLUEY",
	"BLUFF",
	"BLUID",
	"BLUNK",
	"BLUNT",
	"BLURB",
	"BLURS",
	"BLURT",
	"BLUSH",
	"BLYPE",
	"BOABS",
	"BOAKS",
	"BOARD",
	"BOARS",
	"BOAST",
	"BOATS",
	"BOBAC",
	"BOBAK",
	"BOBAS",
	"BOBBY",
	"BOBOS",
	"BOCCA",
	"BOCCI",
	"BOCHE",
	"BODED",
	"BODES",
	"BODGE",
	"BODLE",
	"BODOH",
	"BOEPS",
	"BOERS",
	"BOGGY",
	"BOGLE",
	"BOGUS",
	"BOHEA",
	"BOHOS",
	"BOINK",
	"BOITE",
	"BOKED",
	"BOKOS",
	"BOLAR",
	"BOLAS",
	"BOLDO",
	"BOLDS",
	"BOLES",
	"BOLIX",
	"BOLKS",
	"BOLUS",
	"BOMBO",
	"BOMBS",
	"BOMOH",
	"BONDS",
	"BONES",
	"BONEY",
	"BONGO",
	"BONGS",
	"BONIE",
	"BONUS",
	"BOOAY",
	"BOOBY",
	"BOODY",
	"BOOHS",
	"BOOLS",
	"BOOMY",
	"BOONS",
	"BOORD",
	"BOORS",
	"BOOST",
	"BOOTH",
	"BOOTY",
	"BOOZE",
	"BOOZY",
	"BORAK",
	"BORAL",
	"BORAS",
	"BORAX",
	"BORDE",
	"BORDS",
	"BORED",
	"BORES",
	"BORKS",
	"BORMS",
	"BORNA",
	"BORNE",
	"BORON",
	"BOSEY",
	"BOSKS",
	"BOSOM",
	"BOSON",
	"BOSSA",
	"BOSSY",
	"BOSUN",
	"BOTAS",
	"BOTCH",
	"BOTEH",
	"BOTEL",
	"BOTEW",
	"BOTTE",
	"BOUGE",
	"BOUGH",
	"BOULE",
	"BOUND",
	"BOURD",
	"BOURN",
	"BOUSE",
	"BOUTU",
	"BOWED",
	"BOWEL",
	"BOWER",
	"BOWIE",
	"BOWLS",
	"BOWRS",
	"BOXEN",
	"BOXER",
	"BOYAR",
	"BOYAU",
	"BOYLA",
	"BOYOS",
	"BOZOS",
	"BRAAI",
	"BRACE",
	"BRACH",
	"BRACK",
	"BRACT",
	"BRAES",
	"BRAID",
	"BRAIL",
	"BRAIN",
	"BRAKE",
	"BRAKS",
	"BRAND",
	"BRANK",
	"BRASH",
	"BRASS",
	"BRAVE",
	"BRAVO",
	"BRAWL",
	"BRAWN",
	"BRAWS",
	"BRAXY",
	"BRAYS",
	"BRAZA",
	"BRAZE",
	"BREAD",
	"BREAK",
	"BREDS",
	"BREED",
	"BREES",
	"BREME",
	"BRENS",
	"BRENT",
	"BRERE",
	"BREVE",
	"BREWS",
	"BREYS",
	"BRIAR",
	"BRIBE",
	"BRICK",
	"BRIDE",
	"BRIEF",
	"BRIGS",
	"BRIKI",
	"BRILL",
	"BRIMS",
	"BRINE",
	"BRING",
	"BRINK",
	"BRINY",
	"BRIOS",
	"BRISK",
	"BRITH",
	"BRITT",
	"BROAD",
	"BROCH",
	"BROCK",
	"BRODS",
	"BROGH",
	"BROGS",
	"BROIL",
	"BROKE",
	"BROME",
	"BROND",
	"BROOD",
	"BROOK",
	"BROOM",
	"BROSE",
	"BROSY",
	"BROTH",
	"BROWN",
	"BRUCK",
	"BRUGH",
	"BRUHS",
	"BRUIN",
	"BRUJA",
	"BRUJO",
	"BRUNG",
	"BRUNT",
	"BRUSH",
	"BRUSK",
	"BRUST",
	"BRUTE",
	"BRUTS",
	"BUAZE",
	"BUBBA",
	"BUBBE",
	"BUBBY",
	"BUCHU",
	"BUCKO",
	"BUCKS",
	"BUCKU",
	"BUDAS",
	"BUDDY",
	"BUDED",
	"BUDES",
	"BUDGE",
	"BUDOS",
	"BUFFE",
	"BUGGY",
	"BUGLE",
	"BUHLS",
	"BUILD",
	"BUILT",
	"BUKES",
	"BULBS",
	"BULGE",
	"BULKS",
	"BULKY",
	"BULLS",
	"BULLY",
	"BUMPH",
	"BUNAS",
	"BUNCE",
	"BUNCH",
	"BUNCO",
	"BUNDE",
	"BUNDH",
	"BUNDT",
	"BUNDY",
	"BUNGS",
	"BUNGY",
	"BUNIA",
	"BUNJY",
	"BUNKO",
	"BUNNS",
	"BUNNY",
	"BUNYA",
	"BURAN",
	"BURBS",
	"BURET",
	"BURFI",
	"BURGH",
	"BURGS",
	"BURIN",
	"BURKS",
	"BURLS",
	"BURLY",
	"BURNT",
	"BURPS",
	"BURRA",
	"BURRO",
	"BURRY",
	"BURSA",
	"BURST",
	"BUSED",
	"BUSHY",
	"BUSKS",
	"BUSKY",
	"BUSSU",
	"BUSTI",
	"BUTCH",
	"BUTEO",
	"BUTES",
	"BUTOH",
	"BUTTE",
	"BUTTS",
	"BUTUT",
	"BUTYL",
	"BUXOM",
	"BUYER",
	"BUZZY",
	"BWANA",
	"BWAZI",
	"BYDED",
	"BYDES",
	"BYKED",
	"BYKES",
	"BYLAW",
	"BYSSI",
	"CAAED",
	"CABAL",
	"CABBY",
	"CABIN",
	"CABLE",
	"CABOB",
	"CACAO",
	"CACHE",
	"CACKY",
	"CACTI",
	"CADDY",
	"CADEE",
	"CADES",
	"CADET",
	"CADIS",
	"CAESE",
	"CAFFE",
	"CAFFS",
	"CAGES",
	"CAGEY",
	"CAIRD",
	"CAIRN",
	"CAJON",
	"CAKES",
	"CAKEY",
	"CALFS",
	"CALIF",
	"CALLA",
	"CALLS",
	"CALMY",
	"CALPS",
	"CAMEL",
	"CAMEO",
	"CAMIS",
	"CAMOS",
	"CAMPO",
	"CAMPS",
	"CAMPY",
	"CAMUS",
	"CANAL",
	"CANDO",
	"CANDY",
	"CANED",
	"CANEH",
	"CANER",
	"CANES",
	"CANGS",
	"CANID",
	"CANNS",
	"CANNY",
	"CANOE",
	"CANON",
	"CANSO",
	"CANTY",
	"CAPAS",
	"CAPAX",
	"CAPER",
	"CAPES",
	"CAPEX",
	"CAPHS",
	"CAPLE",
	"CAPON",
	"CAPRI",
	"CAPUL",
	"CAPUT",
	"CARAT",
	"CARBO",
	"CARBY",
	"CARDI",
	"CARDY",
	"CARED",
	"CARET",
	"CARGO",
	"CARLE",
	"CARLS",
	"CARNS",
	"CARNY",
	"CAROL",
	"CAROM",
	"CARON",
	"CARPS",
	"CARRY",
	"CARTE",
	"CARVE",
	"CARVY",
	"CASED",
	"CASKY",
	"CASTE",
	"CASTS",
	"CASUS",
	"CATCH",
	"CATER",
	"CATES",
	"CATTY",
	"CAUKS",
	"CAULD",
	"CAULK",
	"CAUSE",
	"CAVAS",
	"CAVED",
	"CAVIL",
	"CAWED",
	"CAWKS",
	"CAXON",
	"CEASE",
	"CECAL",
	"CEDAR",
	"CEDER",
	"CEDES",
	"CEDIS",
	"CELEB",
	"CELLI",
	"CELLO",
	"CELLS",
	"CELLY",
	"CELTS",
	"CENSE",
	"CENTS",
	"CENTU",
	"CEORL",
	"CERED",
	"CERIC",
	"CERNE",
	"CEROS",
	"CESSE",
	"CESTI",
	"CETES",
	"CHAAT",
	"CHACE",
	"CHACO",
	"CHADO",
	"CHAFE",
	"CHAFF",
	"CHAIN",
	"CHAIR",
	"CHALK",
	"CHALS",
	"CHAMP",
	"CHANT",
	"CHAOS",
	"CHAPS",
	"CHAPT",
	"CHARD",
	"CHARK",
	"CHARM",
	"CHARS",
	"CHART",
	"CHASE",
	"CHASM",
	"CHATS",
	"CHAVA",
	"CHAVS",
	"CHAWL",
	"CHAYA",
	"CHAYS",
	"CHEAP",
	"CHEAT",
	"CHECK",
	"CHEDI",
	"CHEEB",
	"CHEEK",
	"CHEEP",
	"CHEER",
	"CHEET",
	"CHEFS",
	"CHELA",
	"CHERT",
	"CHESS",
	"CHEST",
	"CHEVY",
	"CHIAS",
	"CHICA",
	"CHICK",
	"CHIDE",
	"CHIEF",
	"CHIKO",
	"CHIKS",
	"CHILD",
	"CHILI",
	"CHILL",
	"CHIMB",
	"CHIME",
	"CHIMO",
	"CHINA",
	"CHINE",
	"CHINK",
	"CHINS",
	"CHIPS",
	"CHIRM",
	"CHIRO",
	"CHIRP",
	"CHIRR",
	"CHIRU",
	"CHIVA",
	"CHIVE",
	"CHIZZ",
	"CHOCK",
	"CHOCS",
	"CHOIL",
	"CHOIR",
	"CHOKE",
	"CHOKO",
	"CHOKY",
	"CHOLI",
	"CHOLO",
	"CHONS",
	"CHOOF",
	"CHOOM",
	"CHOON",
	"CHORD",
	"CHORE",
	"CHOSE",
	"CHOSS",
	"CHOTA",
	"CHOUT",
	"CHOWK",
	"CHUCK",
	"CHUFF",
	"CHUMP",
	"CHUMS",
	"CHUNK",
	"CHURN",
	"CHUSE",
	"CHUTE",
	"CHYLE",
	"CIDER",
	"CIGAR",
	"CILIA",
	"CINCH",
	"CINCT",
	"CINES",
	"CIONS",
	"CIRCA",
	"CIRCS",
	"CIRES",
	"CIRRI",
	"CISSY",
	"CITEE",
	"CITER",
	"CIVET",
	"CIVIC",
	"CIVIE",
	"CIVIL",
	"CLACK",
	"CLAES",
	"CLAIM",
	"CLAMP",
	"CLANG",
	"CLANK",
	"CLANS",
	"CLASH",
	"CLASP",
	"CLASS",
	"CLAUT",
	"CLAVI",
	"CLEAN",
	"CLEAR",
	"CLEAT",
	"CLEFS",
	"CLEFT",
	"CLEGS",
	"CLEIK",
	"CLEPT",
	"CLERK",
	"CLEWS",
	"CLICK",
	"CLIES",
	"CLIFF",
	"CLIFT",
	"CLIMB",
	"CLIME",
	"CLING",
	"CLINK",
	"CLIPE",
	"CLIPS",
	"CLITS",
	"CLOAK",
	"CLOAM",
	"CLOCK",
	"CLOFF",
	"CLOGS",
	"CLOKE",
	"CLONE",
	"CLOOP",
	"CLOSE",
	"CLOTH",
	"CLOTS",
	"CLOUD",
	"CLOUT",
	"CLOVE",
	"CLOWN",
	"CLOYS",
	"CLUBS",
	"CLUCK",
	"CLUED",
	"CLUES",
	"CLUMP",
	"CLUNG",
	"CNIDA",
	"COACH",
	"COACT",
	"COALA",
	"COALS",
	"COALY",
	"COAPT",
	"COARB",
	"COAST",
	"COATI",
	"COBBS",
	"COBBY",
	"COBIA",
	"COBRA",
	"COCAS",
	"COCCO",
	"COCOA",
	"CODAS",
	"CODEN",
	"CODER",
	"CODES",
	"CODEX",
	"COGIE",
	"COGUE",
	"COHEN",
	"COHOE",
	"COHOS",
	"COIFS",
	"COIGN",
	"COKEY",
	"COLAS",
	"COLLE",
	"COLOG",
	"COLON",
	"COLOR",
	"COLTS",
	"COMAL",
	"COMAS",
	"COMBI",
	"COMBY",
	"COMER",
	"COMET",
	"COMFY",
	"COMIC",
	"COMMA",
	"COMMO",
	"COMMY",
	"COMPS",
	"CONCH",
	"CONDO",
	"CONEX",
	"CONEY",
	"CONFS",
	"CONGE",
	"CONGO",
	"CONIC",
	"CONIN",
	"CONNE",
	"CONNS",
	"CONTO",
	"CONUS",
	"CONVO",
	"COOED",
	"COOEY",
	"COOKS",
	"COOLS",
	"COOLY",
	"COOMS",
	"COOMY",
	"COOST",
	"COOTS",
	"COOTY",
	"COOZE",
	"COPAL",
	"COPAY",
	"COPEN",
	"COPER",
	"COPES",
	"COPPY",
	"COPRA",
	"COPSE",
	"COQUI",
	"CORAL",
	"CORAM",
	"CORDA",
	"CORDS",
	"CORED",
	"CORER",
	"CORES",
	"CORGI",
	"CORNO",
	"CORNS",
	"CORNY",
	"CORPS",
	"CORSE",
	"CORSO",
	"COSEC",
	"COSED",
	"COSET",
	"COSEY",
	"COSIE",
	"COSTA",
	"COSTS",
	"COTAN",
	"COTCH",
	"COTED",
	"COTHS",
	"COUCH",
	"COUDE",
	"COUGH",
	"COULD",
	"COUNT",
	"COUPE",
	"COUPS",
	"COURB",
	"COURD",
	"COURT",
	"COUTA",
	"COUTH",
	"COVED",
	"COVEN",
	"COVER",
	"COVES",
	"COVET",
	"COVEY",
	"COVIN",
	"COWAL",
	"COWAN",
	"COWER",
	"COWKS",
	"COWLS",
	"COWPS",
	"COXED",
	"COXIB",
	"COYAU",
	"COYER",
	"COYLY",
	"COZEN",
	"CRAAL",
	"CRABS",
	"CRACK",
	"CRAFT",
	"CRAKE",
	"CRAME",
	"CRAMP",
	"CRAMS",
	"CRANE",
	"CRANK",
	"CRANS",
	"CRAPE",
	"CRAPS",
	"CRAPY",
	"CRASH",
	"CRASS",
	"CRATE",
	"CRAVE",
	"CRAWL",
	"CRAWS",
	"CRAZE",
	"CRAZY",
	"CREAK",
	"CREAM",
	"CREDO",
	"CREDS",
	"CREED",
	"CREEK",
	"CREEP",
	"CREES",
	"CREIN",
	"CREMA",
	"CREME",
	"CREMS",
	"CRENA",
	"CREPE",
	"CREPS",
	"CREPT",
	"CRESS",
	"CREST",
	"CRIAS",
	"CRIBO",
	"CRIBS",
	"CRICK",
	"CRIED",
	"CRIER",
	"CRIES",
	"CRIME",
	"CRIMP",
	"CRINK",
	"CRINS",
	"CRIOS",
	"CRIPE",
	"CRIPS",
	"CRISE",
	"CRISP",
	"CRISS",
	"CRITH",
	"CRITS",
	"CROAK",
	"CROCI",
	"CROCK",
	"CROCS",
	"CROFT",
	"CROGS",
	"CROMB",
	"CROME",
	"CRONE",
	"CRONK",
	"CRONS",
	"CRONY",
	"CROOK",
	"CROOL",
	"CROON",
	"CROPS",
	"CRORE",
	"CROSS",
	"CROUP",
	"CROUT",
	"CROWD",
	"CROWL",
	"CROWN",
	"CROWS",
	"CRUCK",
	"CRUDE",
	"CRUDY",
	"CRUEL",
	"CRUES",
	"CRUET",
	"CRUMB",
	"CRUMP",
	"CRUNK",
	"CRUOR",
	"CRUSE",
	"CRUSH",
	"CRUST",
	"CRYPT",
	"CTENE",
	"CUBBY",
	"CUBEB",
	"CUBED",
	"CUBER",
	"CUBES",
	"CUBIC",
	"CUBIT",
	"CUCKS",
	"CUECA",
	"CUFFS",
	"CUIFS",
	"CUING",
	"CUISH",
	"CUKES",
	"CULCH",
	"CULET",
	"CULEX",
	"CULLS",
	"CULMS",
	"CULPA",
	"CULTY",
	"CUMIN",
	"CUNDY",
	"CUNEI",
	"CUNIT",
	"CUPEL",
	"CUPPA",
	"CUPPY",
	"CURBS",
	"CURCH",
	"CURDS",
	"CURDY",
	"CURED",
	"CURER",
	"CURES",
	"CURFS",
	"CURIA",
	"CURIE",
	"CURIO",
	"CURLI",
	"CURLS",
	"CURLY",
	"CURNS",
	"CURRY",
	"CURSE",
	"CURSI",
	"CURST",
	"CURVE",
	"CURVY",
	"CUSHY",
	"CUSPS",
	"CUSSO",
	"CUSUM",
	"CUTCH",
	"CUTER",
	"CUTES",
	"CUTEY",
	"CUTIE",
	"CUTIN",
	"CUTIS",
	"CUTTY",
	"CUVEE",
	"CUZES",
	"CYANO",
	"CYANS",
	"CYBER",
	"CYCAS",
	"CYCLE",
	"CYCLO",
	"CYDER",
	"CYMAE",
	"CYMAS",
	"CYMOL",
	"CYNIC",
	"CYSTS",
	"CYTES",
	"CZARS",
	"DAALS",
	"DACES",
	"DACHA",
	"DACKS",
	"DADAH",
	"DADAS",
	"DADDY",
	"DADOS",
	"DAGGY",
	"DAHLS",
	"DAILY",
	"DAINT",
	"DAIRY",
	"DAISY",
	"DAKER",
	"DALEK",
	"DALES",
	"DALIS",
	"DALLY",
	"DAMAN",
	"DAMNS",
	"DAMPS",
	"DANCE",
	"DANCY",
	"DANDA",
	"DANDY",
	"DANKS",
	"DANNY",
	"DANSE",
	"DANTS",
	"DARAF",
	"DARBS",
	"DARCY",
	"DARED",
	"DARER",
	"DARGA",
	"DARGS",
	"DARKS",
	"DARLS",
	"DARNS",
	"DARRE",
	"DARTS",
	"DARZI",
	"DASHI",
	"DATED",
	"DATER",
	"DATIL",
	"DATTO",
	"DATUM",
	"DAUBE",
	"DAUBS",
	"DAULT",
	"DAUNT",
	"DAVIT",
	"DAWDS",
	"DAWEN",
	"DAWKS",
	"DAWTS",
	"DAZES",
	"DBAGS",
	"DEADS",
	"DEALT",
	"DEARS",
	"DEARY",
	"DEASH",
	"DEATH",
	"DEAWS",
	"DEBAG",
	"DEBAR",
	"DEBEL",
	"DEBES",
	"DEBIT",
	"DEBTS",
	"DEBUG",
	"DEBUR",
	"DEBUT",
	"DEBYE",
	"DECAD",
	"DECAF",
	"DECAL",
	"DECAN",
	"DECAY",
	"DECIM",
	"DECOR",
	"DECOY",
	"DECRY",
	"DECYL",
	"DEEDS",
	"DEEDY",
	"DEELY",
	"DEEPS",
	"DEERE",
	"DEERS",
	"DEETS",
	"DEEVE",
	"DEFAT",
	"DEFER",
	"DEFIS",
	"DEFOG",
	"DEGAS",
	"DEGUS",
	"DEIFY",
	"DEIGN",
	"DEITY",
	"DEKES",
	"DEKKO",
	"DELAY",
	"DELFS",
	"DELIS",
	"DELLS",
	"DELLY",
	"DELOS",
	"DELPH",
	"DELTA",
	"DELVE",
	"DEMAN",
	"DEMOB",
	"DEMON",
	"DEMOS",
	"DEMOT",
	"DEMPT",
	"DEMUR",
	"DENAR",
	"DENCH",
	"DENIM",
	"DENIS",
	"DENSE",
	"DENTS",
	"DEOCH",
	"DEOXY",
	"DEPOT",
	"DEPTH",
	"DERBY",
	"DERES",
	"DERNS",
	"DERNY",
	"DEROS",
	"DERPY",
	"DERRO",
	"DERTH",
	"DERVS",
	"DESHI",
	"DESKS",
	"DETAG",
	"DETER",
	"DETOX",
	"DEUCE",
	"DEVEL",
	"DEVIL",
	"DEVIS",
	"DEVON",
	"DEVOS",
	"DEVOT",
	"DEWAX",
	"DEWED",
	"DEXIE",
	"DHABA",
	"DHAKS",
	"DHALS",
	"DHOLL",
	"DHONI",
	"DIARY",
	"DICED",
	"DICEY",
	"DICTS",
	"DICTY",
	"DIDDY",
	"DIDOS",
	"DIENE",
	"DIETS",
	"DIGHT",
	"DIGIT",
	"DIKES",
	"DILLI",
	"DILLS",
	"DILLY",
	"DIMES",
	"DIMLY",
	"DINER",
	"DINGE",
	"DINGO",
	"DINGY",
	"DINIC",
	"DINNA",
	"DIODE",
	"DIOLS",
	"DIOTA",
	"DIRAM",
	"DIRGE",
	"DIRKE",
	"DIRTY",
	"DISAS",
	"DISCI",
	"DISCO",
	"DISCS",
	"DISHY",
	"DISME",
	"DITCH",
	"DITED",
	"DITSY",
	"DITTO",
	"DITTY",
	"DITZY",
	"DIVAS",
	"DIVER",
	"DIVES",
	"DIVEY",
	"DIVOT",
	"DIVVY",
	"DIWAN",
	"DIXIT",
	"DIYAS",
	"DIZEN",
	"DIZZY",
	"DJINS",
	"DOATS",
	"DOBLA",
	"DOBLE",
	"DOCKS",
	"DOCOS",
	"DOCUS",
	"DODDY",
	"DODGE",
	"DODGY",
	"DODOS",
	"DOERS",
	"DOEST",
	"DOETH",
	"DOGAL",
	"DOGAN",
	"DOGEY",
	"DOGIE",
	"DOGMA",
	"DOILY",
	"DOING",
	"DOJOS",
	"DOLCI",
	"DOLES",
	"DOLEY",
	"DOLIA",
	"DOLIE",
	"DOLLS",
	"DOLLY",
	"DOLMA",
	"DOLTS",
	"DOMES",
	"DOMIC",
	"DONER",
	"DONGA",
	"DONKO",
	"DONNY",
	"DONOR",
	"DONSY",
	"DONUT",
	"DOOKS",
	"DOOLS",
	"DOOMS",
	"DOONA",
	"DOORN",
	"DOORS",
	"DOPAS",
	"DOPER",
	"DOPEY",
	"DOPPE",
	"DORAD",
	"DOREE",
	"DORES",
	"DORIS",
	"DORMS",
	"DORMY",
	"DORPS",
	"DORRS",
	"DORTS",
	"DOSAI",
	"DOSAS",
	"DOSER",
	"DOSES",
	"DOSHA",
	"DOTAL",
	"DOTED",
	"DOUBT",
	"DOUCE",
	"DOUCS",
	"DOUGH",
	"DOUKS",
	"DOUMS",
	"DOUPS",
	"DOVES",
	"DOWAK",
	"DOWDS",
	"DOWDY",
	"DOWED",
	"DOWEL",
	"DOWER",
	"DOWIE",
	"DOWLS",
	"DOWLY",
	"DOWNS",
	"DOWNY",
	"DOWRY",
	"DOWSE",
	"DOWTS",
	"DOXES",
	"DOXIE",
	"DOZEN",
	"DOZER",
	"DOZES",
	"DRACK",
	"DRACO",
	"DRAFT",
	"DRAIN",
	"DRAKE",
	"DRAMA",
	"DRAMS",
	"DRANK",
	"DRANT",
	"DRAPE",
	"DRAPY",
	"DRAWL",
	"DRAWN",
	"DRAYS",
	"DREAD",
	"DREAM",
	"DREER",
	"DREGS",
	"DRENT",
	"DRESS",
	"DRIBS",
	"DRICE",
	"DRIED",
	"DRIER",
	"DRIFT",
	"DRILL",
	"DRILY",
	"DRINK",
	"DRIPS",
	"DRIPT",
	"DRIVE",
	"DROCK",
	"DROID",
	"DROIL",
	"DROIT",
	"DROKE",
	"DROLL",
	"DROME",
	"DRONE",
	"DRONY",
	"DROOB",
	"DROOL",
	"DROOP",
	"DROSS",
	"DROUK",
	"DROVE",
	"DROWN",
	"DRUID",
	"DRUMS",
	"DRUNK",
	"DRUXY",
	"DRYER",
	"DRYLY",
	"DUADS",
	"DUALS",
	"DUBBY",
	"DUCAL",
	"DUCAT",
	"DUCHY",
	"DUCKY",
	"DUCTS",
	"DUDED",
	"DUDES",
	"DUELS",
	"DUETS",
	"DUFUS",
	"DUKED",
	"DUKUN",
	"DULES",
	"DULLS",
	"DULLY",
	"DUMMY",
	"DUMPS",
	"DUMPY",
	"DUNAM",
	"DUNCE",
	"DUNGS",
	"DUNKS",
	"DUNNO",
	"DUNSH",
	"DUOMI",
	"DUPES",
	"DUPLE",
	"DURAL",
	"DURES",
	"DURGY",
	"DURNS",
	"DUROC",
	"DUROY",
	"DURST",
	"DURUM",
	"DURZI",
	"DUSKY",
	"DUSTS",
	"DUSTY",
	"DUTCH",
	"DUVET",
	"DUXES",
	"DWALM",
	"DWANG",
	"DWARF",
	"DWAUM",
	"DWELL",
	"DWELT",
	"DWILE",
	"DWINE",
	"DYADS",
	"DYING",
	"DYKED",
	"DYKON",
	"DYNES",
	"DYNOS",
	"EAGER",
	"EAGLE",
	"EALED",
	"EALES",
	"EANED",
	"EARDS",
	"EARED",
	"EARLY",
	"EARST",
	"EARTH",
	"EASEL",
	"EASER",
	"EASES",
	"EASLE",
	"EATEN",
	"EATER",
	"EATHE",
	"EAVES",
	"EBANK",
	"EBIKE",
	"EBONS",
	"EBONY",
	"ECADS",
	"ECARD",
	"ECHED",
	"ECHES",
	"ECHOS",
	"ECLAT",
	"ECRUS",
	"EDEMA",
	"EDGED",
	"EDGER",
	"EDGES",
	"EDICT",
	"EDIFY",
	"EDITS",
	"EENSY",
	"EERIE",
	"EEVEN",
	"EEVNS",
	"EFFED",
	"EGERS",
	"EGEST",
	"EGRET",
	"EIDER",
	"EIDOS",
	"EIGHT",
	"EIGNE",
	"EIKED",
	"EILDS",
	"EJECT",
	"EJIDO",
	"EKING",
	"ELAIN",
	"ELAND",
	"ELATE",
	"ELBOW",
	"ELDER",
	"ELDIN",
	"ELECT",
	"ELEGY",
	"ELEMI",
	"ELFED",
	"ELFIN",
	"ELIAD",
	"ELIDE",
	"ELITE",
	"ELMEN",
	"ELOGY",
	"ELOIN",
	"ELOPE",
	"ELOPS",
	"ELPEE",
	"ELUDE",
	"ELUTE",
	"ELVAN",
	"ELVEN",
	"ELVER",
	"EMAIL",
	"EMBED",
	"EMBER",
	"EMBOX",
	"EMCEE",
	"EMEER",
	"EMEUS",
	"EMICS",
	"EMIRS",
	"EMMAS",
	"EMMEW",
	"EMOJI",
	"EMOVE",
	"EMPTY",
	"EMURE",
	"EMYDE",
	"EMYDS",
	"ENACT",
	"ENARM",
	"ENATE",
	"ENDED",
	"ENDER",
	"ENDOW",
	"ENDUE",
	"ENEMA",
	"ENEMY",
	"ENEWS",
	"ENFIX",
	"ENJOY",
	"ENMEW",
	"ENNOG",
	"ENNUI",
	"ENOWS",
	"ENROL",
	"ENSEW",
	"ENSUE",
	"ENTER",
	"ENTIA",
	"ENTRE",
	"ENTRY",
	"ENURN",
	"ENVOI",
	"ENVOY",
	"ENZYM",
	"EORLS",
	"EOSIN",
	"EPEES",
	"EPHAH",
	"EPHAS",
	"EPHOD",
	"EPICS",
	"EPOCH",
	"EPOXY",
	"EQUAL",
	"EQUES",
	"EQUID",
	"EQUIP",
	"ERASE",
	"ERBIA",
	"ERECT",
	"EREVS",
	"ERGON",
	"ERGOS",
	"ERGOT",
	"ERHUS",
	"ERICA",
	"ERICK",
	"ERING",
	"ERNES",
	"ERODE",
	"EROSE",
	"ERROR",
	"ERUPT",
	"ERUVS",
	"ERVIL",
	"ESCAR",
	"ESCOT",
	"ESKAR",
	"ESKER",
	"ESNES",
	"ESROG",
	"ESSAY",
	"ESSES",
	"ESTER",
	"ESTOP",
	"ETAGE",
	"ETATS",
	"ETHER",
	"ETHIC",
	"ETHOS",
	"ETHYL",
	"ETICS",
	"ETROG",
	"ETTIN",
	"ETTLE",
	"ETUDE",
	"ETUIS",
	"ETYMA",
	"EUROS",
	"EVADE",
	"EVENT",
	"EVERT",
	"EVERY",
	"EVETS",
	"EVHOE",
	"EVICT",
	"EVITE",
	"EVOHE",
	"EVOKE",
	"EWERS",
	"EWEST",
	"EWHOW",
	"EXACT",
	"EXALT",
	"EXAMS",
	"EXCEL",
	"EXEAT",
	"EXECS",
	"EXERT",
	"EXFIL",
	"EXILE",
	"EXINE",
	"EXIST",
	"EXITS",
	"EXODE",
	"EXOME",
	"EXONS",
	"EXPEL",
	"EXPOS",
	"EXTOL",
	"EXTRA",
	"EXULS",
	"EXULT",
	"EYING",
	"EYOTS",
	"EYRAS",
	"EYRES",
	"EYRIE",
	"EZINE",
	"FABBY",
	"FABLE",
	"FACED",
	"FACET",
	"FACIA",
	"FACIE",
	"FACTA",
	"FACTS",
	"FACTY",
	"FADED",
	"FADER",
	"FADOS",
	"FAENA",
	"FAERY",
	"FAFFS",
	"FAGOT",
	"FAIKS",
	"FAINS",
	"FAINT",
	"FAIRE",
	"FAIRY",
	"FAITH",
	"FAKER",
	"FAKIE",
	"FALLS",
	"FALSE",
	"FALSY",
	"FAMED",
	"FANAL",
	"FANCY",
	"FANDS",
	"FANES",
	"FANGA",
	"FANGO",
	"FANGS",
	"FANNY",
	"FANON",
	"FANUM",
	"FARCE",
	"FARED",
	"FARER",
	"FARLE",
	"FAROS",
	"FASTI",
	"FATAL",
	"FATES",
	"FATSO",
	"FATTY",
	"FATWA",
	"FAULD",
	"FAULT",
	"FAUNA",
	"FAUTE",
	"FAUTS",
	"FAVAS",
	"FAVEL",
	"FAVER",
	"FAVES",
	"FAVOR",
	"FAVUS",
	"FAWNS",
	"FAWNY",
	"FAXES",
	"FAYER",
	"FAYNE",
	"FAYRE",
	"FAZED",
	"FEART",
	"FEAST",
	"FEATS",
	"FEAZE",
	"FECAL",
	"FECHT",
	"FECIT",
	"FEDAI",
	"FEDEX",
	"FEELS",
	"FEELY",
	"FEESE",
	"FEEZE",
	"FEIGN",
	"FEIST",
	"FELIX",
	"FELLA",
	"FELLS",
	"FELLY",
	"FELON",
	"FELTS",
	"FELTY",
	"FEMES",
	"FEMME",
	"FEMMY",
	"FEMUR",
	"FENCE",
	"FENDS",
	"FENDY",
	"FENIS",
	"FENKS",
	"FERAL",
	"FERER",
	"FERNS",
	"FERNY",
	"FEROX",
	"FERRY",
	"FESTS",
	"FESTY",
	"FETAL",
	"FETCH",
	"FETES",
	"FETID",
	"FETOR",
	"FETTS",
	"FETUS",
	"FEUAR",
	"FEUDS",
	"FEVER",
	"FEWER",
	"FIATS",
	"FIBER",
	"FIBRE",
	"FICES",
	"FICOS",
	"FICTA",
	"FICUS",
	"FIDES",
	"FIDGE",
	"FIDUS",
	"FIEFS",
	"FIELD",
	"FIEND",
	"FIENT",
	"FIERI",
	"FIERS",
	"FIERY",
	"FIEST",
	"FIFER",
	"FIFES",
	"FIFIS",
	"FIFTH",
	"FIFTY",
	"FIGGY",
	"FIGHT",
	"FIKES",
	"FILAR",
	"FILED",
	"FILER",
	"FILET",
	"FILII",
	"FILKS",
	"FILLO",
	"FILLS",
	"FILLY",
	"FILMI",
	"FILMY",
	"FILON",
	"FILOS",
	"FILTH",
	"FINAL",
	"FINCA",
	"FINCH",
	"FINDS",
	"FINER",
	"FINIS",
	"FINKS",
	"FIQHS",
	"FIRED",
	"FIRER",
	"FIRMA",
	"FIRMS",
	"FIRNS",
	"FIRRY",
	"FIRST",
	"FISCS",
	"FISHO",
	"FISHY",
	"FISTS",
	"FITLY",
	"FITTE",
	"FITTS",
	"FIVER",
	"FIXED",
	"FIXER",
	"FIXES",
	"FIZZY",
	"FJORD",
	"FLABS",
	"FLACK",
	"FLAIL",
	"FLAIR",
	"FLAKE",
	"FLAKY",
	"FLAME",
	"FLANE",
	"FLANK",
	"FLANS",
	"FLAPS",
	"FLARE",
	"FLARY",
	"FLASH",
	"FLASK",
	"FLATS",
	"FLAVA",
	"FLAWN",
	"FLAWS",
	"FLEAM",
	"FLECK",
	"FLEES",
	"FLEET",
	"FLESH",
	"FLEUR",
	"FLEWS",
	"FLEXO",
	"FLICK",
	"FLIED",
	"FLIER",
	"FLIES",
	"FLIMS",
	"FLING",
	"FLINT",
	"FLIRT",
	"FLISK",
	"FLITT",
	"FLOAT",
	"FLOCK",
	"FLOCS",
	"FLOOD",
	"FLOOR",
	"FLORA",
	"FLORY",
	"FLOSH",
	"FLOSS",
	"FLOTE",
	"FLOUR",
	"FLOUT",
	"FLOWN",
	"FLUBS",
	"FLUFF",
	"FLUID",
	"FLUKE",
	"FLUKY",
	"FLUME",
	"FLUNG",
	"FLUNK",
	"FLUOR",
	"FLURR",
	"FLUSH",
	"FLUTE",
	"FLUTY",
	"FLUYT",
	"FLYBY",
	"FLYER",
	"FOALS",
	"FOAMS",
	"FOAMY",
	"FOCAL",
	"FOCUS",
	"FOEHN",
	"FOGEY",
	"FOGGY",
	"FOGOS",
	"FOGOU",
	"FOHNS",
	"FOIDS",
	"FOINS",
	"FOIST",
	"FOLDS",
	"FOLIE",
	"FOLIO",
	"FOLKS",
	"FOLLY",
	"FONDS",
	"FONDU",
	"FONIO",
	"FONTS",
	"FORAM",
	"FORAY",
	"FORCE",
	"FORDS",
	"FOREL",
	"FORGE",
	"FORGO",
	"FORKS",
	"FORKY",
	"FORME",
	"FORTE",
	"FORTH",
	"FORTY",
	"FORUM",
	"FOSSA",
	"FOSSE",
	"FOUAT",
	"FOUND",
	"FOUNT",
	"FOURS",
	"FOVEA",
	"FOWTH",
	"FOXED",
	"FOXIE",
	"FOYER",
	"FRAIL",
	"FRAIM",
	"FRAIS",
	"FRAME",
	"FRANK",
	"FRAPS",
	"FRATE",
	"FRAUD",
	"FRAUS",
	"FREAK",
	"FREED",
	"FREER",
	"FRENA",
	"FREON",
	"FRERE",
	"FRESH",
	"FRIAR",
	"FRIED",
	"FRIER",
	"FRILL",
	"FRISE",
	"FRISK",
	"FRITE",
	"FRITS",
	"FRITZ",
	"FRIZE",
	"FRIZZ",
	"FROCK",
	"FROMM",
	"FROND",
	"FRONS",
	"FRONT",
	"FRORE",
	"FRORN",
	"FROST",
	"FROTH",
	"FROWN",
	"FROYO",
	"FROZE",
	"FRUGS",
	"FRUIT",
	"FRUST",
	"FUBAR",
	"FUBBY",
	"FUBSY",
	"FUDGE",
	"FUGIO",
	"FUGIS",
	"FUGLY",
	"FUGUE",
	"FUJIS",
	"FULLS",
	"FULLY",
	"FULTH",
	"FUMER",
	"FUMES",
	"FUNDA",
	"FUNDI",
	"FUNGI",
	"FUNKS",
	"FUNKY",
	"FUNNY",
	"FUNTS",
	"FURCA",
	"FURLS",
	"FUROL",
	"FUROR",
	"FURRY",
	"FURZE",
	"FURZY",
	"FUSED",
	"FUSES",
	"FUSSY",
	"FUSTS",
	"FUTON",
	"FUZED",
	"FUZEE",
	"FUZES",
	"FUZZY",
	"FYCES",
	"FYKES",
	"FYRDS",
	"GABBA",
	"GABBY",
	"GABLE",
	"GADGE",
	"GADJE",
	"GADJO",
	"GADSO",
	"GAFFE",
	"GAFFS",
	"GAGER",
	"GAGES",
	"GAIDS",
	"GAILY",
	"GAINS",
	"GAIRS",
	"GAITA",
	"GAITT",
	"GALAS",
	"GALAX",
	"GALEA",
	"GALIA",
	"GALLY",
	"GALUT",
	"GALVO",
	"GAMAS",
	"GAMAY",
	"GAMBE",
	"GAMBO",
	"GAMED",
	"GAMER",
	"GAMIN",
	"GAMMA",
	"GAMMY",
	"GAMPS",
	"GAMUT",
	"GANDY",
	"GANKS",
	"GAPER",
	"GAPOS",
	"GARBE",
	"GARBO",
	"GARDA",
	"GARDE",
	"GARES",
	"GARIS",
	"GARRE",
	"GARRI",
	"GASES",
	"GASHY",
	"GASSY",
	"GATCH",
	"GATED",
	"GATOR",
	"GAUDS",
	"GAUDY",
	"GAUGE",
	"GAUMS",
	"GAUNT",
	"GAUPS",
	"GAURS",
	"GAUZE",
	"GAVEL",
	"GAVOT",
	"GAWDS",
	"GAWKY",
	"GAWPS",
	"GAYER",
	"GAYLY",
	"GAZED",
	"GAZER",
	"GAZON",
	"GAZOO",
	"GEARS",
	"GEASA",
	"GEBUR",
	"GECKO",
	"GEEKS",
	"GEEKY",
	"GEEPS",
	"GEESE",
	"GEIST",
	"GEITS",
	"GELDS",
	"GELEE",
	"GELID",
	"GENAL",
	"GENAS",
	"GENES",
	"GENIE",
	"GENIN",
	"GENIO",
	"GENIP",
	"GENNY",
	"GENOM",
	"GENRE",
	"GENTS",
	"GENTY",
	"GENUA",
	"GEOID",
	"GERBE",
	"GERLE",
	"GERNE",
	"GESSE",
	"GESTE",
	"GEYAN",
	"GEYER",
	"GHAST",
	"GHOST",
	"GHOUL",
	"GHUSL",
	"GHYLL",
	"GIANT",
	"GIBED",
	"GIBLI",
	"GIDDY",
	"GIFTS",
	"GIGHE",
	"GILAS",
	"GILIA",
	"GILLS",
	"GILTS",
	"GIMEL",
	"GIMME",
	"GIMPS",
	"GIMPY",
	"GINGE",
	"GINNY",
	"GINZO",
	"GIPPO",
	"GIPSY",
	"GIRDS",
	"GIRLF",
	"GIRLY",
	"GIRRS",
	"GIRTH",
	"GISMO",
	"GISMS",
	"GISTS",
	"GITES",
	"GIVEN",
	"GIVER",
	"GIVES",
	"GLADE",
	"GLADS",
	"GLAIK",
	"GLAIR",
	"GLAMP",
	"GLAND",
	"GLARE",
	"GLARY",
	"GLASS",
	"GLAUR",
	"GLAZE",
	"GLAZY",
	"GLEAM",
	"GLEAN",
	"GLEBE",
	"GLEBY",
	"GLEDS",
	"GLEED",
	"GLEEK",
	"GLEES",
	"GLIDE",
	"GLIFT",
	"GLIKE",
	"GLIMS",
	"GLINT",
	"GLISK",
	"GLITZ",
	"GLOAM",
	"GLOAT",
	"GLOBE",
	"GLOBS",
	"GLODE",
	"GLOGG",
	"GLOMS",
	"GLOOM",
	"GLOOP",
	"GLOPS",
	"GLORY",
	"GLOSS",
	"GLOST",
	"GLOUT",
	"GLOVE",
	"GLOWS",
	"GLOZE",
	"GLUES",
	"GLUEY",
	"GLUGG",
	"GLUGS",
	"GLUME",
	"GLUTE",
	"GLYPH",
	"GNARL",
	"GNARR",
	"GNARS",
	"GNASH",
	"GNATS",
	"GNAWN",
	"GNAWS",
	"GNOME",
	"GOADS",
	"GOAFS",
	"GOAFT",
	"GOALS",
	"GOATS",
	"GOAVE",
	"GOBAN",
	"GOBAR",
	"GOBBE",
	"GOBBO",
	"GOBIS",
	"GOBOS",
	"GODET",
	"GODLY",
	"GOELS",
	"GOEST",
	"GOFFS",
	"GOGOS",
	"GOING",
	"GOKES",
	"GOLDY",
	"GOLEM",
	"GOLES",
	"GOLFS",
	"GOLLY",
	"GOLPS",
	"GOMBO",
	"GOMER",
	"GOMPA",
	"GONAD",
	"GONER",
	"GONGS",
	"GONNA",
	"GONOF",
	"GONZO",
	"GOOBY",
	"GOODY",
	"GOOEY",
	"GOOFY",
	"GOOGS",
	"GOOKS",
	"GOOLD",
	"GOOLY",
	"GOOMY",
	"GOONS",
	"GOOPS",
	"GOOPY",
	"GOORS",
	"GOORY",
	"GOOSE",
	"GOPAK",
	"GOPIK",
	"GORBS",
	"GORDO",
	"GORGE",
	"GORIS",
	"GORMS",
	"GORPS",
	"GORSE",
	"GORSY",
	"GOTHS",
	"GOTHY",
	"GOTTA",
	"GOUCH",
	"GOUGE",
	"GOUKS",
	"GOURD",
	"GOUTS",
	"GOUTY",
	"GOVED",
	"GOVES",
	"GOWAN",
	"GOWFS",
	"GOWKS",
	"GOWLS",
	"GOYIM",
	"GOYLE",
	"GRABS",
	"GRACE",
	"GRADE",
	"GRADS",
	"GRAFF",
	"GRAFT",
	"GRAIL",
	"GRAIN",
	"GRAIP",
	"GRAMA",
	"GRAMS",
	"GRANA",
	"GRAND",
	"GRANT",
	"GRAPE",
	"GRAPH",
	"GRAPY",
	"GRASP",
	"GRASS",
	"GRATA",
	"GRATE",
	"GRATS",
	"GRAVE",
	"GRAVY",
	"GRAZE",
	"GREAT",
	"GREBO",
	"GREED",
	"GREEN",
	"GREES",
	"GREET",
	"GREGE",
	"GREGO",
	"GREPS",
	"GREVE",
	"GRICE",
	"GRIEF",
	"GRIFF",
	"GRIFT",
	"GRIGS",
	"GRILL",
	"GRIME",
	"GRIMY",
	"GRIND",
	"GRIPE",
	"GRIPY",
	"GRISE",
	"GRITH",
	"GRITS",
	"GROAN",
	"GROGS",
	"GROIN",
	"GROKS",
	"GROMS",
	"GROOF",
	"GROOM",
	"GROPE",
	"GROSS",
	"GROTS",
	"GROUP",
	"GROUT",
	"GROVE",
	"GROWL",
	"GROWN",
	"GROWS",
	"GRRRL",
	"GRUBS",
	"GRUED",
	"GRUEL",
	"GRUFF",
	"GRUME",
	"GRUND",
	"GRUNT",
	"GRYPT",
	"GUANO",
	"GUANS",
	"GUARD",
	"GUARS",
	"GUAVA",
	"GUCKS",
	"GUCKY",
	"GUDES",
	"GUESS",
	"GUEST",
	"GUFFS",
	"GUGAS",
	"GUIDE",
	"GUIDO",
	"GUILD",
	"GUILE",
	"GUILT",
	"GUISE",
	"GULAG",
	"GULAR",
	"GULAS",
	"GULCH",
	"GULET",
	"GULLS",
	"GULLY",
	"GULPH",
	"GUMBO",
	"GUMMA",
	"GUMMY",
	"GUMPS",
	"GUNAS",
	"GUNDI",
	"GUNDY",
	"GUNGE",
	"GUNGY",
	"GUNKS",
	"GUNNY",
	"GUPPY",
	"GUQIN",
	"GURGE",
	"GURKS",
	"GURLS",
	"GURRY",
	"GUSLE",
	"GUSLI",
	"GUSSY",
	"GUSTO",
	"GUSTS",
	"GUSTY",
	"GUTSY",
	"GUTTA",
	"GUTTY",
	"GUYED",
	"GYBED",
	"GYNAE",
	"GYNIE",
	"GYNOS",
	"GYOZA",
	"GYPOS",
	"GYPPO",
	"GYPPY",
	"GYPSY",
	"GYRAL",
	"GYRED",
	"GYRES",
	"GYRON",
	"GYRUS",
	"GYVES",
	"HAAFS",
	"HAARS",
	"HAATS",
	"HABIT",
	"HABUS",
	"HACEK",
	"HACKS",
	"HACKY",
	"HADED",
	"HADST",
	"HAEMS",
	"HAETS",
	"HAFFS",
	"HAFTA",
	"HAFTS",
	"HAHAM",
	"HAIKS",
	"HAILS",
	"HAINS",
	"HAIRY",
	"HAITH",
	"HAJES",
	"HAJIS",
	"HAKEA",
	"HAKUS",
	"HALAL",
	"HALED",
	"HALES",
	"HALID",
	"HALLO",
	"HALMA",
	"HALON",
	"HALOS",
	"HALSH",
	"HALTS",
	"HALVE",
	"HAMAL",
	"HAMBA",
	"HAMED",
	"HAMEL",
	"HAMMY",
	"HANCH",
	"HANDS",
	"HANDY",
	"HANGI",
	"HANTS",
	"HAOLE",
	"HAPAS",
	"HAPAX",
	"HAPPY",
	"HARAM",
	"HARDS",
	"HARDY",
	"HARED",
	"HAREM",
	"HARIM",
	"HARMS",
	"HARNS",
	"HAROS",
	"HARPY",
	"HARRY",
	"HARSH",
	"HASKS",
	"HASTA",
	"HASTE",
	"HASTY",
	"HATCH",
	"HATED",
	"HATER",
	"HATES",
	"HAUFS",
	"HAULM",
	"HAULS",
	"HAUNT",
	"HAUTE",
	"HAVEL",
	"HAVEN",
	"HAVES",
	"HAVOC",
	"HAWED",
	"HAWKS",
	"HAWMS",
	"HAYED",
	"HAYER",
	"HAZAN",
	"HAZED",
	"HAZEL",
	"HAZES",
	"HAZLE",
	"HEADS",
	"HEADY",
	"HEALD",
	"HEALS",
	"HEAME",
	"HEAPY",
	"HEARD",
	"HEARS",
	"HEART",
	"HEAST",
	"HEATH",
	"HEATS",
	"HEATY",
	"HEAVE",
	"HEAVY",
	"HEBES",
	"HECHT",
	"HEDGE",
	"HEEDS",
	"HEEDY",
	"HEELS",
	"HEEZE",
	"HEFTS",
	"HEFTY",
	"HEIDS",
	"HEILS",
	"HEIST",
	"HEJAB",
	"HEJRA",
	"HELED",
	"HELIO",
	"HELIX",
	"HELLO",
	"HELMS",
	"HELOS",
	"HELOT",
	"HELPS",
	"HELVE",
	"HEMES",
	"HEMIC",
	"HEMPS",
	"HENCE",
	"HENCH",
	"HENDS",
	"HENNA",
	"HENNY",
	"HENRY",
	"HENTS",
	"HERBY",
	"HERDS",
	"HERMS",
	"HERON",
	"HERPS",
	"HERRY",
	"HERSE",
	"HERYE",
	"HETHS",
	"HEUCH",
	"HEVEA",
	"HEVEL",
	"HEWER",
	"HEWGH",
	"HEXAD",
	"HEXED",
	"HEXER",
	"HEYED",
	"HIANT",
	"HIBAS",
	"HIDED",
	"HIDES",
	"HIEMS",
	"HIFIS",
	"HIGHS",
	"HIJAB",
	"HIJRA",
	"HIKED",
	"HIKER",
	"HIKES",
	"HIKOI",
	"HILAR",
	"HILLO",
	"HILLY",
	"HILUS",
	"HIMBO",
	"HINDS",
	"HINGE",
	"HINKY",
	"HINNY",
	"HIOIS",
	"HIPER",
	"HIPPO",
	"HIPPY",
	"HIRES",
	"HISSY",
	"HISTS",
	"HITCH",
	"HITHE",
	"HIVED",
	"HIVES",
	"HOACH",
	"HOAED",
	"HOAGY",
	"HOARD",
	"HOARY",
	"HOBBY",
	"HOBOS",
	"HOCKS",
	"HOCUS",
	"HODAD",
	"HOGEN",
	"HOGGS",
	"HOICK",
	"HOIKS",
	"HOING",
	"HOISE",
	"HOIST",
	"HOKIS",
	"HOKUM",
	"HOLDS",
	"HOLKS",
	"HOLLY",
	"HOLMS",
	"HOLON",
	"HOLTS",
	"HOMAS",
	"HOMED",
	"HOMER",
	"HOMES",
	"HOMIE",
	"HOMOS",
	"HONDS",
	"HONER",
	"HONEY",
	"HONKY",
	"HONOR",
	"HOOCH",
	"HOODS",
	"HOODY",
	"HOOFS",
	"HOOHA",
	"HOOLY",
	"HOONS",
	"HOOPS",
	"HOORD",
	"HOOSH",
	"HOOVE",
	"HOPED",
	"HORAL",
	"HORDE",
	"HORKS",
	"HORNS",
	"HORNY",
	"HORSE",
	"HORST",
	"HORSY",
	"HOSEL",
	"HOSER",
	"HOSEY",
	"HOSTA",
	"HOTEL",
	"HOTEN",
	"HOTIS",
	"HOTLY",
	"HOTTY",
	"HOUFF",
	"HOUFS",
	"HOUND",
	"HOUSE",
	"HOUTS",
	"HOVEA",
	"HOVED",
	"HOVEL",
	"HOVER",
	"HOWAY",
	"HOWBE",
	"HOWDY",
	"HOWFF",
	"HOWLS",
	"HOWRE",
	"HOWSO",
	"HOXED",
	"HOYED",
	"HUBBY",
	"HUDUD",
	"HUHUS",
	"HUIAS",
	"HULES",
	"HULKS",
	"HULLS",
	"HUMAN",
	"HUMFS",
	"HUMIC",
	"HUMID",
	"HUMOR",
	"HUMPH",
	"HUMPS",
	"HUMPY",
	"HUMUS",
	"HUNCH",
	"HUNDO",
	"HUNKY",
	"HURLS",
	"HURRA",
	"HURRY",
	"HUSHY",
	"HUSKS",
	"HUSKY",
	"HUSOS",
	"HUSSY",
	"HUTCH",
	"HUZZY",
	"HWYLS",
	"HYDRO",
	"HYENA",
	"HYENS",
	"HYLAS",
	"HYLEG",
	"HYLES",
	"HYMEN",
	"HYMNS",
	"HYNDE",
	"HYOID",
	"HYPED",
	"HYPER",
	"HYPES",
	"HYPHA",
	"HYPHY",
	"HYRAX",
	"HYSON",
	"HYTHE",
	"IAMBI",
	"IAMBS",
	"IBRIK",
	"ICHED",
	"ICILY",
	"ICING",
	"ICONS",
	"IDANT",
	"IDDAH",
	"IDDAT",
	"IDDUT",
	"IDEAL",
	"IDEES",
	"IDENT",
	"IDIOM",
	"IDIOT",
	"IDLED",
	"IDLER",
	"IDOLA",
	"IDOLS",
	"IDYLL",
	"IDYLS",
	"IGAPO",
	"IGGED",
	"IGLOO",
	"IGLUS",
	"IGNIS",
	"IHRAM",
	"IKANS",
	"IKATS",
	"IKONS",
	"ILEAC",
	"ILEAL",
	"ILEUM",
	"ILEUS",
	"ILIAC",
	"ILIAL",
	"IMAGE",
	"IMAMS",
	"IMARI",
	"IMAUM",
	"IMBAR",
	"IMBED",
	"IMBOS",
	"IMBUE",
	"IMIDO",
	"IMIDS",
	"IMINE",
	"IMINO",
	"IMMIT",
	"IMPED",
	"IMPEL",
	"IMPLY",
	"IMPRO",
	"IMSHI",
	"INANE",
	"INBOX",
	"INCAS",
	"INCLE",
	"INCOG",
	"INCUR",
	"INCUS",
	"INCUT",
	"INDEW",
	"INDEX",
	"INDIA",
	"INDIE",
	"INDOL",
	"INEPT",
	"INERT",
	"INFER",
	"INFIX",
	"INGLE",
	"INGOT",
	"INION",
	"INKLE",
	"INLAY",
	"INLET",
	"INNER",
	"INPUT",
	"INRUN",
	"INSEE",
	"INSET",
	"INTEL",
	"INTER",
	"INTRA",
	"INTRO",
	"INULA",
	"INURN",
	"INVAR",
	"IODIC",
	"IODIN",
	"IONIC",
	"IORAS",
	"IOTAS",
	"IRATE",
	"IRKED",
	"IROKO",
	"IRONY",
	"ISBAS",
	"ISLED",
	"ISLET",
	"ISSUE",
	"ITCHY",
	"ITHER",
	"IVORY",
	"IXIAS",
	"IXNAY",
	"IXORA",
	"IXTLE",
	"IZARD",
	"IZARS",
	"JAAPS",
	"JABOT",
	"JACAL",
	"JACKY",
	"JADED",
	"JAFAS",
	"JAGAS",
	"JAGGS",
	"JAGGY",
	"JAILS",
	"JAKES",
	"JAKIE",
	"JALOP",
	"JAMBE",
	"JAMBS",
	"JAMMY",
	"JANES",
	"JANKY",
	"JANNS",
	"JANNY",
	"JANTY",
	"JAPED",
	"JAPES",
	"JARLS",
	"JASEY",
	"JASPS",
	"JATHA",
	"JATOS",
	"JAUNE",
	"JAUNT",
	"JAVEL",
	"JAWAN",
	"JAWNS",
	"JAZZY",
	"JEANS",
	"JEBEL",
	"JEELS",
	"JEERS",
	"JEHAD",
	"JEHUS",
	"JELLO",
	"JELLY",
	"JEMMY",
	"JENNY",
	"JERID",
	"JERKS",
	"JERKY",
	"JETES",
	"JETON",
	"JETTY",
	"JEUNE",
	"JEWEL",
	"JHEEL",
	"JIBBA",
	"JIBED",
	"JIBER",
	"JIBES",
	"JIFFS",
	"JIFFY",
	"JIGOT",
	"JIHAD",
	"JILLS",
	"JILTS",
	"JIMMY",
	"JIMPY",
	"JINGO",
	"JINGS",
	"JINKS",
	"JINNE",
	"JINNS",
	"JIRDS",
	"JIRGA",
	"JISMS",
	"JITTY",
	"JIVED",
	"JIVES",
	"JIVEY",
	"JOCKS",
	"JOCKY",
	"JOCOS",
	"JODEL",
	"JOINS",
	"JOINT",
	"JOIST",
	"JOKED",
	"JOKER",
	"JOKEY",
	"JOKOL",
	"JOLLO",
	"JOLLY",
	"JOLTY",
	"JOMON",
	"JONES",
	"JONTY",
	"JORAM",
	"JOUAL",
	"JOUGS",
	"JOUKS",
	"JOULE",
	"JOURS",
	"JOUST",
	"JOWAR",
	"JOWED",
	"JOWLY",
	"JOYED",
	"JUDGE",
	"JUDGY",
	"JUGUM",
	"JUICE",
	"JUICY",
	"JUJUS",
	"JUKES",
	"JUMAR",
	"JUMBO",
	"JUMPY",
	"JUNKS",
	"JUNTA",
	"JUNTO",
	"JUPES",
	"JUREL",
	"JURIS",
	"JUROR",
	"JUSTE",
	"JUTES",
	"JUVES",
	"KABAB",
	"KACHA",
	"KACKS",
	"KAGOS",
	"KAGUS",
	"KAHAL",
	"KAIDS",
	"KAIES",
	"KAIKA",
	"KAIKS",
	"KAING",
	"KAINS",
	"KAKIS",
	"KALAS",
	"KALIF",
	"KAMAS",
	"KAMIK",
	"KANAS",
	"KANDY",
	"KANEH",
	"KANES",
	"KANGA",
	"KANGS",
	"KANTS",
	"KANZU",
	"KAPAS",
	"KAPOW",
	"KAPPA",
	"KAPUS",
	"KARAI",
	"KAREE",
	"KARKS",
	"KARMA",
	"KAROO",
	"KAROS",
	"KARSY",
	"KARTS",
	"KARZY",
	"KATAL",
	"KATIS",
	"KAUGH",
	"KAURI",
	"KAURU",
	"KAVAL",
	"KAWAS",
	"KAWAU",
	"KAYAK",
	"KAYLE",
	"KAYOS",
	"KAZIS",
	"KEAKI",
	"KEBAB",
	"KECKS",
	"KEDGE",
	"KEEKS",
	"KEEMA",
	"KEENS",
	"KELLY",
	"KELPS",
	"KELTS",
	"KELTY",
	"KEMPS",
	"KEMPY",
	"KENDO",
	"KENTS",
	"KEPIS",
	"KERBS",
	"KEREL",
	"KERKY",
	"KERMA",
	"KEROS",
	"KETAS",
	"KETES",
	"KETOL",
	"KEVIL",
	"KEXES",
	"KEYED",
	"KHADS",
	"KHAKI",
	"KHANA",
	"KHANS",
	"KHATS",
	"KHAZI",
	"KHIRS",
	"KHORS",
	"KHOUM",
	"KHYAL",
	"KIAAT",
	"KIAKI",
	"KIASU",
	"KIBBE",
	"KIBBI",
	"KIBEI",
	"KIBLA",
	"KICKS",
	"KIDDO",
	"KIDEL",
	"KIDEO",
	"KIDGE",
	"KIEFS",
	"KIERS",
	"KIKAY",
	"KIKES",
	"KILEY",
	"KILIM",
	"KILLS",
	"KILTS",
	"KILTY",
	"KINAS",
	"KINDA",
	"KINES",
	"KINGS",
	"KINGY",
	"KININ",
	"KINKS",
	"KINKY",
	"KINOS",
	"KIOSK",
	"KIPAS",
	"KIPES",
	"KIPPA",
	"KIRKS",
	"KIRNS",
	"KISTS",
	"KITER",
	"KITES",
	"KITHE",
	"KITKE",
	"KITTY",
	"KITUL",
	"KIVAS",
	"KLANG",
	"KLICK",
	"KLIEG",
	"KLIKS",
	"KNACK",
	"KNARS",
	"KNAUR",
	"KNAVE",
	"KNAWE",
	"KNEAD",
	"KNEED",
	"KNEEL",
	"KNELT",
	"KNICK",
	"KNIFE",
	"KNISH",
	"KNIVE",
	"KNOBS",
	"KNOCK",
	"KNOLL",
	"KNOOP",
	"KNOPS",
	"KNOUT",
	"KNOWD",
	"KNOWN",
	"KNOWS",
	"KNUBS",
	"KNURR",
	"KNURS",
	"KOALA",
	"KOAPS",
	"KOBOS",
	"KOFFS",
	"KOFTA",
	"KOGAL",
	"KOHAS",
	"KOHLS",
	"KOINE",
	"KOJIS",
	"KOKAM",
	"KOKAS",
	"KOMBU",
	"KONDO",
	"KOORI",
	"KOPPA",
	"KORAI",
	"KORAN",
	"KORAS",
	"KORAT",
	"KORMA",
	"KORUN",
	"KORUS",
	"KOSES",
	"KOTCH",
	"KOTOS",
	"KOURA",
	"KRAIS",
	"KREEN",
	"KREEP",
	"KRENG",
	"KREWE",
	"KRILL",
	"KRIOL",
	"KRONA",
	"KROON",
	"KRUNK",
	"KUBIE",
	"KUDOS",
	"KUDUS",
	"KUFIS",
	"KUKRI",
	"KUKUS",
	"KULAK",
	"KULAN",
	"KULAS",
	"KUNDS",
	"KURTA",
	"KUTAI",
	"KUTCH",
	"KUTIS",
	"KUYAS",
	"KUZUS",
	"KWAAI",
	"KWELA",
	"KWINK",
	"KWIRL",
	"KYANG",
	"KYLES",
	"KYLIE",
	"KYLIX",
	"KYLOE",
	"KYNDE",
	"KYNDS",
	"KYPES",
	"KYTES",
	"LAARF",
	"LABEL",
	"LABIS",
	"LABOR",
	"LABRA",
	"LACCY",
	"LACES",
	"LACET",
	"LACIS",
	"LACKA",
	"LACKS",
	"LACKY",
	"LADDU",
	"LADDY",
	"LADEN",
	"LADER",
	"LADLE",
	"LAERS",
	"LAEVO",
	"LAGAR",
	"LAGER",
	"LAHAL",
	"LAICH",
	"LAIDE",
	"LAIDS",
	"LAIGH",
	"LAIRD",
	"LAIRS",
	"LAITH",
	"LAKED",
	"LAKER",
	"LAKHS",
	"LAKIN",
	"LALDY",
	"LAMAS",
	"LAMES",
	"LAMIA",
	"LAMPS",
	"LANCE",
	"LANDE",
	"LANED",
	"LANKS",
	"LANKY",
	"LAPAS",
	"LAPEL",
	"LAPIN",
	"LAPPY",
	"LAPSE",
	"LARCH",
	"LARDY",
	"LAREE",
	"LARGA",
	"LARGE",
	"LARNS",
	"LARUM",
	"LARVA",
	"LASED",
	"LASER",
	"LASSI",
	"LASSO",
	"LASSY",
	"LATAH",
	"LATCH",
	"LATER",
	"LATHE",
	"LATHS",
	"LATHY",
	"LATKE",
	"LATTE",
	"LATUS",
	"LAUCH",
	"LAUDS",
	"LAUGH",
	"LAUND",
	"LAURA",
	"LAVAL",
	"LAVER",
	"LAVES",
	"LAVVY",
	"LAWNS",
	"LAWNY",
	"LAWSY",
	"LAXED",
	"LAXER",
	"LAXLY",
	"LAYED",
	"LAYER",
	"LAZES",
	"LAZZO",
	"LEACH",
	"LEADY",
	"LEAFY",
	"LEAKY",
	"LEAMS",
	"LEANT",
	"LEANY",
	"LEAPS",
	"LEAPT",
	"LEARE",
	"LEARN",
	"LEARS",
	"LEASE",
	"LEASH",
	"LEAST",
	"LEAVE",
	"LEAVY",
	"LEDES",
	"LEDGE",
	"LEDUM",
	"LEEAR",
	"LEECH",
	"LEERY",
	"LEETS",
	"LEFTS",
	"LEFTY",
	"LEGAL",
	"LEGER",
	"LEGES",
	"LEGGY",
	"LEGNO",
	"LEMEL",
	"LEMMA",
	"LEMON",
	"LEMUR",
	"LENDS",
	"LENGS",
	"LENOS",
	"LENSE",
	"LEONE",
	"LEPER",
	"LEPTA",
	"LESOS",
	"LESTS",
	"LETCH",
	"LETHE",
	"LEUDS",
	"LEUGH",
	"LEVAS",
	"LEVEL",
	"LEVER",
	"LEVES",
	"LEVIN",
	"LEWIS",
	"LEXES",
	"LEXIS",
	"LEZZY",
	"LIANA",
	"LIANG",
	"LIARD",
	"LIARS",
	"LIBEL",
	"LIBOR",
	"LICHI",
	"LICHT",
	"LICIT",
	"LICKS",
	"LIDAR",
	"LIEGE",
	"LIEUS",
	"LIEVE",
	"LIFTS",
	"LIGER",
	"LIGGE",
	"LIGHT",
	"LIKEN",
	"LIKES",
	"LIKIN",
	"LILAC",
	"LILOS",
	"LILTY",
	"LIMAN",
	"LIMAS",
	"LIMAX",
	"LIMBI",
	"LIMBO",
	"LIMBS",
	"LIMBY",
	"LIMEN",
	"LIMEY",
	"LIMIT",
	"LIMPA",
	"LINAC",
	"LINDY",
	"LINED",
	"LINEN",
	"LINER",
	"LINES",
	"LINGA",
	"LINGO",
	"LINGS",
	"LINGY",
	"LINKS",
	"LINNY",
	"LINUM",
	"LIPID",
	"LIPIN",
	"LIPOS",
	"LIRKS",
	"LISES",
	"LISLE",
	"LISPS",
	"LISTS",
	"LITAS",
	"LITED",
	"LITEM",
	"LITHE",
	"LITRE",
	"LIVEN",
	"LIVER",
	"LIVES",
	"LIVID",
	"LIVRE",
	"LIWAA",
	"LLAMA",
	"LLANO",
	"LOAMY",
	"LOAST",
	"LOATH",
	"LOBBY",
	"LOBOS",
	"LOCAL",
	"LOCHE",
	"LOCHS",
	"LOCIE",
	"LOCIS",
	"LOCKS",
	"LOCKY",
	"LOCUM",
	"LOCUS",
	"LODEN",
	"LODGE",
	"LOFTY",
	"LOGIA",
	"LOGIC",
	"LOGIE",
	"LOGIN",
	"LOGON",
	"LOIDS",
	"LOIRS",
	"LOKES",
	"LOKEY",
	"LOKUM",
	"LOLED",
	"LOLLO",
	"LOLLS",
	"LOLOS",
	"LOMES",
	"LONGE",
	"LONGS",
	"LOOED",
	"LOOEY",
	"LOOFA",
	"LOOKS",
	"LOOMS",
	"LOOPY",
	"LOORD",
	"LOOSE",
	"LOPED",
	"LOPER",
	"LOPPY",
	"LORAL",
	"LORAN",
	"LORDS",
	"LOREL",
	"LORES",
	"LORRY",
	"LOSEL",
	"LOSEN",
	"LOSER",
	"LOSES",
	"LOSSY",
	"LOTAH",
	"LOTAS",
	"LOTOS",
	"LOTSA",
	"LOTTE",
	"LOTUS",
	"LOUED",
	"LOUGH",
	"LOUIE",
	"LOUNS",
	"LOUPE",
	"LOUSE",
	"LOUSY",
	"LOUTS",
	"LOVED",
	"LOVER",
	"LOVES",
	"LOVIE",
	"LOWAN",
	"LOWED",
	"LOWEN",
	"LOWER",
	"LOWLY",
	"LOWNS",
	"LOWSE",
	"LOWTS",
	"LOXES",
	"LOYAL",
	"LOZEN",
	"LUBED",
	"LUBRA",
	"LUCES",
	"LUCID",
	"LUCKS",
	"LUCKY",
	"LUDES",
	"LUDIC",
	"LUFFA",
	"LUGED",
	"LUGER",
	"LUMAS",
	"LUMBI",
	"LUMEN",
	"LUMPS",
	"LUMPY",
	"LUNAR",
	"LUNCH",
	"LUNES",
	"LUNET",
	"LUNGE",
	"LUNKS",
	"LUNTS",
	"LUPUS",
	"LURCH",
	"LURGY",
	"LURID",
	"LURRY",
	"LURVE",
	"LUSER",
	"LUSKS",
	"LUSTS",
	"LUSTY",
	"LUSUS",
	"LUTER",
	"LUTES",
	"LUVVY",
	"LYAMS",
	"LYARD",
	"LYCRA",
	"LYING",
	"LYMES",
	"LYMPH",
	"LYNES",
	"LYRES",
	"LYRIC",
	"LYSED",
	"LYSES",
	"LYSIS",
	"LYSOL",
	"LYSSA",
	"LYTED",
	"LYTES",
	"LYTTA",
	"MAARE",
	"MABES",
	"MACAS",
	"MACAW",
	"MACES",
	"MACHI",
	"MACHO",
	"MACHS",
	"MACKA",
	"MACKS",
	"MACON",
	"MACRO",
	"MACTE",
	"MADAL",
	"MADAM",
	"MADAR",
	"MADDY",
	"MADGE",
	"MADLY",
	"MADOS",
	"MADRE",
	"MAFIA",
	"MAFTS",
	"MAGAS",
	"MAGGS",
	"MAGIC",
	"MAGMA",
	"MAGNA",
	"MAGOT",
	"MAHAL",
	"MAHEM",
	"MAHRS",
	"MAIKS",
	"MAILE",
	"MAILO",
	"MAILS",
	"MAIMS",
	"MAIRE",
	"MAIRS",
	"MAIST",
	"MAIZE",
	"MAJAS",
	"MAJOE",
	"MAJOR",
	"MAKAF",
	"MAKAI",
	"MAKAR",
	"MAKER",
	"MAKES",
	"MALAM",
	"MALAR",
	"MALIC",
	"MALIK",
	"MALIS",
	"MALMY",
	"MALTS",
	"MALVA",
	"MAMBA",
	"MAMBO",
	"MAMBU",
	"MAMEY",
	"MAMIL",
	"MAMMA",
	"MAMMY",
	"MANDI",
	"MANDS",
	"MANDY",
	"MANEB",
	"MANED",
	"MANES",
	"MANGA",
	"MANGE",
	"MANGO",
	"MANGS",
	"MANGY",
	"MANIA",
	"MANIC",
	"MANIS",
	"MANKY",
	"MANLY",
	"MANNY",
	"MANOR",
	"MANOS",
	"MANSE",
	"MANTA",
	"MANTE",
	"MANTO",
	"MANUS",
	"MAPAU",
	"MAPLE",
	"MAPOU",
	"MAPPY",
	"MAQAM",
	"MARAE",
	"MARAH",
	"MARAL",
	"MARAN",
	"MARAY",
	"MARCH",
	"MARCS",
	"MARES",
	"MARGA",
	"MARGE",
	"MARIA",
	"MARID",
	"MARKA",
	"MARLS",
	"MARLY",
	"MAROR",
	"MARRA",
	"MARRY",
	"MARSE",
	"MARSH",
	"MARUA",
	"MASKS",
	"MASON",
	"MASSE",
	"MASTS",
	"MASUR",
	"MATAI",
	"MATCH",
	"MATES",
	"MATEY",
	"MATHE",
	"MATHS",
	"MATIN",
	"MATLO",
	"MATRA",
	"MATSU",
	"MATTE",
	"MAUKA",
	"MAULA",
	"MAULS",
	"MAUMY",
	"MAUNT",
	"MAURI",
	"MAUSY",
	"MAUVE",
	"MAUVY",
	"MAUZY",
	"MAVEN",
	"MAVIS",
	"MAWED",
	"MAWKS",
	"MAWKY",
	"MAWNS",
	"MAWRS",
	"MAXIM",
	"MAYBE",
	"MAYOR",
	"MAYOS",
	"MAYST",
	"MAZAC",
	"MAZAS",
	"MAZED",
	"MAZEL",
	"MAZER",
	"MAZUT",
	"MBARI",
	"MBARS",
	"MBILA",
	"MBIRA",
	"MBRET",
	"MEADS",
	"MEAKE",
	"MEALS",
	"MEALY",
	"MEANS",
	"MEANT",
	"MEANY",
	"MEARE",
	"MEASE",
	"MEATY",
	"MECCA",
	"MECHA",
	"MECKS",
	"MECUM",
	"MEDAL",
	"MEDIA",
	"MEDIC",
	"MEDII",
	"MEDIN",
	"MEECH",
	"MEEDS",
	"MEERS",
	"MEFFS",
	"MEIKO",
	"MEINS",
	"MEINT",
	"MEINY",
	"MEITH",
	"MELAM",
	"MELAS",
	"MELBA",
	"MELEE",
	"MELIC",
	"MELIK",
	"MELLS",
	"MELON",
	"MELTS",
	"MELTY",
	"MENAD",
	"MENDS",
	"MENED",
	"MENES",
	"MENGS",
	"MENIL",
	"MENSA",
	"MENSE",
	"MENSH",
	"MENTA",
	"MEOUS",
	"MERCY",
	"MERED",
	"MERES",
	"MERGE",
	"MERIL",
	"MERIS",
	"MERIT",
	"MERKS",
	"MERLS",
	"MERRY",
	"MERSE",
	"MERSK",
	"MESCA",
	"MESEL",
	"MESEM",
	"MESES",
	"MESIA",
	"MESNE",
	"MESTO",
	"MESYL",
	"METAL",
	"METED",
	"METEG",
	"METER",
	"METHO",
	"METIC",
	"METOL",
	"METRE",
	"METRO",
	"METTA",
	"MEUMS",
	"MEVES",
	"MEWED",
	"MEZES",
	"MIAIS",
	"MIAOU",
	"MIAOW",
	"MIASM",
	"MICKS",
	"MICRA",
	"MICRO",
	"MIDDY",
	"MIDGE",
	"MIDIS",
	"MIDST",
	"MIEUX",
	"MIFTY",
	"MIGHT",
	"MIGOD",
	"MIHIS",
	"MIKAN",
	"MIKED",
	"MIKES",
	"MIKOS",
	"MILCH",
	"MILDS",
	"MILIA",
	"MILKO",
	"MILKY",
	"MILLS",
	"MILOS",
	"MILPA",
	"MILTS",
	"MILTZ",
	"MIMED",
	"MIMEO",
	"MIMER",
	"MIMIC",
	"MIMIS",
	"MIMSY",
	"MINAS",
	"MINCE",
	"MINCY",
	"MINER",
	"MINES",
	"MINGE",
	"MINGI",
	"MINGY",
	"MINIM",
	"MINKE",
	"MINKS",
	"MINNY",
	"MINOR",
	"MINSE",
	"MINTY",
	"MINUS",
	"MINXY",
	"MIRAH",
	"MIRED",
	"MIREX",
	"MIRIN",
	"MIRKY",
	"MIRLS",
	"MIRLY",
	"MIRTH",
	"MIRVS",
	"MISAL",
	"MISCH",
	"MISDO",
	"MISER",
	"MISES",
	"MISGO",
	"MISSY",
	"MISTY",
	"MITAS",
	"MITES",
	"MITRY",
	"MIVVY",
	"MIXER",
	"MIXUP",
	"MIYAS",
	"MIZEN",
	"MIZZY",
	"MOANS",
	"MOBBY",
	"MOBES",
	"MOBEY",
	"MOBLE",
	"MOBOS",
	"MOCHA",
	"MOCHY",
	"MOCKS",
	"MOCUS",
	"MODAL",
	"MODEL",
	"MODEM",
	"MODER",
	"MODGE",
	"MODII",
	"MODIN",
	"MODOC",
	"MODOM",
	"MOERS",
	"MOGAR",
	"MOGAS",
	"MOGOS",
	"MOGRA",
	"MOGUE",
	"MOGUL",
	"MOHAR",
	"MOHUR",
	"MOIST",
	"MOITY",
	"MOJOS",
	"MOKER",
	"MOKEY",
	"MOKIS",
	"MOKOS",
	"MOKUS",
	"MOLAL",
	"MOLAR",
	"MOLDS",
	"MOLDY",
	"MOLER",
	"MOLES",
	"MOLEY",
	"MOLLE",
	"MOLLO",
	"MOLLS",
	"MOLOS",
	"MOLYS",
	"MOMES",
	"MOMIE",
	"MOMMA",
	"MOMME",
	"MOMPE",
	"MONAS",
	"MONDO",
	"MONEY",
	"MONIE",
	"MONKS",
	"MONPE",
	"MONTH",
	"MOODY",
	"MOOLI",
	"MOOLS",
	"MOONS",
	"MOONY",
	"MOORY",
	"MOOSE",
	"MOOTH",
	"MOOTS",
	"MOPED",
	"MOPER",
	"MOPES",
	"MOPUS",
	"MORAE",
	"MORAL",
	"MORAN",
	"MOREL",
	"MORGY",
	"MORIA",
	"MORIN",
	"MORON",
	"MOROR",
	"MORPH",
	"MORRA",
	"MORSE",
	"MOSED",
	"MOSKS",
	"MOSSO",
	"MOSSY",
	"MOTEL",
	"MOTEN",
	"MOTET",
	"MOTEY",
	"MOTHY",
	"MOTIF",
	"MOTON",
	"MOTOR",
	"MOTTO",
	"MOTTS",
	"MOTUS",
	"MOULE",
	"MOULT",
	"MOUND",
	"MOUNT",
	"MOUPS",
	"MOURN",
	"MOUSE",
	"MOUST",
	"MOUTH",
	"MOVED",
	"MOVER",
	"MOVES",
	"MOVIE",
	"MOWAS",
	"MOWER",
	"MOWRA",
	"MOYAS",
	"MOYLS",
	"MOZED",
	"MPRET",
	"MRADS",
	"MSASA",
	"MTEPE",
	"MUCHO",
	"MUCID",
	"MUCKY",
	"MUCRO",
	"MUCUS",
	"MUDAR",
	"MUDDY",
	"MUDIF",
	"MUDRA",
	"MUFFS",
	"MUFFY",
	"MUGGS",
	"MUGHO",
	"MUGIL",
	"MUILS",
	"MUJIK",
	"MUKTI",
	"MULCH",
	"MULED",
	"MULGA",
	"MULLA",
	"MULLS",
	"MULSH",
	"MUMMY",
	"MUMPH",
	"MUMPS",
	"MUNCH",
	"MUNDU",
	"MUNGA",
	"MUNGE",
	"MUNGI",
	"MUNGS",
	"MUNGY",
	"MUNIA",
	"MUNIS",
	"MUNTS",
	"MUNTU",
	"MURAL",
	"MURED",
	"MURES",
	"MUREX",
	"MURGI",
	"MURKS",
	"MURKY",
	"MURLS",
	"MURLY",
	"MURRS",
	"MURRY",
	"MUSAR",
	"MUSCA",
	"MUSEE",
	"MUSER",
	"MUSHY",
	"MUSIC",
	"MUSIT",
	"MUSKS",
	"MUSKY",
	"MUSTH",
	"MUSTY",
	"MUTAS",
	"MUTER",
	"MUTES",
	"MUTHA",
	"MUTIC",
	"MUTIS",
	"MUTUM",
	"MUVVA",
	"MUXED",
	"MVULA",
	"MVULE",
	"MVULI",
	"MYALL",
	"MYLAR",
	"MYNAS",
	"MYOPS",
	"MYOPY",
	"MYRRH",
	"MYSID",
	"MYTHI",
	"MYTHY",
	"MYXOS",
	"MZEES",
	"NAAMS",
	"NABBY",
	"NABES",
	"NABKS",
	"NACRE",
	"NADIR",
	"NAFFS",
	"NAGAS",
	"NAHAL",
	"NAICE",
	"NAIEO",
	"NAIFS",
	"NAILY",
	"NAINS",
	"NAIRA",
	"NAIVE",
	"NAKAS",
	"NAKED",
	"NAKER",
	"NAMED",
	"NAMES",
	"NAMMA",
	"NANCY",
	"NANDU",
	"NANNY",
	"NANOS",
	"NANTI",
	"NANTS",
	"NANTY",
	"NANUA",
	"NAPES",
	"NAPOO",
	"NAPPY",
	"NARDS",
	"NARIC",
	"NARIS",
	"NARKS",
	"NARKY",
	"NARRA",
	"NASAL",
	"NASHO",
	"NASON",
	"NASTY",
	"NASUS",
	"NATAK",
	"NATAL",
	"NATCH",
	"NATES",
	"NATIS",
	"NATTO",
	"NATTY",
	"NATYA",
	"NAUNT",
	"NAVAL",
	"NAVAR",
	"NAVED",
	"NAVEL",
	"NAVES",
	"NAVVY",
	"NAWAB",
	"NAWAL",
	"NAZES",
	"NAZZY",
	"NDUJA",
	"NEANT",
	"NEAPS",
	"NEATH",
	"NEBBY",
	"NEBEK",
	"NECHE",
	"NEDDY",
	"NEEDS",
	"NEEDY",
	"NEEFS",
	"NEELD",
	"NEEPS",
	"NEGRI",
	"NEGRO",
	"NEIGH",
	"NEIVE",
	"NELIA",
	"NELLY",
	"NEMAS",
	"NEMIC",
	"NENES",
	"NEOZA",
	"NERAL",
	"NERAM",
	"NERDS",
	"NERDY",
	"NERFS",
	"NERKA",
	"NERKS",
	"NERVE",
	"NERVY",
	"NESKI",
	"NESTS",
	"NETAS",
	"NETTA",
	"NETTS",
	"NEUKS",
	"NEUME",
	"NEUMS",
	"NEVEL",
	"NEVER",
	"NEVES",
	"NEVIS",
	"NEVUS",
	"NEWBS",
	"NEWER",
	"NEWIE",
	"NEWLY",
	"NEWSY",
	"NEXTS",
	"NEXUM",
	"NEXUS",
	"NGAKA",
	"NGANA",
	"NGAPI",
	"NGATI",
	"NGRAM",
	"NGWEE",
	"NICED",
	"NICER",
	"NICEY",
	"NICHE",
	"NICHT",
	"NICKS",
	"NIDOR",
	"NIDUS",
	"NIECE",
	"NIESS",
	"NIFES",
	"NIFFS",
	"NIFFY",
	"NIFLE",
	"NIGER",
	"NIGGA",
	"NIGHS",
	"NIGHT",
	"NIKAH",
	"NIKAU",
	"NILAS",
	"NILLS",
	"NIMBI",
	"NIMBY",
	"NINJA",
	"NINNY",
	"NINON",
	"NINTA",
	"NINTH",
	"NIOPO",
	"NIOZA",
	"NIPPY",
	"NISSE",
	"NISUS",
	"NITRO",
	"NITTA",
	"NITTO",
	"NIVAL",
	"NIVAS",
	"NIXED",
	"NIZAM",
	"NJIRL",
	"NOAHS",
	"NOBBY",
	"NOBLE",
	"NOBLY",
	"NOCKS",
	"NODED",
	"NODES",
	"NODUM",
	"NOELS",
	"NOEME",
	"NOGAL",
	"NOGGY",
	"NOHOW",
	"NOIAS",
	"NOINT",
	"NOIRE",
	"NOISE",
	"NOISY",
	"NOLES",
	"NOLLE",
	"NOLOS",
	"NOMAD",
	"NOMES",
	"NOMIC",
	"NOMOI",
	"NOMOS",
	"NONAS",
	"NONCE",
	"NONCY",
	"NONES",
	"NONET",
	"NONIC",
	"NONIS",
	"NONNA",
	"NONNO",
	"NONNY",
	"NONYL",
	"NOOBS",
	"NOOPS",
	"NOOSE",
	"NOOVE",
	"NORIA",
	"NORIE",
	"NORIS",
	"NORKS",
	"NORTH",
	"NOSED",
	"NOSEY",
	"NOTAM",
	"NOTCH",
	"NOTED",
	"NOTER",
	"NOTUM",
	"NOUJA",
	"NOULD",
	"NOULS",
	"NOVEL",
	"NOVUM",
	"NOWDS",
	"NOWED",
	"NOWLS",
	"NOWTS",
	"NOXAL",
	"NOYAU",
	"NOYED",
	"NOYES",
	"NRTTA",
	"NRTYA",
	"NSIMA",
	"NUBIA",
	"NUCHA",
	"NUCIN",
	"NUDDY",
	"NUDER",
	"NUDES",
	"NUDGE",
	"NUDGY",
	"NUDIE",
	"NUDZH",
	"NUEVO",
	"NUFFS",
	"NUKED",
	"NUKES",
	"NUMBS",
	"NUMEN",
	"NUNNY",
	"NUNUS",
	"NUQUE",
	"NURDS",
	"NURDY",
	"NURSE",
	"NUSED",
	"NUSES",
	"NUTSO",
	"NUTTY",
	"NYAFF",
	"NYALA",
	"NYAMS",
	"NYLON",
	"NYMPH",
	"NYUSE",
	"OAFOS",
	"OAKEN",
	"OAKUM",
	"OATEN",
	"OATER",
	"OATHS",
	"OBANG",
	"OBELI",
	"OBESE",
	"OBEYS",
	"OBIAS",
	"OBIIT",
	"OBJET",
	"OBOLE",
	"OBOLS",
	"OCCAM",
	"OCCUR",
	"OCEAN",
	"OCHRY",
	"OCOTE",
	"OCREA",
	"OCTAL",
	"OCTAS",
	"OCTET",
	"OCULI",
	"ODAHS",
	"ODALS",
	"ODDER",
	"ODDLY",
	"ODIST",
	"ODIUM",
	"ODOOM",
	"ODORS",
	"ODOUR",
	"ODYLE",
	"ODYLS",
	"OFAYS",
	"OFFAL",
	"OFFED",
	"OFFER",
	"OFLAG",
	"OFTEN",
	"OFURO",
	"OGEES",
	"OGGIN",
	"OGLER",
	"OGLES",
	"OGMIC",
	"OGRES",
	"OHELO",
	"OHIAS",
	"OHMIC",
	"OHONE",
	"OICKS",
	"OILED",
	"OINTS",
	"OJIME",
	"OKEHS",
	"OKOLE",
	"OKRUG",
	"OLATE",
	"OLDEN",
	"OLDER",
	"OLDLY",
	"OLEIC",
	"OLEIN",
	"OLEUM",
	"OLIOS",
	"OLIVA",
	"OLIVE",
	"OLLAS",
	"OLLAV",
	"OLLER",
	"OLOGY",
	"OLPAE",
	"OMASA",
	"OMBER",
	"OMBRE",
	"OMDAH",
	"OMDDA",
	"OMDEH",
	"OMEGA",
	"OMIAI",
	"OMITS",
	"OMMEL",
	"OMMIN",
	"OMNES",
	"OMULS",
	"ONCER",
	"ONELY",
	"ONERY",
	"ONGON",
	"ONION",
	"ONIUM",
	"ONKUS",
	"ONLAP",
	"ONNED",
	"ONSEN",
	"ONSET",
	"OOAAS",
	"OOBIT",
	"OOHED",
	"OOJAH",
	"OOMPH",
	"OONTS",
	"OOPAK",
	"OOSES",
	"OOTID",
	"OOZLE",
	"OPALS",
	"OPENS",
	"OPERA",
	"OPERY",
	"OPGAF",
	"OPIHI",
	"OPINE",
	"OPIUM",
	"OPSAT",
	"OPTER",
	"OPTIC",
	"ORANG",
	"ORANS",
	"ORANT",
	"ORATE",
	"ORBAT",
	"ORBIT",
	"ORDER",
	"ORDIE",
	"ORDOS",
	"OREAD",
	"ORFES",
	"ORGAN",
	"ORGIC",
	"ORIGO",
	"ORIXA",
	"ORLON",
	"ORLOP",
	"ORNEE",
	"ORNIS",
	"ORPED",
	"ORRIS",
	"ORTHO",
	"ORVAL",
	"OSCAR",
	"OSETR",
	"OSHAC",
	"OSLIN",
	"OSMIC",
	"OSMOL",
	"OSONE",
	"OTAKU",
	"OTHER",
	"OTHYL",
	"OTIUM",
	"OTTER",
	"OUCHT",
	"OUENS",
	"OUGHT",
	"OUIJA",
	"OULKS",
	"OUNCE",
	"OUPAS",
	"OUPED",
	"OUPHE",
	"OUREY",
	"OURIE",
	"OUSIA",
	"OUSTS",
	"OUTDO",
	"OUTED",
	"OUTEN",
	"OUTER",
	"OUTGO",
	"OUTRE",
	"OUTRO",
	"OUTTA",
	"OUZEL",
	"OUZOS",
	"OVALS",
	"OVARY",
	"OVATE",
	"OVELS",
	"OVENS",
	"OVERS",
	"OVERT",
	"OVINE",
	"OVISM",
	"OVOID",
	"OVOLI",
	"OVOLO",
	"OVULE",
	"OWARI",
	"OWING",
	"OWLET",
	"OWNED",
	"OWNER",
	"OWNIO",
	"OWRES",
	"OWRIE",
	"OXBOW",
	"OXEAS",
	"OXERS",
	"OXIDE",
	"OXIDS",
	"OXMEN",
	"OYERS",
	"OZONE",
	"PAALS",
	"PAANS",
	"PACES",
	"PACEY",
	"PACHA",
	"PACTA",
	"PACTS",
	"PADAM",
	"PADAS",
	"PADDY",
	"PADIS",
	"PADLE",
	"PADRE",
	"PAEAN",
	"PAGAN",
	"PAGED",
	"PAHUS",
	"PAILS",
	"PAINT",
	"PAIPS",
	"PAISA",
	"PAISE",
	"PAKAY",
	"PAKKA",
	"PALAS",
	"PALAY",
	"PALED",
	"PALER",
	"PALES",
	"PALKI",
	"PALLS",
	"PALLY",
	"PALMS",
	"PALMY",
	"PALPS",
	"PALSA",
	"PALSY",
	"PANCE",
	"PANDS",
	"PANDY",
	"PANED",
	"PANEL",
	"PANES",
	"PANIC",
	"PANIM",
	"PANKO",
	"PANNA",
	"PANNE",
	"PANNI",
	"PANNY",
	"PANSY",
	"PANTO",
	"PANTY",
	"PAOLI",
	"PAPAD",
	"PAPAL",
	"PAPAW",
	"PAPER",
	"PAPEY",
	"PAPPY",
	"PARAE",
	"PARAS",
	"PARCH",
	"PARDI",
	"PARDS",
	"PAREN",
	"PARER",
	"PARES",
	"PARGE",
	"PARGO",
	"PARIS",
	"PARKA",
	"PARKS",
	"PARKY",
	"PARMA",
	"PARMO",
	"PARRA",
	"PARRY",
	"PARSE",
	"PARTE",
	"PARTI",
	"PARTS",
	"PARTY",
	"PARVE",
	"PASAG",
	"PASAR",
	"PASCH",
	"PASEO",
	"PASHM",
	"PASPY",
	"PASTA",
	"PASTE",
	"PASTY",
	"PATAS",
	"PATCH",
	"PATED",
	"PATEE",
	"PATES",
	"PATHS",
	"PATIA",
	"PATIO",
	"PATKA",
	"PATLY",
	"PATSY",
	"PATTA",
	"PATTE",
	"PATTU",
	"PATTY",
	"PAUAS",
	"PAULS",
	"PAUSE",
	"PAVEN",
	"PAVES",
	"PAVID",
	"PAVIS",
	"PAVON",
	"PAWNS",
	"PAYED",
	"PAYEE",
	"PAYER",
	"PAYSD",
	"PEACE",
	"PEACH",
	"PEAGE",
	"PEAGS",
	"PEAKS",
	"PEAKY",
	"PEANS",
	"PEARE",
	"PEARL",
	"PEART",
	"PEATS",
	"PEAVY",
	"PEBAS",
	"PECAN",
	"PECIA",
	"PECKE",
	"PECKY",
	"PEDAL",
	"PEDES",
	"PEDON",
	"PEDRO",
	"PEEKY",
	"PEELS",
	"PEENS",
	"PEENT",
	"PEEOY",
	"PEEPY",
	"PEGMA",
	"PEGOS",
	"PEINS",
	"PEISE",
	"PEISY",
	"PEKAU",
	"PEKES",
	"PEKID",
	"PEKIN",
	"PELES",
	"PELFS",
	"PELLS",
	"PELOG",
	"PELTA",
	"PELTS",
	"PENAL",
	"PENCE",
	"PENDS",
	"PENES",
	"PENGO",
	"PENKS",
	"PENNE",
	"PENNY",
	"PENSY",
	"PEPLA",
	"PEPON",
	"PEPSI",
	"PEQUI",
	"PERAE",
	"PERAI",
	"PERCE",
	"PERCH",
	"PERDU",
	"PERES",
	"PERFS",
	"PERIL",
	"PERIS",
	"PERKS",
	"PERKY",
	"PERMY",
	"PERNE",
	"PERNS",
	"PEROG",
	"PERPS",
	"PERRY",
	"PERSP",
	"PERST",
	"PERVS",
	"PESKY",
	"PESTO",
	"PETAL",
	"PETER",
	"PETIT",
	"PETRE",
	"PETRI",
	"PETTY",
	"PEWED",
	"PEWEE",
	"PFFTT",
	"PHAGE",
	"PHARE",
	"PHARM",
	"PHASE",
	"PHASM",
	"PHEME",
	"PHENE",
	"PHEON",
	"PHESE",
	"PHIAL",
	"PHIZZ",
	"PHLOX",
	"PHONE",
	"PHONO",
	"PHONY",
	"PHOOH",
	"PHOOO",
	"PHOTO",
	"PHOTS",
	"PHUBS",
	"PHUTS",
	"PHWAT",
	"PHYLA",
	"PHYLE",
	"PHYMA",
	"PHYNX",
	"PHYSA",
	"PIANO",
	"PIANS",
	"PIBAL",
	"PICAS",
	"PICCY",
	"PICHI",
	"PICKS",
	"PICKY",
	"PICON",
	"PICRA",
	"PIECE",
	"PIETA",
	"PIETY",
	"PIEZO",
	"PIGGY",
	"PIGHT",
	"PIGLY",
	"PIING",
	"PIKAS",
	"PIKEL",
	"PIKER",
	"PIKES",
	"PIKEY",
	"PIKIS",
	"PILAR",
	"PILAU",
	"PILCH",
	"PILED",
	"PILER",
	"PILEY",
	"PILON",
	"PILOT",
	"PILOW",
	"PILUM",
	"PILUS",
	"PIMAS",
	"PIMPS",
	"PINAS",
	"PINCE",
	"PINCH",
	"PINDA",
	"PINDS",
	"PINED",
	"PINER",
	"PINES",
	"PINEY",
	"PINGA",
	"PINGE",
	"PINGS",
	"PINKO",
	"PINKS",
	"PINKY",
	"PINNA",
	"PINNY",
	"PINTA",
	"PINTO",
	"PINTS",
	"PINUP",
	"PIONY",
	"PIOUS",
	"PIOYS",
	"PIPAL",
	"PIPED",
	"PIPER",
	"PIPID",
	"PIPIS",
	"PIPIT",
	"PIQUE",
	"PIQUI",
	"PIRLS",
	"PIRNS",
	"PIROG",
	"PIRRE",
	"PISCO",
	"PISOS",
	"PISSY",
	"PITCH",
	"PITHY",
	"PITON",
	"PITOT",
	"PIUMA",
	"PIVOT",
	"PIXEL",
	"PIXIE",
	"PIZED",
	"PIZES",
	"PIZZA",
	"PLAAS",
	"PLACE",
	"PLACK",
	"PLAID",
	"PLAIG",
	"PLAIN",
	"PLAIT",
	"PLANC",
	"PLANE",
	"PLANH",
	"PLANK",
	"PLANS",
	"PLANT",
	"PLASM",
	"PLATE",
	"PLATS",
	"PLATT",
	"PLATY",
	"PLAUR",
	"PLAYA",
	"PLAYS",
	"PLAZA",
	"PLEAD",
	"PLEAT",
	"PLEBS",
	"PLENE",
	"PLENO",
	"PLEON",
	"PLESH",
	"PLETS",
	"PLEWS",
	"PLEXI",
	"PLIED",
	"PLIER",
	"PLIES",
	"PLINK",
	"PLISH",
	"PLOAT",
	"PLOCE",
	"PLODS",
	"PLOIT",
	"PLONK",
	"PLOOK",
	"PLOOT",
	"PLOPS",
	"PLORE",
	"PLOTS",
	"PLOUT",
	"PLOWS",
	"PLOWT",
	"PLOYS",
	"PLUCK",
	"PLUES",
	"PLUKE",
	"PLUMB",
	"PLUME",
	"PLUMP",
	"PLUNK",
	"PLUOT",
	"PLUPS",
	"PLUSH",
	"PLUTE",
	"PNEUS",
	"POACH",
	"POAKE",
	"POBBY",
	"POBOY",
	"POCAN",
	"POCKY",
	"PODAL",
	"PODGY",
	"PODOS",
	"POENA",
	"POEPS",
	"POESY",
	"POETE",
	"POGEY",
	"POGGE",
	"POGGY",
	"POGOS",
	"POGUE",
	"POIND",
	"POINT",
	"POIRE",
	"POISE",
	"POKED",
	"POKER",
	"POKES",
	"POKIE",
	"POLAR",
	"POLED",
	"POLES",
	"POLIO",
	"POLIS",
	"POLJE",
	"POLKA",
	"POLLO",
	"POLLS",
	"POLOS",
	"POLTS",
	"POLYP",
	"POLYS",
	"POMAS",
	"POMBE",
	"POMOS",
	"POMPA",
	"POMPS",
	"PONCE",
	"PONGO",
	"PONGS",
	"PONGY",
	"PONKS",
	"PONTS",
	"PONZU",
	"POOCH",
	"POODS",
	"POOED",
	"POOHS",
	"POOHY",
	"POOLS",
	"POOPA",
	"POORI",
	"POOTS",
	"POOVE",
	"POOVY",
	"POPES",
	"POPOS",
	"POPPY",
	"POPSY",
	"PORCH",
	"PORED",
	"PORES",
	"PORGE",
	"PORKS",
	"PORTA",
	"PORTE",
	"PORTH",
	"PORUS",
	"POSCA",
	"POSED",
	"POSER",
	"POSET",
	"POSEY",
	"POSIT",
	"POSSE",
	"POSTS",
	"POTAE",
	"POTAI",
	"POTCH",
	"POTED",
	"POTSY",
	"POTTS",
	"POUCE",
	"POUCH",
	"POUFS",
	"POUFY",
	"POUKE",
	"POULP",
	"POULT",
	"POUND",
	"POUPT",
	"POURS",
	"POUTS",
	"POUTY",
	"POVOS",
	"POWAN",
	"POWER",
	"POWIS",
	"POWNY",
	"POWRE",
	"POXES",
	"POYNT",
	"POZZY",
	"PRADS",
	"PRAHU",
	"PRANA",
	"PRANK",
	"PRAPS",
	"PRASE",
	"PRATE",
	"PRATT",
	"PRATY",
	"PRAWN",
	"PRAYS",
	"PREED",
	"PREEM",
	"PREEN",
	"PREES",
	"PREKE",
	"PREMS",
	"PREMY",
	"PRENT",
	"PRESS",
	"PREST",
	"PREVE",
	"PREXY",
	"PREYS",
	"PRIAL",
	"PRICE",
	"PRICK",
	"PRICY",
	"PRIDE",
	"PRIDY",
	"PRIED",
	"PRIGS",
	"PRIMA",
	"PRIME",
	"PRIMO",
	"PRIMP",
	"PRIMY",
	"PRINT",
	"PRION",
	"PRIOR",
	"PRISE",
	"PRISM",
	"PRISS",
	"PRIVY",
	"PRIZE",
	"PROAS",
	"PROBE",
	"PROBS",
	"PRODD",
	"PROIN",
	"PROKE",
	"PROLL",
	"PROMS",
	"PRONE",
	"PRONG",
	"PRONK",
	"PROOF",
	"PRORE",
	"PROSE",
	"PROSS",
	"PROST",
	"PROSY",
	"PROUD",
	"PROUL",
	"PROVE",
	"PROWL",
	"PROWS",
	"PROXY",
	"PRUDE",
	"PRUNE",
	"PRYAN",
	"PRYER",
	"PSALM",
	"PSEUD",
	"PSHAW",
	"PSIAS",
	"PSION",
	"PSOAS",
	"PSYCH",
	"PTISH",
	"PUBCO",
	"PUBIC",
	"PUBSY",
	"PUCAN",
	"PUCER",
	"PUCES",
	"PUCKA",
	"PUCKS",
	"PUDDY",
	"PUDGE",
	"PUDGY",
	"PUDSY",
	"PUDUS",
	"PUERS",
	"PUFFA",
	"PUFFS",
	"PUFFY",
	"PUGGY",
	"PUGIL",
	"PUHAS",
	"PUKER",
	"PUKES",
	"PUKKA",
	"PUKUS",
	"PULAO",
	"PULES",
	"PULIS",
	"PULLS",
	"PULPS",
	"PULPY",
	"PULSE",
	"PULUT",
	"PUMPS",
	"PUMPY",
	"PUNAS",
	"PUNCH",
	"PUNGO",
	"PUNGS",
	"PUNIM",
	"PUNJI",
	"PUNKA",
	"PUNKY",
	"PUNNY",
	"PUNTS",
	"PUPAE",
	"PUPAS",
	"PUPIL",
	"PUPPY",
	"PUPUS",
	"PURAO",
	"PURAU",
	"PURDA",
	"PUREE",
	"PURER",
	"PURGA",
	"PURGE",
	"PURIN",
	"PURIS",
	"PURPY",
	"PURRS",
	"PURSE",
	"PUSES",
	"PUSHY",
	"PUSLE",
	"PUSSY",
	"PUTAS",
	"PUTER",
	"PUTON",
	"PUTOS",
	"PUTTI",
	"PUTTO",
	"PUTTU",
	"PUTTY",
	"PUUKO",
	"PUZEL",
	"PUZTA",
	"PWNED",
	"PYGMY",
	"PYNES",
	"PYOID",
	"PYRIC",
	"PYRUS",
	"PYXED",
	"PYXES",
	"PYXIE",
	"PZAZZ",
	"QAIDS",
	"QAJAQ",
	"QANAT",
	"QAPIK",
	"QILAS",
	"QIPAO",
	"QOPHS",
	"QORMA",
	"QUACK",
	"QUADS",
	"QUAFF",
	"QUAGS",
	"QUAIL",
	"QUAIS",
	"QUAKE",
	"QUALM",
	"QUALY",
	"QUANK",
	"QUARK",
	"QUARL",
	"QUART",
	"QUASH",
	"QUASI",
	"QUAWK",
	"QUAWS",
	"QUAYD",
	"QUAYS",
	"QUBIT",
	"QUEAN",
	"QUECK",
	"QUEEK",
	"QUEEN",
	"QUEER",
	"QUELL",
	"QUEME",
	"QUENA",
	"QUERY",
	"QUEST",
	"QUEUE",
	"QUEYS",
	"QUEYU",
	"QUIBS",
	"QUICH",
	"QUICK",
	"QUIDS",
	"QUIET",
	"QUILA",
	"QUILL",
	"QUILT",
	"QUINA",
	"QUINK",
	"QUINS",
	"QUINT",
	"QUIPO",
	"QUIPU",
	"QUIRK",
	"QUIRL",
	"QUIST",
	"QUITE",
	"QUOAD",
	"QUODS",
	"QUOIF",
	"QUOIN",
	"QUOIS",
	"QUOIT",
	"QUOLL",
	"QUONK",
	"QUORL",
	"QUOTA",
	"QUOTE",
	"QUOTH",
	"QURAN",
	"QURSH",
	"RABBI",
	"RABIC",
	"RABID",
	"RABIS",
	"RACED",
	"RACER",
	"RACHE",
	"RACKS",
	"RACON",
	"RADAR",
	"RADDI",
	"RADDY",
	"RADGE",
	"RADGY",
	"RADII",
	"RADIO",
	"RADON",
	"RAFFS",
	"RAFFY",
	"RAFIK",
	"RAFIQ",
	"RAFTS",
	"RAFTY",
	"RAGAS",
	"RAGER",
	"RAGES",
	"RAGGA",
	"RAGGS",
	"RAGGY",
	"RAGIS",
	"RAGUS",
	"RAHUI",
	"RAIAH",
	"RAIDS",
	"RAIKE",
	"RAIKS",
	"RAINS",
	"RAINY",
	"RAIRD",
	"RAISE",
	"RAITA",
	"RAITH",
	"RAJAH",
	"RAJAS",
	"RAKED",
	"RAKEE",
	"RAKER",
	"RAKES",
	"RAKIA",
	"RAKIS",
	"RALLI",
	"RALLY",
	"RALPH",
	"RAMAL",
	"RAMEN",
	"RAMES",
	"RAMET",
	"RAMIE",
	"RAMIN",
	"RAMIS",
	"RAMUS",
	"RANCE",
	"RANCH",
	"RANDO",
	"RANDS",
	"RANDY",
	"RANES",
	"RANGE",
	"RANKE",
	"RANKS",
	"RANNS",
	"RANNY",
	"RANTS",
	"RAPED",
	"RAPER",
	"RAPES",
	"RAPID",
	"RAPPE",
	"RAPSO",
	"RAREE",
	"RARER",
	"RARES",
	"RARKS",
	"RASAM",
	"RASED",
	"RASES",
	"RASPS",
	"RASPY",
	"RASSE",
	"RATAL",
	"RATAN",
	"RATAS",
	"RATCH",
	"RATEL",
	"RATER",
	"RATHA",
	"RATHE",
	"RATHS",
	"RATIO",
	"RATOO",
	"RATOS",
	"RATTI",
	"RATTY",
	"RAULI",
	"RAUNS",
	"RAVEN",
	"RAVER",
	"RAVEY",
	"RAWDY",
	"RAWIN",
	"RAWKS",
	"RAWLY",
	"RAWNS",
	"RAXES",
	"RAYAH",
	"RAYAS",
	"RAYED",
	"RAYLE",
	"RAYLS",
	"RAYON",
	"RAZAI",
	"RAZER",
	"RAZET",
	"RAZOO",
	"RAZOR",
	"REACH",
	"REACT",
	"READD",
	"READY",
	"REAIS",
	"REALM",
	"REAMY",
	"REANS",
	"REARM",
	"REARS",
	"REATA",
	"REAVE",
	"REBAB",
	"REBAR",
	"REBEL",
	"REBIT",
	"REBOP",
	"REBUD",
	"REBUS",
	"REBUT",
	"REBUY",
	"RECAP",
	"RECCE",
	"RECCO",
	"RECEP",
	"RECIT",
	"RECKS",
	"RECON",
	"RECTA",
	"RECTO",
	"RECUR",
	"RECUT",
	"REDAN",
	"REDES",
	"REDIA",
	"REDID",
	"REDIG",
	"REDLY",
	"REDON",
	"REDRY",
	"REDUX",
	"REDYE",
	"REEAF",
	"REEDY",
	"REEFY",
	"REELY",
	"REEMS",
	"REENS",
	"REEST",
	"REEVE",
	"REEZE",
	"REFAN",
	"REFED",
	"REFEL",
	"REFER",
	"REFIT",
	"REFIX",
	"REGAL",
	"REGES",
	"REGEX",
	"REGIA",
	"REGIE",
	"REHAB",
	"REIGN",
	"REIKS",
	"REINE",
	"REINK",
	"REIRD",
	"REIST",
	"REJAS",
	"REKEY",
	"RELAX",
	"RELAY",
	"RELET",
	"RELIC",
	"RELIT",
	"RELOS",
	"REMAN",
	"REMAP",
	"REMEN",
	"REMET",
	"REMEX",
	"REMIT",
	"REMIX",
	"REMOU",
	"RENAL",
	"RENDS",
	"RENEW",
	"RENGA",
	"RENGS",
	"RENIG",
	"RENIN",
	"RENOS",
	"RENTE",
	"RENTS",
	"REORG",
	"REPAS",
	"REPAT",
	"REPAY",
	"REPEL",
	"REPLY",
	"REPOS",
	"REPOT",
	"REPPS",
	"REPRO",
	"REPUN",
	"REPUT",
	"RERAN",
	"RERIG",
	"RERUN",
	"RESAM",
	"RESAY",
	"RESES",
	"RESET",
	"RESID",
	"RESIN",
	"RESOD",
	"RESOL",
	"RESTS",
	"RESUS",
	"RETAX",
	"RETCH",
	"RETIA",
	"RETIE",
	"RETIP",
	"RETOX",
	"RETRO",
	"RETRY",
	"REUPS",
	"REUSE",
	"REVEL",
	"REVET",
	"REVUE",
	"REWAN",
	"REWIN",
	"REWON",
	"RHEME",
	"RHIES",
	"RHINE",
	"RHINO",
	"RHOMB",
	"RHONE",
	"RHYME",
	"RHYNE",
	"RHYTA",
	"RIADS",
	"RIBAS",
	"RIBBY",
	"RICED",
	"RICES",
	"RICHE",
	"RICHT",
	"RICIN",
	"RIDER",
	"RIDGE",
	"RIELS",
	"RIEMS",
	"RIEVE",
	"RIFFY",
	"RIFLE",
	"RIFTE",
	"RIGGS",
	"RIGHT",
	"RIGID",
	"RIGMO",
	"RIGOR",
	"RIKWA",
	"RILES",
	"RILEY",
	"RILLS",
	"RIMER",
	"RIMES",
	"RINCE",
	"RINES",
	"RINGE",
	"RINGS",
	"RINKS",
	"RINSE",
	"RIOTS",
	"RIPED",
	"RIPEN",
	"RIPER",
	"RIPPS",
	"RISEN",
	"RISER",
	"RISHI",
	"RISKS",
	"RISKY",
	"RISUS",
	"RITES",
	"RITZY",
	"RIVAL",
	"RIVED",
	"RIVER",
	"RIVES",
	"RIVET",
	"RIYAL",
	"RIZAS",
	"ROACH",
	"ROADS",
	"ROADY",
	"ROAKY",
	"ROAMS",
	"ROANS",
	"ROANY",
	"ROAST",
	"ROBER",
	"ROBES",
	"ROBIN",
	"ROBOT",
	"ROBUR",
	"ROCHE",
	"ROCKY",
	"RODEO",
	"RODES",
	"ROERS",
	"ROGER",
	"ROGUE",
	"ROGUY",
	"ROHES",
	"ROHUN",
	"ROILY",
	"ROJAK",
	"ROJIS",
	"ROKED",
	"ROKER",
	"ROKES",
	"ROKOS",
	"ROLAG",
	"ROLEO",
	"ROLFS",
	"ROLLS",
	"ROLLY",
	"ROMAL",
	"ROMER",
	"ROMPY",
	"RONEO",
	"RONNE",
	"RONTE",
	"RONTS",
	"RONUK",
	"ROODS",
	"ROOMY",
	"ROOPS",
	"ROOPY",
	"ROOSA",
	"ROOST",
	"ROPEY",
	"RORIE",
	"RORTS",
	"RORTY",
	"ROSAL",
	"ROSCO",
	"ROSET",
	"ROSHA",
	"ROSSA",
	"ROSTI",
	"ROSTS",
	"ROTAN",
	"ROTED",
	"ROTIS",
	"ROTLS",
	"ROTOR",
	"ROTTE",
	"ROTTY",
	"ROUEN",
	"ROUFS",
	"ROUGE",
	"ROUGH",
	"ROUKY",
	"ROUND",
	"ROUSE",
	"ROUTE",
	"ROUTH",
	"ROVED",
	"ROVER",
	"ROVES",
	"ROWAN",
	"ROWDY",
	"ROWED",
	"ROWEL",
	"ROWER",
	"ROWET",
	"ROWME",
	"ROWNS",
	"ROWTS",
	"ROYAL",
	"ROYET",
	"ROYNE",
	"ROYST",
	"ROZET",
	"RUACH",
	"RUANA",
	"RUBAI",
	"RUBBY",
	"RUBES",
	"RUBLI",
	"RUCHE",
	"RUDDS",
	"RUDDY",
	"RUDER",
	"RUDES",
	"RUDIE",
	"RUERS",
	"RUFFS",
	"RUGBY",
	"RUING",
	"RUINS",
	"RULER",
	"RUMAL",
	"RUMBA",
	"RUMBO",
	"RUMES",
	"RUMOR",
	"RUMPO",
	"RUNED",
	"RUNES",
	"RUNGS",
	"RUNIC",
	"RUNNY",
	"RUNOS",
	"RUNTS",
	"RUNTY",
	"RUPEE",
	"RUPIA",
	"RURAL",
	"RURUS",
	"RUSHY",
	"RUSKS",
	"RUSKY",
	"RUSSE",
	"RUSTY",
	"RUTHS",
	"RUTIN",
	"RUTTY",
	"RUVID",
	"RYBAT",
	"RYIJI",
	"RYIJY",
	"RYMME",
	"RYNDS",
	"RYOTI",
	"RYOTS",
	"RYPER",
	"RYTHE",
	"SAAGS",
	"SABAL",
	"SABER",
	"SABES",
	"SABHA",
	"SABIR",
	"SABJI",
	"SABLE",
	"SABOT",
	"SABRA",
	"SABRE",
	"SACKS",
	"SACRA",
	"SADES",
	"SADHU",
	"SADLY",
	"SADOS",
	"SAFER",
	"SAGAR",
	"SAGER",
	"SAGGY",
	"SAGOS",
	"SAGUM",
	"SAHAB",
	"SAICE",
	"SAICS",
	"SAIDS",
	"SAINE",
	"SAINT",
	"SAKAI",
	"SAKER",
	"SAKES",
	"SAKIA",
	"SAKIS",
	"SAKTI",
	"SALAD",
	"SALAS",
	"SALAT",
	"SALIC",
	"SALLE",
	"SALLY",
	"SALMI",
	"SALON",
	"SALPA",
	"SALPS",
	"SALSA",
	"SALSE",
	"SALTO",
	"SALTS",
	"SALTY",
	"SALVE",
	"SALVO",
	"SAMAS",
	"SAMEK",
	"SAMEL",
	"SAMEN",
	"SAMMY",
	"SANAD",
	"SANDY",
	"SANED",
	"SANER",
	"SANES",
	"SANGA",
	"SANGH",
	"SANSA",
	"SANTO",
	"SAOLA",
	"SAPOR",
	"SAPPY",
	"SARAN",
	"SARDS",
	"SAREE",
	"SARGE",
	"SARGO",
	"SARIN",
	"SARIR",
	"SAROS",
	"SARVO",
	"SASSY",
	"SATED",
	"SATIN",
	"SATIS",
	"SATYR",
	"SAUBA",
	"SAUCE",
	"SAUCY",
	"SAULS",
	"SAUNA",
	"SAUNF",
	"SAURY",
	"SAUTE",
	"SAUTS",
	"SAVED",
	"SAVEY",
	"SAVIN",
	"SAVOR",
	"SAVOY",
	"SAVVY",
	"SAWER",
	"SAYAS",
	"SAYED",
	"SAYEE",
	"SAYER",
	"SAYID",
	"SAYNE",
	"SCADS",
	"SCAFF",
	"SCAIL",
	"SCALD",
	"SCALE",
	"SCALL",
	"SCALP",
	"SCALY",
	"SCAMP",
	"SCAMS",
	"SCANT",
	"SCAPA",
	"SCAPE",
	"SCAPI",
	"SCARE",
	"SCARF",
	"SCARP",
	"SCARY",
	"SCATS",
	"SCATT",
	"SCAUP",
	"SCAUR",
	"SCAWS",
	"SCEAT",
	"SCENA",
	"SCEND",
	"SCENE",
	"SCENT",
	"SCHUL",
	"SCIFI",
	"SCION",
	"SCIRE",
	"SCOBE",
	"SCODY",
	"SCOFF",
	"SCOGS",
	"SCOLD",
	"SCONE",
	"SCOOG",
	"SCOOP",
	"SCOOT",
	"SCOPE",
	"SCOPS",
	"SCORE",
	"SCORN",
	"SCORP",
	"SCOTE",
	"SCOTS",
	"SCOUG",
	"SCOUR",
	"SCOUT",
	"SCOWL",
	"SCRAB",
	"SCRAG",
	"SCRAM",
	"SCRAP",
	"SCRAT",
	"SCRAW",
	"SCRAY",
	"SCREE",
	"SCREW",
	"SCRIP",
	"SCROB",
	"SCROD",
	"SCROO",
	"SCROW",
	"SCRUB",
	"SCRUM",
	"SCUBA",
	"SCUDI",
	"SCUFT",
	"SCUGS",
	"SCULK",
	"SCULL",
	"SCULP",
	"SCULS",
	"SCURF",
	"SCURS",
	"SCUSE",
	"SCUTA",
	"SCUTS",
	"SDEIN",
	"SEALS",
	"SEAME",
	"SEAMS",
	"SEAMY",
	"SEARE",
	"SEARS",
	"SEASE",
	"SEATS",
	"SEAZE",
	"SEBUM",
	"SEDAN",
	"SEDUM",
	"SEEDS",
	"SEEDY",
	"SEEKS",
	"SEELS",
	"SEELY",
	"SEEMS",
	"SEEPS",
	"SEEPY",
	"SEFER",
	"SEGNO",
	"SEGOS",
	"SEGUE",
	"SEHRI",
	"SEIFS",
	"SEILS",
	"SEINE",
	"SEIRS",
	"SEISM",
	"SEIZE",
	"SEKTS",
	"SELFS",
	"SELFY",
	"SELLE",
	"SEMEN",
	"SEMES",
	"SENAS",
	"SENDS",
	"SENES",
	"SENEX",
	"SENGI",
	"SENOR",
	"SENSA",
	"SENSE",
	"SENSI",
	"SENSU",
	"SENTI",
	"SENZA",
	"SEPAD",
	"SEPAL",
	"SEPIA",
	"SEPTA",
	"SERAC",
	"SERAI",
	"SERAL",
	"SERGE",
	"SERIF",
	"SERIR",
	"SERKS",
	"SERON",
	"SEROW",
	"SERRS",
	"SERRY",
	"SERUM",
	"SERVE",
	"SESEY",
	"SETAE",
	"SETAL",
	"SETER",
	"SETHS",
	"SETON",
	"SETTS",
	"SETUP",
	"SEVEN",
	"SEVER",
	"SEVIR",
	"SEWAR",
	"SEWED",
	"SEWEN",
	"SEWER",
	"SHACK",
	"SHADE",
	"SHADY",
	"SHAFT",
	"SHAGS",
	"SHAHS",
	"SHAKE",
	"SHAKY",
	"SHALE",
	"SHALL",
	"SHALM",
	"SHALT",
	"SHALY",
	"SHAME",
	"SHAMS",
	"SHAND",
	"SHANK",
	"SHAPE",
	"SHAPS",
	"SHARD",
	"SHARE",
	"SHARK",
	"SHARP",
	"SHART",
	"SHASH",
	"SHAUL",
	"SHAVE",
	"SHAWL",
	"SHAWN",
	"SHAYA",
	"SHCHI",
	"SHEAL",
	"SHEAR",
	"SHEAS",
	"SHEDS",
	"SHEEN",
	"SHEEP",
	"SHEER",
	"SHEET",
	"SHEIK",
	"SHELF",
	"SHELL",
	"SHEND",
	"SHENG",
	"SHETS",
	"SHEVA",
	"SHIAI",
	"SHIED",
	"SHIER",
	"SHIFT",
	"SHILL",
	"SHINE",
	"SHINS",
	"SHINY",
	"SHIPS",
	"SHIRE",
	"SHIRK",
	"SHIRT",
	"SHISO",
	"SHIST",
	"SHIUR",
	"SHIVA",
	"SHIVE",
	"SHLEP",
	"SHLUB",
	"SHOAL",
	"SHOAT",
	"SHOCK",
	"SHOER",
	"SHOJI",
	"SHOJO",
	"SHONE",
	"SHOOK",
	"SHOOT",
	"SHOPE",
	"SHORE",
	"SHORL",
	"SHORN",
	"SHORT",
	"SHOTS",
	"SHOTT",
	"SHOUD",
	"SHOUT",
	"SHOVE",
	"SHOWN",
	"SHOWY",
	"SHOYU",
	"SHRED",
	"SHREW",
	"SHRUB",
	"SHRUG",
	"SHTAR",
	"SHTUP",
	"SHUCK",
	"SHUNS",
	"SHUNT",
	"SHURA",
	"SHUSH",
	"SHUTS",
	"SHWAS",
	"SHYER",
	"SHYLY",
	"SICES",
	"SICHT",
	"SICKO",
	"SICKS",
	"SICKY",
	"SIDAS",
	"SIDER",
	"SIDES",
	"SIDEY",
	"SIDHE",
	"SIDLE",
	"SIEGE",
	"SIETH",
	"SIEVE",
	"SIGHT",
	"SIGIL",
	"SIGMA",
	"SIGNA",
	"SIGRI",
	"SIKER",
	"SIKES",
	"SILEN",
	"SILER",
	"SILES",
	"SILEX",
	"SILKY",
	"SILLS",
	"SILLY",
	"SILOS",
	"SILTS",
	"SIMAS",
	"SIMBA",
	"SIMIS",
	"SIMPS",
	"SINCE",
	"SINED",
	"SINEW",
	"SINGE",
	"SINGS",
	"SINHS",
	"SINKS",
	"SIPED",
	"SIPES",
	"SIPPY",
	"SIREN",
	"SIRIH",
	"SIRIS",
	"SIRRA",
	"SIRUP",
	"SISAL",
	"SISES",
	"SISSY",
	"SITES",
	"SITKA",
	"SITUS",
	"SIXES",
	"SIXMO",
	"SIXTH",
	"SIXTY",
	"SIZER",
	"SIZES",
	"SKAGS",
	"SKANK",
	"SKARN",
	"SKATE",
	"SKATS",
	"SKAWS",
	"SKEDS",
	"SKEED",
	"SKEEF",
	"SKEEN",
	"SKEER",
	"SKEES",
	"SKEEV",
	"SKEGG",
	"SKEIN",
	"SKELF",
	"SKELL",
	"SKELM",
	"SKENE",
	"SKENS",
	"SKEOS",
	"SKERM",
	"SKERS",
	"SKETS",
	"SKEWS",
	"SKIER",
	"SKIEY",
	"SKIFF",
	"SKILL",
	"SKIMP",
	"SKINK",
	"SKINS",
	"SKIOS",
	"SKIPS",
	"SKIRL",
	"SKIRT",
	"SKITS",
	"SKIVE",
	"SKOAL",
	"SKOFF",
	"SKOGS",
	"SKOLS",
	"SKOOL",
	"SKUAS",
	"SKUGS",
	"SKULK",
	"SKULL",
	"SKUNK",
	"SKYED",
	"SKYEY",
	"SKYRE",
	"SKYTE",
	"SLACK",
	"SLADE",
	"SLAES",
	"SLAID",
	"SLAIN",
	"SLAKE",
	"SLAMS",
	"SLANE",
	"SLANG",
	"SLANK",
	"SLANT",
	"SLAPS",
	"SLASH",
	"SLATE",
	"SLAWS",
	"SLAYS",
	"SLEBS",
	"SLEDS",
	"SLEEK",
	"SLEEP",
	"SLEET",
	"SLEPT",
	"SLEWS",
	"SLICE",
	"SLICK",
	"SLIDE",
	"SLIME",
	"SLIMS",
	"SLIMY",
	"SLING",
	"SLINK",
	"SLIPS",
	"SLIPT",
	"SLISH",
	"SLITS",
	"SLIVE",
	"SLOAN",
	"SLOBS",
	"SLOES",
	"SLOGS",
	"SLOID",
	"SLOOP",
	"SLOOT",
	"SLOPE",
	"SLOPS",
	"SLORM",
	"SLOSH",
	"SLOTH",
	"SLUBB",
	"SLUED",
	"SLUES",
	"SLUFF",
	"SLUMP",
	"SLUMS",
	"SLUNG",
	"SLUNK",
	"SLURP",
	"SLUSE",
	"SLUSH",
	"SLUTS",
	"SLYLY",
	"SMACK",
	"SMALL",
	"SMALM",
	"SMARM",
	"SMART",
	"SMASH",
	"SMEAR",
	"SMEEK",
	"SMEES",
	"SMELL",
	"SMELT",
	"SMERK",
	"SMEWS",
	"SMILE",
	"SMILY",
	"SMIRK",
	"SMITE",
	"SMITH",
	"SMITS",
	"SMIZE",
	"SMOCK",
	"SMOGS",
	"SMOKE",
	"SMOKO",
	"SMOKY",
	"SMOLT",
	"SMORE",
	"SMOTE",
	"SMUGS",
	"SMUSH",
	"SNABS",
	"SNACK",
	"SNAGS",
	"SNAIL",
	"SNAKE",
	"SNAKY",
	"SNARE",
	"SNARF",
	"SNARK",
	"SNARL",
	"SNAWS",
	"SNEAK",
	"SNEAP",
	"SNEBS",
	"SNEER",
	"SNEES",
	"SNIBS",
	"SNICK",
	"SNIDE",
	"SNIED",
	"SNIES",
	"SNIFF",
	"SNIFT",
	"SNIPE",
	"SNIPS",
	"SNIPY",
	"SNIRT",
	"SNITS",
	"SNOBS",
	"SNOEK",
	"SNOKE",
	"SNOOD",
	"SNOOL",
	"SNOOP",
	"SNORE",
	"SNORT",
	"SNOUT",
	"SNOWK",
	"SNOWY",
	"SNUBS",
	"SNUCK",
	"SNUFF",
	"SNUGS",
	"SNUSH",
	"SOAKS",
	"SOAPY",
	"SOARE",
	"SOARS",
	"SOAVE",
	"SOBER",
	"SOCIA",
	"SOCKS",
	"SOCLE",
	"SODDY",
	"SOFAR",
	"SOFTA",
	"SOFTS",
	"SOFTY",
	"SOGER",
	"SOGGY",
	"SOILS",
	"SOILY",
	"SOKAH",
	"SOLAN",
	"SOLAR",
	"SOLEI",
	"SOLES",
	"SOLID",
	"SOLON",
	"SOLUS",
	"SOLVE",
	"SOMAN",
	"SOMAS",
	"SONAR",
	"SONCE",
	"SONGY",
	"SONIC",
	"SONLY",
	"SONNE",
	"SONSY",
	"SOOKS",
	"SOOKY",
	"SOOMS",
	"SOOPS",
	"SOOTE",
	"SOOTH",
	"SOOTY",
	"SOPHS",
	"SOPOR",
	"SOPRA",
	"SORDA",
	"SORDO",
	"SORED",
	"SOREE",
	"SORER",
	"SOREX",
	"SORGO",
	"SORNS",
	"SORRY",
	"SORTS",
	"SOTOL",
	"SOTTO",
	"SOULS",
	"SOUND",
	"SOUPS",
	"SOUPY",
	"SOURS",
	"SOUTH",
	"SOUTS",
	"SOWCE",
	"SOWER",
	"SOWFS",
	"SOWLS",
	"SOWNE",
	"SOWPS",
	"SOWSE",
	"SOYAS",
	"SOYUZ",
	"SPACE",
	"SPACK",
	"SPACY",
	"SPADE",
	"SPADO",
	"SPADS",
	"SPAED",
	"SPAER",
	"SPAHI",
	"SPAIN",
	"SPALE",
	"SPALT",
	"SPAMS",
	"SPANE",
	"SPANG",
	"SPANK",
	"SPANS",
	"SPARE",
	"SPARK",
	"SPARS",
	"SPASM",
	"SPAWL",
	"SPAWN",
	"SPAYD",
	"SPAZA",
	"SPAZZ",
	"SPEAK",
	"SPEAN",
	"SPEAR",
	"SPECK",
	"SPECS",
	"SPECT",
	"SPEED",
	"SPEEL",
	"SPEER",
	"SPEIL",
	"SPEIR",
	"SPELL",
	"SPELT",
	"SPEND",
	"SPENT",
	"SPEOS",
	"SPERM",
	"SPETS",
	"SPEUG",
	"SPEWS",
	"SPICE",
	"SPICK",
	"SPICY",
	"SPIDE",
	"SPIED",
	"SPIEL",
	"SPIER",
	"SPIFF",
	"SPIFS",
	"SPIKE",
	"SPIKS",
	"SPIKY",
	"SPILE",
	"SPILL",
	"SPILT",
	"SPIMS",
	"SPINE",
	"SPINK",
	"SPINS",
	"SPINY",
	"SPIRE",
	"SPIRT",
	"SPITE",
	"SPITZ",
	"SPIVS",
	"SPLAT",
	"SPLIT",
	"SPLOG",
	"SPODS",
	"SPOIL",
	"SPOKE",
	"SPOOF",
	"SPOOK",
	"SPOOL",
	"SPOON",
	"SPOOR",
	"SPORE",
	"SPORT",
	"SPOSH",
	"SPOTS",
	"SPOUT",
	"SPRAD",
	"SPRAG",
	"SPRAT",
	"SPRAY",
	"SPRED",
	"SPREE",
	"SPREW",
	"SPRIG",
	"SPRIT",
	"SPRUG",
	"SPUED",
	"SPUER",
	"SPUGS",
	"SPULE",
	"SPUME",
	"SPUNK",
	"SPURN",
	"SPURS",
	"SPURT",
	"SPUTA",
	"SQUAB",
	"SQUAD",
	"SQUAT",
	"SQUAW",
	"SQUEE",
	"SQUIB",
	"SQUID",
	"SQUIZ",
	"STACK",
	"STAFF",
	"STAGE",
	"STAGY",
	"STAID",
	"STAIG",
	"STAIN",
	"STAIR",
	"STAKE",
	"STALE",
	"STALK",
	"STALL",
	"STAMP",
	"STAND",
	"STANK",
	"STANS",
	"STAPH",
	"STAPS",
	"STARE",
	"STARK",
	"STARS",
	"START",
	"STASH",
	"STATE",
	"STATS",
	"STATU",
	"STAVE",
	"STAYS",
	"STEAD",
	"STEAK",
	"STEAL",
	"STEAM",
	"STEAN",
	"STEAR",
	"STEDD",
	"STEDE",
	"STEED",
	"STEEK",
	"STEEL",
	"STEEP",
	"STEER",
	"STEIL",
	"STEIN",
	"STELA",
	"STELE",
	"STELL",
	"STEND",
	"STENO",
	"STEPS",
	"STEPT",
	"STERE",
	"STERN",
	"STETS",
	"STEWS",
	"STEWY",
	"STICH",
	"STICK",
	"STIFF",
	"STILE",
	"STILL",
	"STILT",
	"STING",
	"STINK",
	"STINT",
	"STIPA",
	"STIRE",
	"STIRP",
	"STIVE",
	"STOAE",
	"STOAI",
	"STOAS",
	"STOCK",
	"STOGY",
	"STOIC",
	"STOKE",
	"STOLE",
	"STOLN",
	"STOMP",
	"STONE",
	"STONK",
	"STONN",
	"STONY",
	"STOOD",
	"STOOL",
	"STOOP",
	"STOOR",
	"STOPE",
	"STOPT",
	"STORE",
	"STORK",
	"STORM",
	"STORY",
	"STOSS",
	"STOTS",
	"STOUR",
	"STOUT",
	"STOVE",
	"STOWN",
	"STOWP",
	"STOWS",
	"STRAD",
	"STRAG",
	"STRAP",
	"STRAW",
	"STRAY",
	"STREP",
	"STREW",
	"STRIA",
	"STRIM",
	"STRIP",
	"STROW",
	"STROY",
	"STRUM",
	"STRUT",
	"STUCK",
	"STUDE",
	"STUDY",
	"STUFF",
	"STUMM",
	"STUMP",
	"STUNG",
	"STUNK",
	"STUNT",
	"STURE",
	"STURT",
	"STUSH",
	"STYES",
	"STYLE",
	"STYLO",
	"STYMY",
	"STYRE",
	"STYTE",
	"SUAVE",
	"SUBAS",
	"SUCRE",
	"SUDDS",
	"SUDOR",
	"SUENT",
	"SUERS",
	"SUETE",
	"SUETS",
	"SUGAN",
	"SUGAR",
	"SUGHS",
	"SUGOS",
	"SUING",
	"SUITE",
	"SUITS",
	"SUJEE",
	"SULCI",
	"SULFO",
	"SULKY",
	"SULLS",
	"SULLY",
	"SULUS",
	"SUMAC",
	"SUMIS",
	"SUMMA",
	"SUMOS",
	"SUMPH",
	"SUMPS",
	"SUNKS",
	"SUNNA",
	"SUNNS",
	"SUNNY",
	"SUNTS",
	"SUPER",
	"SUPES",
	"SUPRA",
	"SURAL",
	"SURAS",
	"SURAT",
	"SURDS",
	"SURER",
	"SURFS",
	"SURGE",
	"SURGY",
	"SURLY",
	"SURRA",
	"SUSES",
	"SUSHI",
	"SUSUS",
	"SWABS",
	"SWAIL",
	"SWALE",
	"SWALY",
	"SWAMI",
	"SWAMP",
	"SWANG",
	"SWAPS",
	"SWAPT",
	"SWARD",
	"SWARF",
	"SWARM",
	"SWASH",
	"SWATH",
	"SWAYL",
	"SWEAR",
	"SWEAT",
	"SWEDE",
	"SWEEP",
	"SWEER",
	"SWEES",
	"SWEET",
	"SWEIR",
	"SWELL",
	"SWELT",
	"SWEPT",
	"SWERF",
	"SWIFT",
	"SWIGS",
	"SWILL",
	"SWIMS",
	"SWINE",
	"SWING",
	"SWINK",
	"SWIRE",
	"SWIRL",
	"SWISH",
	"SWISS",
	"SWITH",
	"SWITS",
	"SWIZZ",
	"SWOBS",
	"SWOLE",
	"SWOLN",
	"SWOON",
	"SWOOP",
	"SWOPT",
	"SWORD",
	"SWORE",
	"SWORN",
	"SWOTS",
	"SWOUN",
	"SWUNG",
	"SYBBE",
	"SYCEE",
	"SYCES",
	"SYCON",
	"SYEDS",
	"SYKER",
	"SYLIS",
	"SYLPH",
	"SYMAR",
	"SYNDS",
	"SYNED",
	"SYNES",
	"SYNOD",
	"SYPES",
	"SYPHS",
	"SYRAH",
	"SYREN",
	"SYRUP",
	"SYSOP",
	"SYTHE",
	"SYVER",
	"TAALS",
	"TAATA",
	"TABBY",
	"TABES",
	"TABLA",
	"TABLE",
	"TABLS",
	"TABOO",
	"TABOR",
	"TABOS",
	"TABUN",
	"TABUS",
	"TACET",
	"TACHE",
	"TACHO",
	"TACIT",
	"TACKS",
	"TACKY",
	"TACOS",
	"TACTS",
	"TAFFY",
	"TAGMA",
	"TAGUA",
	"TAIGA",
	"TAIKO",
	"TAILS",
	"TAINT",
	"TAIRA",
	"TAISH",
	"TAJES",
	"TAKAS",
	"TAKEN",
	"TAKER",
	"TAKES",
	"TAKHT",
	"TAKIS",
	"TAKKY",
	"TALAK",
	"TALAQ",
	"TALAR",
	"TALCS",
	"TALCY",
	"TALLY",
	"TALMA",
	"TALON",
	"TALUS",
	"TAMAL",
	"TAMAS",
	"TAMED",
	"TAMER",
	"TAMIN",
	"TAMIS",
	"TAMMY",
	"TANGA",
	"TANGI",
	"TANGO",
	"TANGY",
	"TANHS",
	"TANKA",
	"TANKS",
	"TANSU",
	"TANSY",
	"TANTE",
	"TANTI",
	"TAPED",
	"TAPER",
	"TAPES",
	"TAPIR",
	"TAPUS",
	"TARAS",
	"TARDO",
	"TARDY",
	"TARED",
	"TARKA",
	"TAROC",
	"TAROS",
	"TAROT",
	"TARRE",
	"TARSE",
	"TARSI",
	"TARTS",
	"TARTY",
	"TARZY",
	"TASAR",
	"TASES",
	"TASSA",
	"TASSO",
	"TASTE",
	"TASTY",
	"TATER",
	"TATHS",
	"TATIE",
	"TATOU",
	"TATTS",
	"TATTY",
	"TAULD",
	"TAUNT",
	"TAVAH",
	"TAVAS",
	"TAWAF",
	"TAWAI",
	"TAWER",
	"TAWNY",
	"TAWSE",
	"TAXER",
	"TAXES",
	"TAXUS",
	"TAYRA",
	"TAZZE",
	"TEACH",
	"TEAED",
	"TEAKS",
	"TEALS",
	"TEAMS",
	"TEARS",
	"TEARY",
	"TEASE",
	"TEATS",
	"TECHS",
	"TECUM",
	"TEDDY",
	"TEENS",
	"TEENY",
	"TEERS",
	"TEETH",
	"TEGGS",
	"TEGUA",
	"TEGUS",
	"TEHRS",
	"TEIID",
	"TEILS",
	"TEINS",
	"TEKKE",
	"TELES",
	"TELEX",
	"TELIA",
	"TELIC",
	"TELLY",
	"TELOI",
	"TELOS",
	"TEMED",
	"TEMPO",
	"TEMPS",
	"TEMSE",
	"TENDU",
	"TENES",
	"TENET",
	"TENGE",
	"TENON",
	"TENOR",
	"TENSE",
	"TENTH",
	"TENTS",
	"TENTY",
	"TEPEE",
	"TEPID",
	"TEPOY",
	"TERAS",
	"TEREK",
	"TERES",
	"TERFE",
	"TERFS",
	"TERMS",
	"TERRA",
	"TERRY",
	"TERSE",
	"TERTS",
	"TERZA",
	"TESTA",
	"TESTY",
	"TETES",
	"TETRI",
	"TEUGH",
	"TEWEL",
	"TEWIT",
	"TEXTS",
	"THAGI",
	"THALE",
	"THALI",
	"THANA",
	"THANE",
	"THANK",
	"THANS",
	"THANX",
	"THARM",
	"THAWT",
	"THEBE",
	"THECA",
	"THEEK",
	"THEFT",
	"THEGN",
	"THEIC",
	"THEIR",
	"THEMA",
	"THEME",
	"THEOR",
	"THEOW",
	"THERE",
	"THERM",
	"THESE",
	"THESP",
	"THETA",
	"THETE",
	"THEWS",
	"THEWY",
	"THICK",
	"THIEF",
	"THIGH",
	"THING",
	"THINK",
	"THINS",
	"THIRD",
	"THIRL",
	"THOFT",
	"THONG",
	"THORN",
	"THOSE",
	"THRAE",
	"THREE",
	"THREW",
	"THRID",
	"THRIP",
	"THROB",
	"THROE",
	"THROW",
	"THRUM",
	"THUDS",
	"THUGS",
	"THUJA",
	"THUMB",
	"THUMP",
	"THURL",
	"THUYA",
	"THYME",
	"THYMI",
	"THYMY",
	"TIARA",
	"TIARE",
	"TIARS",
	"TIBIA",
	"TICED",
	"TICES",
	"TICKS",
	"TICKY",
	"TIDAL",
	"TIDDY",
	"TIDED",
	"TIDES",
	"TIEFS",
	"TIERS",
	"TIFOS",
	"TIGER",
	"TIGES",
	"TIGHT",
	"TIKKA",
	"TILAK",
	"TILDE",
	"TILER",
	"TILES",
	"TIMBO",
	"TIMED",
	"TIMER",
	"TIMES",
	"TIMID",
	"TIMON",
	"TINAS",
	"TINED",
	"TINGE",
	"TINGS",
	"TINKS",
	"TINNY",
	"TINTS",
	"TIPIS",
	"TIPPY",
	"TIPSY",
	"TIPUP",
	"TIRES",
	"TIRLS",
	"TIRRS",
	"TIRTH",
	"TITAN",
	"TITHE",
	"TITHI",
	"TITIN",
	"TITIR",
	"TITLE",
	"TITRE",
	"TITUP",
	"TIYIN",
	"TIYNS",
	"TIZES",
	"TIZZY",
	"TOADS",
	"TOADY",
	"TOAST",
	"TOAZE",
	"TOCKS",
	"TODAY",
	"TODDE",
	"TODDY",
	"TODOS",
	"TOEAS",
	"TOFFY",
	"TOFUS",
	"TOGAE",
	"TOGES",
	"TOGUE",
	"TOIDY",
	"TOILE",
	"TOILS",
	"TOING",
	"TOKAY",
	"TOKEN",
	"TOKER",
	"TOKES",
	"TOKOS",
	"TOLAN",
	"TOLAS",
	"TOLES",
	"TOLTS",
	"TOLUS",
	"TOMBS",
	"TOMES",
	"TOMIA",
	"TOMMY",
	"TOMOS",
	"TONAL",
	"TONDO",
	"TONED",
	"TONES",
	"TONGA",
	"TONGS",
	"TONIC",
	"TONKA",
	"TONUS",
	"TOOLS",
	"TOOMS",
	"TOOTH",
	"TOPAZ",
	"TOPEE",
	"TOPER",
	"TOPHI",
	"TOPHS",
	"TOPIC",
	"TOPIS",
	"TOPOS",
	"TOQUE",
	"TORAS",
	"TORCH",
	"TORCS",
	"TORII",
	"TORRS",
	"TORSE",
	"TORSO",
	"TORTA",
	"TORTE",
	"TORTS",
	"TORUS",
	"TOSHY",
	"TOSSY",
	"TOSYL",
	"TOTAL",
	"TOTEM",
	"TOTTY",
	"TOUCH",
	"TOUGH",
	"TOUKS",
	"TOUNS",
	"TOUSY",
	"TOUZE",
	"TOUZY",
	"TOWED",
	"TOWEL",
	"TOWER",
	"TOWIE",
	"TOWSE",
	"TOWSY",
	"TOWTS",
	"TOWZE",
	"TOXIC",
	"TOXIN",
	"TOYED",
	"TOYER",
	"TOYOS",
	"TOZES",
	"TOZIE",
	"TRABS",
	"TRACE",
	"TRACK",
	"TRACT",
	"TRADE",
	"TRADS",
	"TRAGS",
	"TRAIL",
	"TRAIN",
	"TRAIT",
	"TRAMP",
	"TRAMS",
	"TRANK",
	"TRANS",
	"TRANT",
	"TRAPO",
	"TRAPS",
	"TRASH",
	"TRATS",
	"TRAWL",
	"TRAYF",
	"TREAD",
	"TREAT",
	"TRECK",
	"TREEN",
	"TREIF",
	"TREMS",
	"TREND",
	"TREWS",
	"TREYF",
	"TRIAD",
	"TRIAL",
	"TRIBE",
	"TRICE",
	"TRICK",
	"TRIDE",
	"TRIED",
	"TRIES",
	"TRIFA",
	"TRIFF",
	"TRIGS",
	"TRIKE",
	"TRIMS",
	"TRINE",
	"TRIOR",
	"TRIOS",
	"TRIPE",
	"TRIPY",
	"TRIST",
	"TRITE",
	"TROAD",
	"TROKE",
	"TROLL",
	"TRONA",
	"TRONC",
	"TRONE",
	"TROOP",
	"TROPE",
	"TROTS",
	"TROUT",
	"TROVE",
	"TRUCE",
	"TRUCK",
	"TRUER",
	"TRUES",
	"TRUGO",
	"TRUGS",
	"TRULL",
	"TRULY",
	"TRUMP",
	"TRUNK",
	"TRUSS",
	"TRUST",
	"TRUTH",
	"TRYMA",
	"TRYPS",
	"TRYST",
	"TSADI",
	"TSARS",
	"TSKED",
	"TSUBA",
	"TSUBO",
	"TUANS",
	"TUART",
	"TUBAE",
	"TUBAL",
	"TUBAR",
	"TUBAS",
	"TUBED",
	"TUBER",
	"TUCKS",
	"TUFFE",
	"TUFFS",
	"TUFTY",
	"TUILE",
	"TUKTU",
	"TULIP",
	"TULLE",
	"TULPA",
	"TULSI",
	"TUMID",
	"TUMMY",
	"TUMOR",
	"TUMPS",
	"TUNDS",
	"TUNED",
	"TUNER",
	"TUNES",
	"TUNGS",
	"TUNIC",
	"TUPEK",
	"TUPLE",
	"TUQUE",
	"TURBO",
	"TURDS",
	"TURFS",
	"TURKS",
	"TURMS",
	"TURNS",
	"TURNT",
	"TURPS",
	"TURRS",
	"TUSHY",
	"TUSKY",
	"TUTEE",
	"TUTOR",
	"TUTTI",
	"TUXES",
	"TUYER",
	"TWAIN",
	"TWALS",
	"TWANG",
	"TWANK",
	"TWATS",
	"TWAYS",
	"TWEAK",
	"TWEED",
	"TWEEN",
	"TWEET",
	"TWERK",
	"TWERP",
	"TWICE",
	"TWIER",
	"TWIGS",
	"TWINE",
	"TWINK",
	"TWINY",
	"TWIRL",
	"TWIRP",
	"TWIST",
	"TWIXT",
	"TWYER",
	"TYEES",
	"TYERS",
	"TYING",
	"TYKES",
	"TYMPS",
	"TYNES",
	"TYPEY",
	"TYPIC",
	"TYPOS",
	"TYRAN",
	"TYRES",
	"TZARS",
	"UDDER",
	"UDYOG",
	"UGALI",
	"UGGED",
	"UHLAN",
	"UKASE",
	"ULANS",
	"ULCER",
	"ULEMA",
	"ULMOS",
	"ULNAD",
	"ULNAE",
	"ULNAR",
	"ULPAN",
	"ULTRA",
	"UMAMI",
	"UMBEL",
	"UMBRA",
	"UMBRE",
	"UMIAC",
	"UMPTY",
	"UMRAH",
	"UMRAS",
	"UNAGI",
	"UNAIS",
	"UNAPT",
	"UNARM",
	"UNBAG",
	"UNBAN",
	"UNBAR",
	"UNBID",
	"UNBOX",
	"UNCES",
	"UNCLE",
	"UNCOS",
	"UNCUS",
	"UNCUT",
	"UNDAM",
	"UNDEE",
	"UNDER",
	"UNDID",
	"UNDOS",
	"UNDUE",
	"UNFED",
	"UNFIT",
	"UNGAG",
	"UNGET",
	"UNHAT",
	"UNHIP",
	"UNIFY",
	"UNION",
	"UNIOS",
	"UNITE",
	"UNITS",
	"UNITY",
	"UNKED",
	"UNKET",
	"UNKID",
	"UNKUT",
	"UNLAY",
	"UNLET",
	"UNLID",
	"UNLIT",
	"UNMAN",
	"UNMET",
	"UNMEW",
	"UNOLD",
	"UNPAY",
	"UNPEN",
	"UNRED",
	"UNSAY",
	"UNSET",
	"UNSEW",
	"UNSOD",
	"UNTAG",
	"UNTAX",
	"UNTIE",
	"UNTIL",
	"UNWED",
	"UNWET",
	"UNZIP",
	"UPBOW",
	"UPDOS",
	"UPDRY",
	"UPFUL",
	"UPJET",
	"UPPER",
	"UPSEE",
	"UPSET",
	"UPTIE",
	"URAEI",
	"URAOS",
	"URATE",
	"URBAN",
	"URDEE",
	"UREAS",
	"UREDO",
	"UREIC",
	"URENA",
	"URENT",
	"URGED",
	"URIAL",
	"URINE",
	"URITE",
	"URMAN",
	"URNAL",
	"URNED",
	"URSID",
	"URUBU",
	"USAGE",
	"USHER",
	"USING",
	"USNEA",
	"USQUE",
	"USTER",
	"USUAL",
	"USURP",
	"UTERI",
	"UTILE",
	"UTTER",
	"UVEAL",
	"UVEAS",
	"UVULA",
	"VACAY",
	"VACUI",
	"VADED",
	"VADES",
	"VAGUE",
	"VAIRS",
	"VAIRY",
	"VAKAS",
	"VAKIL",
	"VALET",
	"VALID",
	"VALOR",
	"VALUE",
	"VALVE",
	"VAMPS",
	"VAMPY",
	"VANES",
	"VANGA",
	"VANGS",
	"VANTS",
	"VAPER",
	"VAPES",
	"VAPID",
	"VAPOR",
	"VARDA",
	"VARDO",
	"VAREC",
	"VARES",
	"VARIX",
	"VARNA",
	"VARVE",
	"VASAL",
	"VASES",
	"VASTY",
	"VATAS",
	"VATUS",
	"VAULT",
	"VAUNT",
	"VAUTS",
	"VAWTE",
	"VAXES",
	"VEENA",
	"VEEPS",
	"VEERS",
	"VEERY",
	"VEGAN",
	"VEGAS",
	"VEGES",
	"VEGGO",
	"VEILS",
	"VEINS",
	"VELAR",
	"VELES",
	"VELLS",
	"VENAE",
	"VENAL",
	"VENDU",
	"VENEY",
	"VENOM",
	"VENTI",
	"VENUE",
	"VERBS",
	"VERGE",
	"VERRA",
	"VERSE",
	"VERSO",
	"VERST",
	"VERTE",
	"VERTS",
	"VERTU",
	"VERVE",
	"VESTA",
	"VESTS",
	"VETCH",
	"VEXED",
	"VEXER",
	"VEXES",
	"VEXIL",
	"VEZIR",
	"VIALS",
	"VIAND",
	"VIBED",
	"VIBES",
	"VIBEY",
	"VICAR",
	"VICED",
	"VICHY",
	"VIDEO",
	"VIEWY",
	"VIFFS",
	"VIGIL",
	"VIGOR",
	"VILDE",
	"VILLA",
	"VILLS",
	"VINAS",
	"VINCA",
	"VINED",
	"VINHO",
	"VINIC",
	"VINNY",
	"VINOS",
	"VINYL",
	"VIOLA",
	"VIOLD",
	"VIOLS",
	"VIPER",
	"VIRAL",
	"VIRES",
	"VIRGA",
	"VIRGE",
	"VIRGO",
	"VIRID",
	"VIRLS",
	"VIRUS",
	"VISAS",
	"VISES",
	"VISIE",
	"VISIT",
	"VISNE",
	"VISON",
	"VISOR",
	"VISTA",
	"VISTO",
	"VITAL",
	"VITEX",
	"VITRO",
	"VITTA",
	"VIVAS",
	"VIVDA",
	"VIVER",
	"VIVES",
	"VIVID",
	"VIVOS",
	"VIVRE",
	"VIXEN",
	"VIZIR",
	"VIZOR",
	"VLEIS",
	"VLOGS",
	"VOARS",
	"VOBLA",
	"VOCAB",
	"VOCAL",
	"VOCES",
	"VODKA",
	"VOEMA",
	"VOGIE",
	"VOGUE",
	"VOICE",
	"VOICI",
	"VOIDS",
	"VOILA",
	"VOLAE",
	"VOLAR",
	"VOLED",
	"VOLET",
	"VOLKS",
	"VOLTA",
	"VOLTE",
	"VOLTI",
	"VOLTS",
	"VOLVE",
	"VOMER",
	"VOMIT",
	"VOTED",
	"VOTER",
	"VOTES",
	"VOUCH",
	"VOUGE",
	"VOWEL",
	"VOXES",
	"VRAIC",
	"VROUS",
	"VROUW",
	"VROWS",
	"VUGGS",
	"VUGHY",
	"VULGO",
	"VUTTY",
	"VYING",
	"WACKO",
	"WACKS",
	"WACKY",
	"WADAS",
	"WADDS",
	"WADED",
	"WADES",
	"WADGE",
	"WAFER",
	"WAFFS",
	"WAGED",
	"WAGER",
	"WAGES",
	"WAGGA",
	"WAGON",
	"WAGYU",
	"WAHEY",
	"WAIFS",
	"WAIFT",
	"WAILS",
	"WAIRS",
	"WAIST",
	"WAITE",
	"WAITS",
	"WAIVE",
	"WAKEN",
	"WAKER",
	"WAKES",
	"WAKFS",
	"WALDO",
	"WALER",
	"WALIS",
	"WALLA",
	"WALLY",
	"WALTZ",
	"WAMUS",
	"WANES",
	"WANKS",
	"WANKY",
	"WANLY",
	"WANTA",
	"WANTY",
	"WANZE",
	"WARBS",
	"WARBY",
	"WARED",
	"WAREZ",
	"WARKS",
	"WARPS",
	"WARTS",
	"WARTY",
	"WASES",
	"WASHI",
	"WASHY",
	"WASPS",
	"WASPY",
	"WASTE",
	"WASTS",
	"WATAP",
	"WATCH",
	"WATER",
	"WATTS",
	"WAUFF",
	"WAUGH",
	"WAULS",
	"WAVED",
	"WAVER",
	"WAVES",
	"WAVEY",
	"WAXEN",
	"WAXER",
	"WAZIR",
	"WAZOO",
	"WEALD",
	"WEAMB",
	"WEANS",
	"WEARY",
	"WEAVE",
	"WEBER",
	"WEDGE",
	"WEDGY",
	"WEEDS",
	"WEEDY",
	"WEEKE",
	"WEELS",
	"WEEMS",
	"WEENS",
	"WEEPS",
	"WEEST",
	"WEETE",
	"WEIGH",
	"WEILS",
	"WEIRD",
	"WEIRS",
	"WEISE",
	"WEKAS",
	"WELCH",
	"WELKE",
	"WELKS",
	"WELLY",
	"WELSH",
	"WENNY",
	"WENTS",
	"WERFS",
	"WERSH",
	"WESTS",
	"WETLY",
	"WEXED",
	"WEXES",
	"WHACK",
	"WHALE",
	"WHAMS",
	"WHANG",
	"WHARF",
	"WHATA",
	"WHATS",
	"WHEAR",
	"WHEAT",
	"WHEEL",
	"WHEEN",
	"WHEEP",
	"WHELK",
	"WHELM",
	"WHELP",
	"WHENS",
	"WHERE",
	"WHEWS",
	"WHEYS",
	"WHICH",
	"WHIFF",
	"WHIGS",
	"WHILE",
	"WHINE",
	"WHINS",
	"WHINY",
	"WHIOS",
	"WHIPT",
	"WHIRL",
	"WHIRR",
	"WHIRS",
	"WHISK",
	"WHIST",
	"WHITE",
	"WHITY",
	"WHIZZ",
	"WHOLE",
	"WHOOP",
	"WHOOT",
	"WHORE",
	"WHORL",
	"WHOSE",
	"WHOSO",
	"WICKY",
	"WIDEN",
	"WIDER",
	"WIDOW",
	"WIDTH",
	"WIELD",
	"WIELS",
	"WIFES",
	"WIFEY",
	"WIFTS",
	"WIFTY",
	"WIGAN",
	"WIGGY",
	"WIGHT",
	"WILCO",
	"WILED",
	"WILES",
	"WILGA",
	"WILIS",
	"WILJA",
	"WILLS",
	"WILLY",
	"WILTS",
	"WIMPY",
	"WINCE",
	"WINCH",
	"WINDS",
	"WINDY",
	"WINED",
	"WINES",
	"WINGS",
	"WINNA",
	"WINNS",
	"WINZE",
	"WIPED",
	"WIRED",
	"WIRER",
	"WIRES",
	"WIRRA",
	"WIRRI",
	"WISED",
	"WISER",
	"WISHA",
	"WISHT",
	"WISPS",
	"WISPY",
	"WISTS",
	"WITAN",
	"WITCH",
	"WITES",
	"WITHY",
	"WITTY",
	"WIVED",
	"WIVER",
	"WIZEN",
	"WIZZO",
	"WOADS",
	"WOADY",
	"WOCKS",
	"WODGY",
	"WOFUL",
	"WOKEN",
	"WOKKA",
	"WOLFS",
	"WOLLY",
	"WOMAN",
	"WOMAS",
	"WOMBS",
	"WOMBY",
	"WOMEN",
	"WOMYN",
	"WONGA",
	"WONKS",
	"WOODS",
	"WOODY",
	"WOOED",
	"WOOER",
	"WOOFY",
	"WOOLD",
	"WOOLS",
	"WOOLY",
	"WOONS",
	"WOOPY",
	"WOOTZ",
	"WOOZY",
	"WORDS",
	"WORDY",
	"WORKS",
	"WORLD",
	"WORRY",
	"WORSE",
	"WORST",
	"WORTH",
	"WORTS",
	"WOULD",
	"WOUND",
	"WOVEN",
	"WOWED",
	"WRACK",
	"WRAPS",
	"WRAST",
	"WRATH",
	"WRAWL",
	"WREAK",
	"WRECK",
	"WRENS",
	"WREST",
	"WRICK",
	"WRIED",
	"WRIER",
	"WRIES",
	"WRING",
	"WRIST",
	"WRITE",
	"WRITS",
	"WRONG",
	"WROOT",
	"WROTE",
	"WROTH",
	"WRUNG",
	"WRYLY",
	"WUDUS",
	"WULLS",
	"WURST",
	"WUSHU",
	"WUSSY",
	"WUXIA",
	"WYLED",
	"WYLES",
	"WYTES",
	"XERIC",
	"XOANA",
	"XOLOS",
	"XRAYS",
	"XYLAN",
	"XYLIC",
	"XYLOL",
	"XYSTI",
	"YAARS",
	"YABAS",
	"YABBA",
	"YACCA",
	"YACHT",
	"YACKA",
	"YACKS",
	"YAFFS",
	"YAGER",
	"YAHOO",
	"YAJNA",
	"YAKKA",
	"YAKOW",
	"YAMEN",
	"YAMPA",
	"YAMUN",
	"YAPPY",
	"YARAK",
	"YARCO",
	"YARER",
	"YARFA",
	"YARRA",
	"YARRS",
	"YARTA",
	"YARTO",
	"YATES",
	"YAUDS",
	"YAWED",
	"YAWNS",
	"YAWPS",
	"YAYAS",
	"YCLAD",
	"YCOND",
	"YEADS",
	"YEAHS",
	"YEARD",
	"YEARN",
	"YEAST",
	"YEESH",
	"YELKS",
	"YELLS",
	"YELTS",
	"YERBA",
	"YESES",
	"YESKS",
	"YETIS",
	"YEUKS",
	"YEVES",
	"YEWEN",
	"YFERE",
	"YIELD",
	"YILLS",
	"YINCE",
	"YIPPY",
	"YIRDS",
	"YIRKS",
	"YIRTH",
	"YLEMS",
	"YLIDS",
	"YMOLT",
	"YMPES",
	"YOBBO",
	"YOBBY",
	"YOCKS",
	"YODHS",
	"YOGEE",
	"YOJAN",
	"YOKEG",
	"YOKEL",
	"YOKES",
	"YOKUL",
	"YOLKY",
	"YOMPS",
	"YONIC",
	"YONNY",
	"YOOFS",
	"YOPPO",
	"YORKS",
	"YOUKS",
	"YOUNG",
	"YOUTH",
	"YOWES",
	"YOWIE",
	"YOWLS",
	"YOYOS",
	"YRAPT",
	"YRNEH",
	"YSAME",
	"YUCAS",
	"YUCCA",
	"YUGAS",
	"YUKES",
	"YUKKY",
	"YUKOS",
	"YUMMY",
	"YUPPY",
	"YURTS",
	"YUZUS",
	"ZABRA",
	"ZACKS",
	"ZAIDE",
	"ZAKAT",
	"ZAMAC",
	"ZAMAK",
	"ZAMAN",
	"ZANJA",
	"ZANZA",
	"ZANZE",
	"ZATIS",
	"ZAWNS",
	"ZAZEN",
	"ZEALS",
	"ZEBEC",
	"ZEBRA",
	"ZEBUB",
	"ZEBUS",
	"ZEDAS",
	"ZEERA",
	"ZEINS",
	"ZENDO",
	"ZEROS",
	"ZESTY",
	"ZETAS",
	"ZHUSH",
	"ZHUZH",
	"ZIBET",
	"ZIGAN",
	"ZILCH",
	"ZILLA",
	"ZILLS",
	"ZINCO",
	"ZINEB",
	"ZINES",
	"ZINGS",
	"ZINGY",
	"ZINKE",
	"ZINKY",
	"ZINOS",
	"ZIPPY",
	"ZOAEA",
	"ZOCCO",
	"ZOEAE",
	"ZOEAL",
	"ZOISM",
	"ZONAL",
	"ZONDA",
	"ZONKS",
	"ZOOEA",
	"ZOOEY",
	"ZOOMY",
	"ZOONS",
	"ZOOTY",
	"ZOPPA",
	"ZOPPO",
	"ZORIL",
	"ZORIS",
	"ZORRO",
	"ZOWEE",
	"ZOWIE",
	"ZULUS",
	"ZUPAN",
	"ZUPPA",
	"ZURFS",
	"ZUZIM",
	"ZYGAL",
	"SWIPE"
];
//#endregion
//#region extension/src/solver/solverOrchestrator.ts
var DEFAULT_CONFIG$1 = {
	hardMode: false,
	llmFallbackEnabled: false,
	typingDelay: 120,
	groqApiKey: ""
};
function calculateWinProbability(N, currentRow, entropy, isExploratory) {
	if (N <= 0) return 0;
	if (N === 1) return 1;
	let attemptsLeft = 6 - currentRow;
	if (isExploratory) attemptsLeft--;
	if (attemptsLeft <= 0) return 0;
	if (attemptsLeft === 1) return 1 / N;
	if (N <= attemptsLeft) return 1;
	const expectedGuessesRequired = .5 + Math.log2(N) / Math.max(.5, entropy);
	const margin = attemptsLeft - expectedGuessesRequired;
	const sigmoidP = 1 / (1 + Math.exp(-1.5 * margin));
	return Math.min(.99, Math.max(1 / N, sigmoidP));
}
var SolverOrchestrator = class {
	constructor(onUpdate) {
		this.config = { ...DEFAULT_CONFIG$1 };
		this.isRunning = false;
		this.shouldStop = false;
		this.remainingWords = [];
		this.guessHistory = [];
		this.currentRow = 0;
		this.topCandidates = [];
		this.expectedRemaining = 0;
		this.winProbability = .99;
		this.onUpdate = onUpdate;
	}
	updateConfig(config) {
		this.config = {
			...this.config,
			...config
		};
	}
	getConfig() {
		return { ...this.config };
	}
	stop() {
		this.shouldStop = true;
		this.isRunning = false;
		this.emit({
			phase: "idle",
			statusMessage: "Stopped by user."
		});
	}
	emit(partial) {
		this.onUpdate({
			...partial,
			isRunning: this.isRunning,
			mode: "auto"
		});
	}
	emitAssist(partial) {
		this.onUpdate({
			...partial,
			isRunning: this.isRunning,
			mode: "assist"
		});
	}
	sleep(ms) {
		return new Promise((resolve) => setTimeout(resolve, ms));
	}
	async startAutoSolve(bridge) {
		if (this.isRunning) return;
		this.isRunning = true;
		this.shouldStop = false;
		clearPatternCache();
		try {
			await bridge.updateBadge("🧠 Reading board...");
			const boardState = await bridge.readBoard();
			if (boardState.gameStatus !== "playing") {
				this.emit({
					phase: boardState.gameStatus === "won" ? "won" : "lost",
					statusMessage: `Game already ${boardState.gameStatus}.`
				});
				this.isRunning = false;
				return;
			}
			this.currentRow = boardState.currentRow;
			this.guessHistory = [];
			this.remainingWords = [...answerList];
			for (let r = 0; r < this.currentRow; r++) {
				const row = boardState.rows[r];
				const word = row.map((t) => t.letter).join("").toUpperCase();
				const result = row.map((t) => {
					if (t.state === "correct") return "correct";
					if (t.state === "present") return "present";
					return "absent";
				});
				if (word.length === 5) {
					this.guessHistory.push({
						word,
						result
					});
					this.remainingWords = filterWordList(this.remainingWords, word, result);
				}
			}
			this.emit({
				phase: "thinking",
				currentRow: this.currentRow,
				remainingWords: this.remainingWords,
				guessHistory: this.guessHistory,
				statusMessage: `Resuming from row ${this.currentRow + 1}. ${this.remainingWords.length} words.`
			});
			while (this.currentRow < 6 && !this.shouldStop) {
				await bridge.updateBadge(`🧠 Attempt ${this.currentRow + 1}/6`);
				const guess = await this.computeBestGuess();
				if (this.shouldStop) break;
				if (!guess) {
					this.emit({
						phase: "lost",
						statusMessage: "No valid guess found."
					});
					break;
				}
				this.emit({
					phase: "typing",
					currentGuess: guess,
					statusMessage: `Typing: ${guess}`
				});
				await bridge.updateBadge(`⌨️ ${guess}`);
				await bridge.typeWord(guess, this.config.typingDelay);
				if (this.shouldStop) break;
				await bridge.submitGuess();
				this.emit({
					phase: "evaluating",
					statusMessage: "Waiting for reveal..."
				});
				await this.sleep(300);
				let results;
				try {
					results = await bridge.waitForReveal(this.currentRow);
				} catch (err) {
					if (err.message === "NOT_IN_WORD_LIST") {
						this.emit({
							phase: "thinking",
							statusMessage: `Word rejected by dictionary: ${guess}. Retrying...`
						});
						this.remainingWords = this.remainingWords.filter((w) => w.toUpperCase() !== guess.toUpperCase());
						continue;
					}
					throw err;
				}
				if (this.shouldStop) break;
				this.emit({
					phase: "evaluating",
					statusMessage: "Word entered. Waiting 3 seconds..."
				});
				await this.sleep(3e3);
				if (this.shouldStop) break;
				const guessResult = results.map((r) => r.state);
				const isWon = guessResult.every((r) => r === "correct");
				this.guessHistory.push({
					word: guess,
					result: guessResult
				});
				this.remainingWords = filterWordList(this.remainingWords, guess, guessResult);
				this.currentRow++;
				if (isWon) {
					await bridge.updateBadge(`🎉 Solved in ${this.currentRow}!`);
					this.emit({
						phase: "won",
						currentRow: this.currentRow,
						guessHistory: this.guessHistory,
						remainingWords: this.remainingWords,
						statusMessage: `Solved in ${this.currentRow} attempt${this.currentRow > 1 ? "s" : ""}!`
					});
					break;
				}
				if (this.currentRow >= 6) {
					await bridge.updateBadge("❌ Failed");
					this.emit({
						phase: "lost",
						currentRow: this.currentRow,
						guessHistory: this.guessHistory,
						remainingWords: this.remainingWords,
						statusMessage: "Failed to solve within 6 attempts."
					});
					break;
				}
				this.emit({
					phase: "thinking",
					currentRow: this.currentRow,
					guessHistory: this.guessHistory,
					remainingWords: this.remainingWords,
					topCandidates: this.topCandidates,
					expectedRemaining: this.expectedRemaining,
					winProbability: this.winProbability,
					statusMessage: `${this.remainingWords.length} words remaining.`
				});
			}
		} catch (err) {
			const msg = err instanceof Error ? err.message : String(err);
			this.emit({
				phase: "lost",
				statusMessage: `Error: ${msg}`
			});
			await bridge.updateBadge(`❌ Error`);
		} finally {
			this.isRunning = false;
			this.emit({});
		}
	}
	async computeAssist(bridge) {
		if (this.isRunning) return;
		this.isRunning = true;
		this.shouldStop = false;
		clearPatternCache();
		try {
			const boardState = await bridge.readBoard();
			this.currentRow = boardState.currentRow;
			this.guessHistory = [];
			this.remainingWords = [...answerList];
			for (let r = 0; r < this.currentRow; r++) {
				const row = boardState.rows[r];
				const word = row.map((t) => t.letter).join("").toUpperCase();
				const result = row.map((t) => {
					if (t.state === "correct") return "correct";
					if (t.state === "present") return "present";
					return "absent";
				});
				if (word.length === 5) {
					this.guessHistory.push({
						word,
						result
					});
					this.remainingWords = filterWordList(this.remainingWords, word, result);
				}
			}
			const guess = await this.computeBestGuess();
			this.emitAssist({
				phase: "idle",
				currentRow: this.currentRow,
				guessHistory: this.guessHistory,
				remainingWords: this.remainingWords,
				topCandidates: this.topCandidates,
				expectedRemaining: this.expectedRemaining,
				winProbability: this.winProbability,
				currentGuess: guess || "",
				statusMessage: guess ? `Recommended: ${guess} (${this.remainingWords.length} words)` : "No recommendation available."
			});
		} catch (err) {
			const msg = err instanceof Error ? err.message : String(err);
			this.emitAssist({
				phase: "idle",
				statusMessage: `Error: ${msg}`
			});
		} finally {
			this.isRunning = false;
		}
	}
	async computeBestGuess() {
		let bestGuess = null;
		if (this.remainingWords.length === 0) {
			let fullDictRemaining = [...wordList];
			for (const g of this.guessHistory) fullDictRemaining = filterWordList(fullDictRemaining, g.word, g.result);
			if (fullDictRemaining.length > 0) {
				this.remainingWords = fullDictRemaining;
				this.emit({
					remainingWords: fullDictRemaining,
					statusMessage: "Switched to full dictionary pool..."
				});
			} else {
				if (!this.config.llmFallbackEnabled) return null;
				this.emit({ statusMessage: "🤖 Asking Groq for help..." });
				let retries = 2;
				while (retries >= 0) {
					const llmResult = await askGrokForGuess(this.guessHistory, `The word has ${this.currentRow} guesses so far.`, []);
					if (!("error" in llmResult)) {
						bestGuess = llmResult.word;
						this.topCandidates = [{
							word: bestGuess,
							entropy: 0,
							remainingCount: 0
						}];
						break;
					}
					retries--;
				}
				return bestGuess || "CRANE";
			}
		}
		if (this.currentRow === 0 && this.guessHistory.length === 0) {
			bestGuess = "ADIEU";
			this.topCandidates = [{
				word: bestGuess,
				entropy: 5.74,
				remainingCount: this.remainingWords.length
			}];
			this.expectedRemaining = this.remainingWords.length / Math.pow(2, 5.74);
			this.winProbability = calculateWinProbability(this.remainingWords.length, this.currentRow, 5.74, false);
			return bestGuess;
		}
		const candidates = await rankGuessesAsync(this.remainingWords);
		if (candidates.length === 0) return null;
		bestGuess = candidates[0].word;
		this.topCandidates = candidates.slice(0, 10);
		this.expectedRemaining = this.remainingWords.length / Math.pow(2, candidates[0].entropy);
		this.winProbability = calculateWinProbability(this.remainingWords.length, this.currentRow, candidates[0].entropy, false);
		if (!this.config.hardMode && this.remainingWords.length > 1 && this.remainingWords.length <= 150) {
			const fullScored = await rankGuessesAsync(this.remainingWords, wordList);
			const bestValid = candidates[0];
			const bestFull = fullScored[0];
			if (bestFull && bestValid) {
				if (bestFull.entropy - bestValid.entropy > .15 && this.remainingWords.length >= 3) {
					this.topCandidates = fullScored.slice(0, 10);
					bestGuess = bestFull.word;
					this.expectedRemaining = this.remainingWords.length / Math.pow(2, bestFull.entropy);
					this.winProbability = calculateWinProbability(this.remainingWords.length, this.currentRow, bestFull.entropy, true);
					this.emit({ statusMessage: `Exploratory guess: ${bestGuess}` });
				}
			}
		}
		return bestGuess;
	}
};
//#endregion
//#region extension/src/background/background.ts
/**
* Chrome Extension Service Worker (Background Script).
* 
* Central message router and solver orchestrator.
* - Routes messages between popup and content script
* - Runs the solver orchestrator
* - Handles Groq API calls (content scripts can't make cross-origin requests)
* - Persists config in chrome.storage.local
*/
var orchestrator = null;
var currentTabId = null;
var latestState = {
	isRunning: false,
	mode: "auto",
	phase: "idle",
	statusMessage: "Ready"
};
var DEFAULT_CONFIG = {
	hardMode: false,
	llmFallbackEnabled: false,
	typingDelay: 120,
	groqApiKey: ""
};
async function loadConfig() {
	return new Promise((resolve) => {
		chrome.storage.local.get(["solverConfig"], (result) => {
			resolve({
				...DEFAULT_CONFIG,
				...result.solverConfig || {}
			});
		});
	});
}
async function saveConfig(config) {
	return new Promise((resolve) => {
		chrome.storage.local.set({ solverConfig: config }, resolve);
	});
}
function createBrowserBridge(tabId) {
	function sendToContent(message) {
		return new Promise((resolve, reject) => {
			chrome.tabs.sendMessage(tabId, message, (response) => {
				if (chrome.runtime.lastError) {
					reject(new Error(chrome.runtime.lastError.message));
					return;
				}
				if (response?.success) resolve(response.data);
				else reject(new Error(response?.error || "Unknown content script error"));
			});
		});
	}
	return {
		async readBoard() {
			return sendToContent({ type: "READ_BOARD" });
		},
		async typeWord(word, delay) {
			await sendToContent({
				type: "TYPE_WORD",
				word,
				delay
			});
		},
		async submitGuess() {
			await sendToContent({ type: "SUBMIT_GUESS" });
		},
		async waitForReveal(rowIndex) {
			return sendToContent({
				type: "WAIT_REVEAL",
				row: rowIndex
			});
		},
		async updateBadge(text) {
			try {
				await sendToContent({
					type: "UPDATE_BADGE",
					text
				});
			} catch {}
		}
	};
}
function onSolverUpdate(update) {
	latestState = {
		...latestState,
		...update
	};
	const message = {
		type: "SOLVER_UPDATE",
		state: update,
		mode: update.mode,
		isRunning: update.isRunning
	};
	chrome.runtime.sendMessage(message).catch(() => {});
}
async function findWordleTab() {
	const [activeTab] = await chrome.tabs.query({
		active: true,
		currentWindow: true
	});
	if (activeTab && activeTab.id) return activeTab.id;
	const tabs = await chrome.tabs.query({ lastFocusedWindow: true });
	for (const tab of tabs) if (tab.id) return tab.id;
	return null;
}
async function pingContentScript(tabId) {
	return new Promise((resolve) => {
		chrome.tabs.sendMessage(tabId, { type: "PING" }, (response) => {
			if (chrome.runtime.lastError || !response?.success) resolve({ alive: false });
			else resolve({
				alive: true,
				isSupported: response.isSupported,
				gameInfo: response.gameInfo
			});
		});
	});
}
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	const handler = async () => {
		switch (message.type) {
			case "START_SOLVER": {
				const tabId = await findWordleTab();
				if (!tabId) {
					sendResponse({
						success: false,
						error: "No active Wordle tab found. Open a supported Wordle game page first."
					});
					return;
				}
				if (latestState.isRunning && message.mode === "auto") {
					sendResponse({ success: true });
					return;
				}
				const status = await pingContentScript(tabId);
				if (!status.alive) {
					sendResponse({
						success: false,
						error: "Content script not responding. Refresh the page and try again."
					});
					return;
				}
				if (status.isSupported === false) {
					sendResponse({
						success: false,
						error: "Unsupported Website: This Wordle variant is not supported yet."
					});
					return;
				}
				currentTabId = tabId;
				const config = await loadConfig();
				orchestrator = new SolverOrchestrator(onSolverUpdate);
				orchestrator.updateConfig(config);
				const bridge = createBrowserBridge(tabId);
				if (message.mode === "auto") {
					orchestrator.startAutoSolve(bridge);
					sendResponse({ success: true });
				} else {
					await orchestrator.computeAssist(bridge);
					sendResponse({ success: true });
					chrome.tabs.sendMessage(tabId, {
						type: "START_ASSIST_LOOP",
						currentRow: latestState.currentRow
					}, () => {
						if (chrome.runtime.lastError) {}
					});
				}
				break;
			}
			case "STOP_SOLVER": {
				if (orchestrator) orchestrator.stop();
				const tabId = currentTabId || await findWordleTab();
				if (tabId) chrome.tabs.sendMessage(tabId, {
					type: "UPDATE_BADGE",
					text: "🧠 Solver Ready"
				}, () => {
					if (chrome.runtime.lastError) {}
				});
				sendResponse({ success: true });
				break;
			}
			case "GET_STATE":
				sendResponse({
					success: true,
					data: latestState
				});
				break;
			case "UPDATE_CONFIG": {
				const updated = {
					...await loadConfig(),
					...message.config
				};
				await saveConfig(updated);
				if (message.config.groqApiKey !== void 0) await new Promise((resolve) => {
					chrome.storage.local.set({ groqApiKey: message.config.groqApiKey }, resolve);
				});
				if (orchestrator) orchestrator.updateConfig(updated);
				sendResponse({ success: true });
				break;
			}
			case "GET_CONFIG":
				sendResponse({
					success: true,
					data: await loadConfig()
				});
				break;
			default: sendResponse({
				success: false,
				error: `Unknown message type`
			});
		}
	};
	handler();
	return true;
});
chrome.runtime.onInstalled.addListener(async () => {
	console.log("[Wordle Solver] Extension installed.");
	await saveConfig(await loadConfig());
});
console.log("[Wordle Solver] Service worker loaded.");
//#endregion
