...
/**
 * Build a prompt that transfers all solver context to the LLM.
 */
...
- You MUST guess a real, valid dictionary word. Do not invent fake words (like ICICY).
- Do not guess insect, plant, or similar names. Use only Wordle-appropriate words. Also look for words that are globally used from other languages.

FORBIDDEN WORDS (Do NOT guess these under any circumstances):
${guessHistory.length > 0 ? guessHistory.map(g => g.word).join(', ') : 'None'}

Based on the constraints above, suggest the single best 5-letter word to guess next. Think step-by-step about which letters are confirmed, which positions are locked, and which letters are eliminated.

IMPORTANT: Respond with ONLY the 5-letter word in uppercase, nothing else. No explanation, no punctuation, just the word.`;
}
...
export async function askGrokForGuess(
  ...
  if (!apiKey) {
    return { error: 'Groq API key not configured. Add VITE_GROK_API_KEY to your .env file.' };
  }
  ...
  try {
    ...
    // Extract a 5-letter word from the response
